...
while($data = oci_fetch_array($smt, OCI_BOTH+OCI_RETURN_NULLS+OCI_RETURN_LOBS)) {
    ...
    echo "prodcd = ".$data[PRODCD]." / colorcd = ".$data[COLORCD]." / prodnm = ".$data[PRODNM]." / category = ".$category." / productcode = ".$productcode." / sizearr = ".$sizearr."\r\n";
	...
	$sabangnet_prop			= getJungboInfo("001", $mixrate, $data[COLORCD], $sizeopt[SIZE], $data[DELI_DATE], $data[PROD_NATION]);
	...
	FROM (SELECT  A.SIZE_CODE AS SIZECD,
	 ...
	 WHERE 1=1
	   ...
	   AND (B.REALPART_GB = '1' AND B.PART_DIV IN ('D','G','K')
		OR  (B.PART_DIV = 'A' AND B.PART_NO = '1803'))
