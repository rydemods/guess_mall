...
<?php
#######################################################################################
# FileName          : cr_send_grade_mail_test3.php
# Desc              : 매년 2월1일 00시에 실행되어 등급조정 및 메일링과 쿠폰 지급(구매확정기준)
...
# VIP STAR          : #dd3e77
##!/usr/local/php/bin/php
# [deco@deco1 batch]$ ./run_send_grade_mail3.sh
#######################################################################################
...
while($row = pmysql_fetch_object($result)) {
	...
	if($row->af_group==0000){
		$sql2="select name, id, email, news_yn, bf_group, coalesce(NULLIF(M_grade,''), '0003') as af_group, act
            from (
                    SELECT  name, id, min(email) as email, min(news_yn) as news_yn, min(bf_group) as bf_group, sum(act) as act,
							(CASE 
		...
		$act = $act + $act2;
		if($act >= 300000 && $act2 >=300000)$af_group = '0007';
	}
