...
exit;
/** 우편번호 안짤리는지 체크하자. ..실명인증 (m : 핸드폰인증, i : 아이핀 인증) 항목 추가하자..
 *  delete from tblmember where mb_nick_date != '2016-02-23' and id not in ('ikazeus', 'dong', 'ssuya', 'moondding2', 'decotest1', 'decotest2', 'decotest3', 'publish', 'ykm1', 'ykm2', 'belle');
 *  delete from tblpoint;
 *  0. 구분을 위해 mb_nick_date 에 '2016-03-10' 값을 넣었음.
...
$sql = "
        ...
        (id, passwd, name, email, mobile, news_yn, 
         ...
         mb_type, mb_nick_date, member_grade, auth_type 
        ) 
        SELECT 	a.m2 AS id,  a.m53 AS passwd, a.m48 AS name, a.m51 AS email, a.m50 AS mobile, (CASE WHEN a.m23 = 'T' THEN 'Y' ELSE 'N' END) AS news_yn, 
                ...
                'web' as mb_type, '2016-03-10' as mb_nick_date, 
                (case when a.m4 = 'A' then 1 when a.m4 = 'S' then 2 when a.m4 = 'C' then 3 else 4 end) as member_grade, 
                (case when a.m40 = 'm' then 'mobile' when a.m40 = 'i' then 'ipin' else '' end) as auth_type 
        FROM 	deconc_member a
        LEFT JOIN deconc_member_addr b ON a.m2 = b.m6 
        where   a.m2 not in ('ikazeus') 
        order by a.m6 asc 
        ";
