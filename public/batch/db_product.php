...
function GetOpt1Name($goodsno, $op_type, $op_type_code = '') {
    ...
    $opt1_name = "";
    if($op_type == "0") {   // 조합형
        $qry = "select p5 from deco_product_shopinfo where p3 = $goodsno And p14 = 'F' Group by p5";
        list($opt1_name) = pmysql_fetch($qry);

        $opt1_name = str_replace("#$%", "@#", $opt1_name);

    } else {    // 독립형
        $qry = "SELECT	o7 FROM deco_product_optioninfo_link WHERE o1 = '$op_type_code' AND o4 = 'T' GROUP BY o7 ORDER BY max(o10) ASC";
...
function GetOpt1TF($goodsno, $op_type, $op_type_code = '') {
    ...
    if($op_type == "0") {   // 조합형은 무조건 필수
        ...
        $qry = "SELECT	o7, MAX(o11) AS o11 FROM deco_product_optioninfo_link WHERE o1 = '$op_type_code' AND o4 = 'T' GROUP BY o7 ORDER BY max(o10) ASC";
        $ret = pmysql_query($qry);
        while($row = pmysql_fetch_object($ret)) {
            $opt1_tf .= $row->o11."@#";
        }
...
function DbInsertProductMultiImages($goodsno, $productcode) {
    ...
    $cnt = 0;
    $sql = "SELECT * FROM deco_product_etc_img WHERE pe1 = $goodsno ORDER BY pe2";
