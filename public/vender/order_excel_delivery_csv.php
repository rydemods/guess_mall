...
Header("Content-Disposition: attachment; filename=order_excel_delivery_csv_".date("Ymd",$CurrentTime).".csv"); 
...
$excel_info = "60,35,29,21,36,38,53,6,25,23,11,18,19,15,16,54,1,55,57,58,56,3,59,34";
...
$excelval=array(
	...
	array("브랜드"									,&$brand),				#52
	array("자체품목코드"							,&$self_goods_code),			#53
...
$qry.= "WHERE 1=1 ";
$qry.= "AND a.vender='".$_VenderInfo->getVidx()."' ";
...
$sql = "SELECT  a.vender, v.com_name, a.ordercode, a.productcode, a.productname, a.opt1_name, a.opt2_name, 
                ...
                b.paymethod, b.oi_step1, b.oi_step2, a.self_goods_code 
        FROM {$qry_from} {$qry} 
...
while ($row=pmysql_fetch_object($result)) {
	...
	if($isproductall=="N") {
		for($i=0;$i<$cnt;$i++) {
			if($i!=0) echo ",";
			if ($arr_excel[$i] == '36' || $arr_excel[$i] == '53') echo "=";
			echo '"' . doubleQuote(iconv('UTF-8', 'EUC-KR', $excelval[$arr_excel[$i]][1])) . '"';
		}
		echo "\n";
	}
}
