<?php
	...
	if($idxs) {
		...
		if($mode == "1") {	// 해당 주문건 배송준비중 => 결제완료
			...
			$log_content = "## 주문건 배송준비중 => 결제완료 처리 ## - 처리자 ".$_VenderInfo->getId()." - 주문건 : ".$orderproductinfo." - 시간 : ".$curdate;
			$_VenderInfo->ShopVenderLog($_VenderInfo->getVidx(),$connect_ip,$log_content);
			echo "<script>parent.opener.location.reload();alert('결제완료로 처리하였습니다.'); parent.location.reload();</script>";
			exit;
			...
			$log_content = "## 주문건 배송중 => 배송준비중 처리 ## - 처리자 ".$_VenderInfo->getId()." - 주문건 : ".$orderproductinfo." - 시간 : ".$curdate;
			$_VenderInfo->ShopVenderLog($_VenderInfo->getVidx(),$connect_ip,$log_content);
			echo "<script>parent.opener.location.reload();alert('배송준비중으로 처리하였습니다.'); parent.location.reload();</script>";
			exit;
		}
	}
?>