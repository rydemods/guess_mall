	...
	if(strlen($likecode)==0) {			//0차분류 (0차에 속한 모든 1차,2차분류를 보여준다) - 2차가 있는지 검사
		...
		if($cnt>0) {
			...
			while($row=pmysql_fetch_object($result)) {
				...
				$category_list.="<tr>";
				$category_list.="	<td width=\"25%\" bgcolor=\"#F6F6F6\" style=\"padding:10px;\"><img src=\"".$Dir."images/common/brandproduct/".$_bdata->list_type."/plist_skin_iconaa.gif\" border=\"0\" align=\"absmiddle\" hspace=\"5\"><a href=\"".$Dir.FrontDir."productblist.php?".$brand_link."code=".$row->code_a.$row->code_b.$row->code_c.$row->code_d."\"><FONT class=upcodename>".$row->code_name."</font></a></td>\n";
...
?>
	...
	<tr>
		...
		<col></col>
		<tr>
			<td colspan="2" style="padding-left:10px;padding-bottom:5px;line-height:24px;" class=choicecodename><?=($_cdata->code_name?$_cdata->code_name:$_bdata->brandname)?></td>
		</tr>
		<tr>
...
<?
		...
		while($row=pmysql_fetch_object($result)) {
			...
			if ($i!=0 && $i%2==0) {
				echo "</tr><tr><td height=\"1\" colspan=\"3\" background=\"".$Dir."images/common/brandproduct/".$_bdata->list_type."/plist_skin_line3.gif\"></td></tr>\n";
			}
			echo "<td>\n";
