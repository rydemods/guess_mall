					...
					*/
					...
					if ($strResultCode == 1) {
						...
						if ($sCPRequest == $strCPRequest) {
							...
							$user_name = iconv("euckr", "utf8", $strUserName);
// 							echo "<script>alert('$sRtnMsg'); opener.parent.ipin_chk('ipin');  self.close();</script>";
							/*if($auth_type == "find_id"){
								echo "<script>alert('$sRtnMsg'); document.location.href = '/m/findid.php?mode=result&now_find_type=id&now_cert_type=ipin&name={$user_name}';</script>";
							}else if($auth_type == "find_pw"){
								echo "<script>alert('$sRtnMsg'); document.location.href = '/m/findid.php?mode=pw_change&now_find_type=pw&now_cert_type=ipin&name={$user_name}';</script>";
							}else{*/
								if ($arrFormData[0] == 'erp_mem_join') {
?>
