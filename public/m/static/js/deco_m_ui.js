...
$(function() {
	...
	var $main = $(".js-main").each(function() {
		...
		function main_resize() {
			...
			//$listInner.height($content.eq(tempNum).outerHeight()); 2016-07-04 삭제 > height 삽입방지 - 유창
		...
		function page_scroll(_e) {
			...
			$(window).scroll(function(){
				if( $(this).scrollTop() > 200 ){
					$('.js-btn-top').fadeIn(100);
				}else{
					$('.js-btn-top').fadeOut(100);
				}
			})
...
//	$(".js-shop-hero").carousel({ list:".js-carousel-list ul", isAuto:true });
	...
	$(".js-shop-cash").each(function() {
		...
		*/
		...
		var $ui = $(this);
		var $sliderPage = $ui.find(".js-carousel-page");
		var $content = $ui.find(".js-carousel-list .js-carousel-content");
		var $uiHeight = $ui.find(".js-carousel-list ");
		var pageNum = 0;
...
function hasScrolled() {
    ...
    if (st > lastScrollTop && st > toolbarHeight){ //스크롤 내릴때 툴바,위젯 사라지기
		...
		$('.js-widget.is_web').stop().transition({y:95,delay:0},200,'linear');
		$('.js-btn-top.is_web').stop().transition({y:84,delay:0},200,'linear');
        ...
        if(st + $(window).height() < $(document).height()) { //스크롤 올릴때 툴바,위젯 나타나기
			...
			$('.js-widget.is_web').stop().transition({y:0,delay:0},200,'linear');
			$('.js-btn-top.is_web').stop().transition({y:37,delay:0},200,'linear');
...
$(window).scroll(function () {
	if ($(this).scrollTop() >= 80) {
		$('.btn-back-fixed').fadeIn();
	} else {
		$('.btn-back-fixed').fadeOut();
	}
});
...
$(window).load(function(){
	...
	setTimeout(function(){
	var promo_height = $('.js-promo-list .js-tab-content').outerHeight();
	
	//$('.js-promo-list .js-tab-content').css('height',promo_height);

	
	/*if ( $('#end_promo_list').css('display') == 'block' )
	{
		$('#end_promo_list').addClass('asdf');
	}*/
