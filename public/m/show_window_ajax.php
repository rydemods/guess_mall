...
*/
	...
	if($data->section){
		$likeHtml = "<button class=\"comp-like btn-like btn_like like_p_button".$data->productcode."\" title=\"선택됨\"  ><span class=\"icon\"><i id=\"like_".$data->productcode."\" onclick=\"like.clickLike('product','".$data->productcode."','".$data->prodcode."')\" class=\"icon-like on like_p".$data->productcode."\">좋아요</i></span><span id=\"like_cnt_".$data->productcode."\" class=\"count\">".$data->hott_cnt."</span></button>";
	}else{
		$likeHtml = "<button class=\"comp-like btn-like btn_like like_p_button".$data->productcode."\" title=\"선택안됨\" ><span class=\"icon\"><i id=\"like_".$data->productcode."\" onclick=\"like.clickLike('product','".$data->productcode."','".$data->prodcode."')\" class=\"icon-like like_p".$data->productcode."\">좋아요</i></span><span id=\"like_cnt_".$data->productcode."\" class=\"count\">".$data->hott_cnt."</span></button>";																												
	}
	...
	?>
		<li class="swd_item">
			<div class="brand_info">
				<span class="brand"><img src="../data/shopimages/brand/<?=$brandlogo?>" alt="VIKI"></span>
				<span class="shop"><?=$storename?></span>
			</div>
			<div class="goods_area">
				<a href="<?=$prod_link?>">
					<div class="img">
						<img src="<?=$data->img_url?>" alt="쇼윈도 상품 이미지">
						<span class="price_area"><del class=<?=$consumer_class?>>￦<?=$consumerprice?></del> <span class=<?=$soldout_class?>>￦</span><?=$sellprice?></span>
						<?if($dcrate){?><span class="tag_sale"><strong><?=$dcrate?></strong>%</span><?}?>
					</div>
					<div class="exp">
						<p class="tit"><?=$data->productname?></p>
