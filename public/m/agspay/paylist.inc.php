...
$pgurl=$Dir."m/agspay/AGS_pay.php?storeid=".$storeid."&storenm=".urlencode(titleCut(47,$_data->shopname))."&ordno=".urlencode($ordercode)."&prodnm=".urlencode($goodname)."&amt=".$last_price."&userid=".$_ShopInfo->getMemid()."&useremail=".urlencode($sender_email)."&ordnm=".urlencode($sender_name)."&ordphone=".urlencode($sender_tel)."&rcpnm=".urlencode($receiver_name)."&rcpphone=".urlencode($receiver_tel1)."&escrow=".$escrow."&paymethod=".$paymethod."&hp_id=".$hp_id."&hp_pwd=".encrypt_md5($hp_pwd)."&hp_unittype=".$hp_unittype."&hp_subid=".$hp_subid."&prodcode=".$prodcode;
