...
********************************************************************************/
	...
	****************************************************************************/
	...
	$agspay->SetValue("AgsPayHome",$_SERVER[DOCUMENT_ROOT]."/m/agspay");			//올더게이트 결제설치 디렉토리 (상점에 맞게 수정)
	...
	****************************************************************************/
	...
	{ 
		if($agspay->GetResult("AuthTy") == "virtual"){
			...
			$PAY_FLAG="0000"; // 결과 코드(0000:정상승인, 9999:승인실패)
			$DELI_GBN="N"; // 배송여부(N:미처리, C:주문취소)
			$MSG1=$agspay->GetResult("rResMsg");

			if($agspay->GetResult("VIRTUAL_CENTERCD") == "20"){
				$pay_data="우리은행(20) ".$agspay->GetResult("rVirNo")." (예금주:이지스효성)";
			}else if($agspay->GetResult("VIRTUAL_CENTERCD") == "88"){
				$pay_data="신한은행(88) ".$agspay->GetResult("rVirNo")." (예금주:이지스효성)";
			}else if($agspay->GetResult("VIRTUAL_CENTERCD") == "34"){
				$pay_data="광주은행(34) ".$agspay->GetResult("rVirNo")." (예금주:이지스효성)";
			}else if($agspay->GetResult("VIRTUAL_CENTERCD") == "04"){
				$pay_data="국민은행(04) ".$agspay->GetResult("rVirNo")." (예금주:이지스효성)";
			}else if($agspay->GetResult("VIRTUAL_CENTERCD") == "11"){
				$pay_data="농협중앙회(11) ".$agspay->GetResult("rVirNo")." (예금주:이지스효성)";
			}else if($agspay->GetResult("VIRTUAL_CENTERCD") == "31"){
				$pay_data="대구은행(31) ".$agspay->GetResult("rVirNo")." (예금주:이지스효성)";
			}else if($agspay->GetResult("VIRTUAL_CENTERCD") == "32"){
				$pay_data="부산은행(32) ".$agspay->GetResult("rVirNo")." (예금주:이지스효성)";
			}else if($agspay->GetResult("VIRTUAL_CENTERCD") == "02"){
				$pay_data="산업은행(02) ".$agspay->GetResult("rVirNo")." (예금주:이지스효성)";
			}else if($agspay->GetResult("VIRTUAL_CENTERCD") == "45"){
				$pay_data="새마을금고(45) ".$agspay->GetResult("rVirNo")." (예금주:이지스효성)";
			}else if($agspay->GetResult("VIRTUAL_CENTERCD") == "07"){
				$pay_data="수협중앙회(07) ".$agspay->GetResult("rVirNo")." (예금주:이지스효성)";
			}else if($agspay->GetResult("VIRTUAL_CENTERCD") == "48"){
				$pay_data="신용협동조합(48) ".$agspay->GetResult("rVirNo")." (예금주:이지스효성)";
			}else if($agspay->GetResult("VIRTUAL_CENTERCD") == "26"){
				$pay_data="(구)신한은행(26) ".$agspay->GetResult("rVirNo")." (예금주:이지스효성)";
			}else if($agspay->GetResult("VIRTUAL_CENTERCD") == "05"){
				$pay_data="외환은행(05) ".$agspay->GetResult("rVirNo")." (예금주:이지스효성)";
			}else if($agspay->GetResult("VIRTUAL_CENTERCD") == "71"){
				$pay_data="우체국(71) ".$agspay->GetResult("rVirNo")." (예금주:이지스효성)";
			}else if($agspay->GetResult("VIRTUAL_CENTERCD") == "37"){
				$pay_data="전북은행(37) ".$agspay->GetResult("rVirNo")." (예금주:이지스효성)";
			}else if($agspay->GetResult("VIRTUAL_CENTERCD") == "23"){
				$pay_data="제일은행(23) ".$agspay->GetResult("rVirNo")." (예금주:이지스효성)";
			}else if($agspay->GetResult("VIRTUAL_CENTERCD") == "35"){
				$pay_data="제주은행(35) ".$agspay->GetResult("rVirNo")." (예금주:이지스효성)";
			}else if($agspay->GetResult("VIRTUAL_CENTERCD") == "21"){
				$pay_data="(구)조흥은행(21) ".$agspay->GetResult("rVirNo")." (예금주:이지스효성)";
			}else if($agspay->GetResult("VIRTUAL_CENTERCD") == "03"){
				$pay_data="중소기업은행(03) ".$agspay->GetResult("rVirNo")." (예금주:이지스효성)";
			}else if($agspay->GetResult("VIRTUAL_CENTERCD") == "81"){
				$pay_data="하나은행(81) ".$agspay->GetResult("rVirNo")." (예금주:이지스효성)";
			}else if($agspay->GetResult("VIRTUAL_CENTERCD") == "27"){
				$pay_data="한미은행(27) ".$agspay->GetResult("rVirNo")." (예금주:이지스효성)";
			}
			...
			$MSG1="정상승인 - 승인번호 : ".$agspay->GetResult("rApprNo");
			$pay_data="승인번호 : ".$agspay->GetResult("rApprNo");
	...
	if($temp!="ok") {
		...
		if(strlen(AdminMail)>0) {
			@mail(AdminMail,"[PG] ".$agspay->GetResult("rOrdNo")." 결제정보 업데이트 오류","$return_host<br>$return_script<br>$return_data");
		}
	} else {
