<?
//관리자 페이지 카테고리 설정에만 쓰이는 클래스 입니다.
## /admin/forum_code.php 에서만 쓰입니다.
## 09 19 원재 ㅠㅠ
class CATEGORYLIST_FORUM{

	function CATEGORYLIST_FORUM(){
		
	}

	function getCateTree(){ //상품 카테고리 리스트

		$sql = "SELECT * FROM tblforumcode where code_b='000' ORDER BY cate_sort, code_name ";
		...
		$clz='';
			...
			if(strstr($row->type,'X')){
				...
				$sql_b = "SELECT * FROM tblforumcode where code_a='".$row->code_a."' and code_b!='000' and code_c='000' ORDER BY cate_sort, code_name ";
				...
				while ($row_b = pmysql_fetch_object($result_b)) {
					...
					if(strstr($row_b->type,'X')){
						...
						$sql_c = "SELECT * FROM tblforumcode where code_a='".$row_b->code_a."' and code_b='".$row_b->code_b."' and code_c!='000' and code_d='000' ORDER BY cate_sort, code_name";
						...
						while ($row_c = pmysql_fetch_object($result_c)) {
							...
							if(strstr($row_c->type,'X')){
								...
								$sql_d = "SELECT * FROM tblforumcode where code_a='".$row_c->code_a."' and code_b='".$row_c->code_b."' and code_c='".$row_c->code_c."' and code_d!='000' ORDER BY cate_sort, code_name";
