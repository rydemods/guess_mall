...
$(function() {
	...
	$(".rollingwrap").each(function(index) {
		var $event.push($("#"+$(this).attr('id')));
		var $arrow.push($event[index].find("> span > a.arrow"));
		var $menu.push($event[index].find("> nav li"));
		var $container.push($event[index].find(".containerB"));
		var $list.push($container[index].find("> ul"));
		var $content.push($list[index].find("> li"));
		var contentTotal.push($content[index].length);
		var pageNum.push('0');
		var viewNum.push('1');
		var scrollTop.push('');
		var timer.push('');
