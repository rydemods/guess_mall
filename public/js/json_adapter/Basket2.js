				...
				if(gubun=='M'){
					...
					if(delivery_type=='1'){
						...
						if(sum_div_sellprice >= deli_miniprice){
							...
							sum_div_basong = deli_basefee_origin;
						}
						
						//임직원
						if(sum_div_sellprice_staff >= deli_miniprice){
							sum_div_basong_staff = 0;
						}else{
							sum_div_basong_staff = deli_basefee_origin;
						}
						
						//협력
						if(sum_div_sellprice_cooper >= deli_miniprice){
							sum_div_basong_cooper = 0;
						}else{
							sum_div_basong_cooper = deli_basefee_origin;
						}
					...
					//중간상품합계
					sum_div_totalprice = sum_div_sellprice + Number(sum_div_basong);
					
					//중간상품합계(임직원)
					sum_div_totalprice_staff = sum_div_sellprice_staff + Number(sum_div_basong_staff) ;
					sum_div_totalprice_cooper = sum_div_sellprice_cooper + Number(sum_div_basong_cooper) ;
