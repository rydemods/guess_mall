...
//        $new_arrival_html .= '<li><a href="/front/productdetail.php?productcode=' . $prod_code . '"><span class="img"><img src="/data/shopimages/product/' . $prod_thumb . '" alt="' . $prod_name . '" width="230" height="230"></span><span class="nm">' . $prod_name . '</span></a></li>';
    ...
    // ================================================================================================================================
    // 2,3,4차 카테고리 리스트 보여주기
    // ================================================================================================================================

    $category_list_html = '';


    $sub_sql  = "SELECT code_a,code_b,code_c,code_d, code_a||code_b||code_c||code_d as cate_code,code_name,idx,cate_sort ";
    $sub_sql .= "FROM tblproductcode ";
    ...
    $arrCate2 = array();    // 2차 카테고리
    $arrCate3 = array();    // 3차 카테고리
    $arrCate4 = array();    // 4차 카테고리

    while ( $sub_row = pmysql_fetch_object($sub_result) ) {
        if ( $sub_row->code_c == "000" ) {
            // 2차 카테고리
            $arrCate2[$sub_row->cate_code] = array($sub_row->cate_sort, $sub_row->cate_code, $sub_row->code_name);
        } elseif ( $sub_row->code_d == "000" ) {
            // 3차 카테고리
            $arrCate3[$sub_row->cate_code] = array($sub_row->cate_sort, $sub_row->code_name);
        } elseif ( $sub_row->code_d != "000" ) {
            // 4차 카테코리
            $arrCate4[$sub_row->cate_code] = array($sub_row->cate_sort, $sub_row->code_name);
        } 
    }
    pmysql_free_result($sub_result);
...
/*
    ...
    $sub_sql  = "SELECT code_a,code_b,code_c,code_d, code_a||code_b||code_c||code_d as cate_code,code_name,idx,cate_sort ";
    $sub_sql .= "FROM tblproductcode ";
    $sub_sql .= "WHERE code_a = '" . $cateListA_row->code_a . "' AND code_b != '000' AND code_c = '000' AND group_code !='NO' AND display_list is NULL ";
...
exdebug($sub_sql);
    ...
    while ( $sub_row = pmysql_fetch_object($sub_result) ) {
        ...
        $category_list_html .= '<div class="submenu_container">
                                    <h3>' . $sub_row->code_name . '</h3>
                                    <ul>';

        // 3차 카테고리
        $sub_sql  = "SELECT code_a,code_b,code_c,code_d, code_a||code_b||code_c||code_d as cate_code,code_name,idx FROM tblproductcode ";
        $sub_sql .= "WHERE code_a = '" . $sub_row->code_a . "' AND code_b = '" . $sub_row->code_b . "' AND code_c <> '000' AND code_d = '000' ";
        $sub_sql .= "AND group_code !='NO' AND display_list is NULL ";
        $sub_sql .= "ORDER BY cate_sort ASC ";

exdebug($sub_sql);

        $sub_result2 = pmysql_query($sub_sql);

        while ( $sub_row2 = pmysql_fetch_array($sub_result2) ) {

            $cate_code = $sub_row2->cate_code;
            $cate_code_3depth = $sub_row2->code_a.$sub_row2->code_b.$sub_row2->code_c;

            // 4차 카테고리
            $sub_sql  = "SELECT code_a,code_b,code_c,code_d, code_a||code_b||code_c||code_d as cate_code,code_name,idx FROM tblproductcode ";
            $sub_sql .= "WHERE code_a = '" . $sub_row2->code_a . "' AND code_b = '" . $sub_row2->code_b . "' AND code_c = '" . $sub_row2->code_c . "' AND code_d = '000' ";
            $sub_sql .= "AND group_code !='NO' AND display_list is NULL ";
            $sub_sql .= "ORDER BY cate_sort ASC ";

exdebug($sub_sql);

            $sub_result3 = pmysql_query($sub_sql);

            $lastDepthCnt = 0;
            $last_category_list_html = '';
            while ( $sub_row3 = pmysql_fetch_array($sub_result3) ) {
                $last_category_list_html .= '<li><a href="' . $Dir. FrontDir . "productlist.php?code=" . $sub_row3->cate_code . '">' . $sub_row3->code_name . '</a></li>';
                $lastDepthCnt++;
            }
            pmysql_free_result($sub_result3);
            ...
            if ( $lastDepthCnt > 0 ) {
                ...
                $category_list_html .= $last_category_list_html;
            ...
            $category_list_html .= '<a href="' . $Dir.FrontDir."productlist.php?code=" . $cate_code . '">' . $sub_row2->code_name . '</a>';

            $category_list_html .= '</li>';
        } 
        pmysql_free_result($sub_result2);
    }
    pmysql_free_result($sub_result);
*/
