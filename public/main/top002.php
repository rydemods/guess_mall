	...
	<tr>
		...
		<tr valign="bottom">
			...
			<tr>
				...
				<tr>
					...
					<tr>
						<td align="right"><?if($_data->ETCTYPE["TAGTYPE"]!="N") {?><a href="<?=$Dir.FrontDir?>tag.php" <?=$main_target?>><IMG SRC="<?=$Dir?>images/<?=$_data->icon_type?>/main_skin1_top_teg.gif" border="0"></a><?}?></td>
						<td align="right"><a href="<?=$Dir.FrontDir?>rssinfo.php" <?=$main_target?>><IMG SRC="<?=$Dir?>images/<?=$_data->icon_type?>/main_skin1_top_rss.gif" border="0" hspace="5"></a></td>
						<td style="padding-left:4px"><input type=text name="id" size="10" onkeydown="this.style.backgroundImage='';" onmousedown="this.style.backgroundImage='';" style="width:60px;height:18px;font-size:11px;background-color:#EDEDED;padding-top:2pt;padding-bottom:1pt;border-top:#C2C2C2 1px solid;border-right:#FFFFFF 1px solid;border-bottom:#FFFFFF 1px solid;border-left:#C2C2C2 1px solid; border-style:solid;height:19px;background-image:url('<?=$Dir?>images/<?=$_data->icon_type?>/main_skin1_login_text01.gif');background-repeat:no-repeat;background-position:left;"></td>
						<td style="padding-left:3px;"><input type=password name="passwd" size="10" onkeydown="TopCheckKeyLogin();this.style.backgroundImage='';" onmousedown="this.style.backgroundImage='';" style="width:60px;height:18px;font-size:11px;background-color:#EDEDED;padding-top:2pt;padding-bottom:1pt;border-top:#C2C2C2 1px solid;border-right:#FFFFFF 1px solid;border-bottom:#FFFFFF 1px solid;border-left:#C2C2C2 1px solid; border-style:solid;height:19px;background-image:url('<?=$Dir?>images/<?=$_data->icon_type?>/main_skin1_login_text02.gif');background-repeat:no-repeat;background-position:left;"></td>
						<td>
						<?if($_data->ssl_type=="Y" && strlen($_data->ssl_domain)>0 && strlen($_data->ssl_port)>0 && $_data->ssl_pagelist["LOGIN"]=="Y") {?>
						...
						<td style="padding-left:3px;"><a href="javascript:top_login_check();"><IMG SRC="<?=$Dir?>images/<?=$_data->icon_type?>/main_skin1_top_login.gif" border="0"></a></td>
						<td><A HREF="<?=$Dir.FrontDir?>member_agree.php" <?=$main_target?>><IMG SRC="<?=$Dir?>images/<?=$_data->icon_type?>/main_skin1_top_join.gif" border="0"></a></td>
						<td><A HREF="<?=$Dir.FrontDir?>findid.php" <?=$main_target?>><IMG SRC="<?=$Dir?>images/002/main_skin1_top_btn_findid.gif" border="0"></a></td>
						<td><A HREF="<?=$Dir.FrontDir?>findpw.php" <?=$main_target?>><IMG SRC="<?=$Dir?>images/002/main_skin1_top_btn_findpw.gif" border="0"></a></td>
						<td><A HREF="<?=$Dir.FrontDir?>mypage.php" <?=$main_target?>><IMG SRC="<?=$Dir?>images/<?=$_data->icon_type?>/main_skin1_top_mypage.gif" border="0"></a></td>
					</tr>
					</table>
					<?}else{########## 로그인을 하였다 ############?>
					<table cellpadding="0" cellspacing="0">
					<tr>
						<td><?if($_data->ETCTYPE["TAGTYPE"]!="N") {?><a href="<?=$Dir.FrontDir?>tag.php" <?=$main_target?>><IMG SRC="<?=$Dir?>images/<?=$_data->icon_type?>/main_skin1_top_teg.gif" border="0" align="absmiddle" hspace="3"></a><?}?></td>
						<td align="right"><a href="<?=$Dir.FrontDir?>rssinfo.php" <?=$main_target?>><IMG SRC="<?=$Dir?>images/<?=$_data->icon_type?>/main_skin1_top_rss.gif" border="0" hspace="5"></a></td>
						<td><b><font color="#FF6600" style="font-size:8pt;"><?=$_ShopInfo->getMemid()?></font></b><font style="font-size:8pt;">님 환영합니다.</font></td>
						<td><A HREF="javascript:logout()"><IMG SRC="<?=$Dir?>images/<?=$_data->icon_type?>/main_skin1_top_logout.gif" border="0"></a></td>
						<td><A HREF="<?=$Dir.FrontDir?>mypage_usermodify.php" <?=$main_target?>><IMG SRC="<?=$Dir?>images/<?=$_data->icon_type?>/main_skin1_top_edit.gif" border="0"></a></td>
						<td><A HREF="<?=$Dir.FrontDir?>mypage.php" <?=$main_target?>><IMG SRC="<?=$Dir?>images/<?=$_data->icon_type?>/main_skin1_top_mypage.gif" border="0"></a></td>
					</tr>
					</table>
					<?}?>
		...
		<tr>
			...
			<tr>
				...
				<TR>
					<TD><a href="<?=$Dir.FrontDir?>productnew.php" <?=$main_target?>><IMG SRC="<?=$Dir?>images/<?=$_data->icon_type?>/main_skin1_topmenu1.gif" border="0"></a></TD>
					<TD><a href="<?=$Dir.FrontDir?>productbest.php" <?=$main_target?>><IMG SRC="<?=$Dir?>images/<?=$_data->icon_type?>/main_skin1_topmenu2.gif" border="0"></a></TD>
					<TD><a href="<?=$Dir.FrontDir?>producthot.php" <?=$main_target?>><IMG SRC="<?=$Dir?>images/<?=$_data->icon_type?>/main_skin1_topmenu3.gif" border="0"></a></TD>
					<TD><a href="<?=$Dir.FrontDir?>productspecial.php" <?=$main_target?>><IMG SRC="<?=$Dir?>images/<?=$_data->icon_type?>/main_skin1_topmenu4.gif" border="0"></a></TD>
				</tr>
				</table>
				</td>
				...
				<tr>
					...
					<TD><a href="<?=$Dir.FrontDir?>useinfo.php" <?=$main_target?>><IMG SRC="<?=$Dir?>images/<?=$_data->icon_type?>/main_skin1_topmenu9.gif" border="0"></a></TD>
					<TD><a href="<?=$Dir.FrontDir?>company.php" <?=$main_target?>><IMG SRC="<?=$Dir?>images/<?=$_data->icon_type?>/main_skin1_topmenu10.gif" border="0"></a></TD>
				</tr>
				</table>
				</td>
			</tr>
		...
		<tr>
			<td colspan="2" align="center" valign="top" style="padding-top:5px;padding-bottom:15px;" background="<?=$Dir?>images/<?=$_data->icon_type?>/main_skin1_top_searchbg.gif">
			<table cellpadding="0" cellspacing="0">
			<form name=search_tform method=get action="<?=$Dir.FrontDir?>productsearch.php" <?=$main_target?>>
			<tr>
				<td style="padding-right:3px;">
				<INPUT type=text name="search" value="<?=(isset($_POST["search"])?$_POST["search"]:"")?>" size="18" onkeydown="CheckKeyTopSearch()" style="font-size:11px;background-color:#FFFFFF;padding-top:2pt;padding-bottom:1pt;border-top:#808080 1px solid;border-left:#808080 1px solid;border-bottom:#E8E8E8 1px solid;border-right:#E8E8E8 1px solid; width:180px; height:19px;">
				</td>
				<td style="padding-right:3px;"><A HREF="javascript:TopSearchCheck()"><IMG SRC="<?=$Dir?>images/<?=$_data->icon_type?>/main_skin1_top_search.gif" border="0"></a></td>
				<?if($_data->search_info["bestkeyword"]=="Y"){?>
				<td><IMG SRC="<?=$Dir?>images/<?=$_data->icon_type?>/main_skin1_top_popular.gif" border="0"></td>
				<td style="padding-right:3px;"><font color="#969696" face="돋움" style="font-size:8pt;">
				<?
				$maxkeylen=30;
				$keygbn=",";
