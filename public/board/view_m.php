...
$page_code = $board;	//page_code를 현제 보고있는 게시판 형태에 맞춘다
include ("head.php");
/*
...
if($_data->icon_type == 'tem_001'){
	...
	//echo $dir."/view_head.php";
}


//comment
if ($setup['use_comment']=="Y" && $this_comment > 0 || true) {
...
//윗글
if ($s_check) {
	$p_query  = "SELECT num,thread,title,name,email,writetime FROM tblboard WHERE board='$board' ";
...
if (!$p_row['num'] || $mypageid) {
	...
	if ($setup['grant_write'] == "A") $p_row['name'] = $_data->shopname; // 쓰기가 관리자 전용이면 이름을 몰이름으로 변경한다.(2015.12.02 - 김재수)
	...
	$prevDate =  date("Y-m-d",$p_row["writetime"]);
...
//아랫글
if ($s_check) {
	$n_query  = "SELECT num,thread,title,name,email,writetime FROM tblboard WHERE board='$board' ";
...
if (!$n_row['num'] || $mypageid) {
	...
	if ($setup['grant_write'] == "A") $n_row['name'] = $_data->shopname; // 쓰기가 관리자 전용이면 이름을 몰이름으로 변경한다.(2015.12.02 - 김재수)
	...
	$nextDate =  date("Y-m-d",$n_row["writetime"]);
	...
	if ($total_re == 1) {
		...
		if($total_re>1) {
			...
			while ($row5 = pmysql_fetch_array($result_re)) {
				...
				if ($setup['grant_write'] == "A") $row5['name'] = $_data->shopname; // 쓰기가 관리자 전용이면 이름을 몰이름으로 변경한다.(2015.12.02 - 김재수)
...
//exdebug($dir."/view.php");
include ($dir."/view.php");
$com_list_cnt = 0;
if ($setup['use_comment'] == "Y" && $member['grant_comment']=="Y" || true) {
	
	@include ($dir."/comment_head_m.php");
	
	$com_list_cnt = count($com_list);
	for ($jjj=0; $jjj<$com_list_cnt; $jjj++) {
		$c_is_secret = '';
		$c_num = $com_list[$jjj]['num'];
		$c_name = $com_list[$jjj]['name'];
		$c_is_secret = $com_list[$jjj]['is_secret'];
		$c_mem_id = $com_list[$jjj]['c_mem_id'];
		...
		@include($dir."/comment_list_m.php");
	}
...
if ($setup['use_comment'] == "Y" && $member['grant_comment']=="Y" && strlen( $_ShopInfo->memid ) > 0 || true ) {
    echo "ppp";
	@include($dir."/comment_write_m.php");
}
