...
$list_url = "/{$redirectPrefix}/promotion_detail.php?idx=${promo_idx}&event_type={$event_type}&view_mode={$view_mode}&view_type={$view_type}";
$mod_url = "/{$redirectPrefix}/promotion_detail.php?num={$num}&idx={$promo_idx}&event_type={$event_type}&view_mode={$view_mode}&view_type={$view_type}";
...
if ($exec == "write") {	//글쓰기
	...
	if(($_POST['mode']=="up_result") && ($_POST['ins4e'][mode]=="up_result") && ($_POST['up_subject']!="") && ($_POST['ins4e'][up_subject]!="")) {
		...
		//if ($thread<=0) {
			$que2 = "SELECT MIN(thread) FROM tblboard_promo ";
		...
		if (!$next_no) {
			$que3 = "SELECT MAX(num) FROM tblboard_promo WHERE board='".$board."' AND promo_idx = {$promo_idx} AND pos=0 AND deleted!='1'";
		...
		if($_data->icon_type == 'tem_001'){
			...
			if( $_REQUEST["file_exist1"] == "Y" && $banner_img["up_filename"][0]["v_file"] ){
				//$banner_file->removeFile($select_data->banner_img);
				$up_filename    = $banner_img["up_filename"][0]["r_file"];
				$up_vfilename    = $banner_img["up_filename"][0]["v_file"];
            } 
            
			if( $_REQUEST["file_exist2"] == "Y" && $banner_img["up_filename"][1]["v_file"] ){
				$up_filename2   = $banner_img["up_filename"][1]["r_file"];
				$up_vfilename2   = $banner_img["up_filename"][1]["v_file"];
            }

			if( $_REQUEST["file_exist3"] == "Y" && $banner_img["up_filename"][2]["v_file"] ){
				$up_filename3   = $banner_img["up_filename"][2]["r_file"];
				$up_vfilename3   = $banner_img["up_filename"][2]["v_file"];
            }

			if( $_REQUEST["file_exist4"] == "Y" && $banner_img["up_filename"][3]["v_file"] ){
				$up_filename4   = $banner_img["up_filename"][3]["r_file"];
				$up_vfilename4   = $banner_img["up_filename"][3]["v_file"];
			}
		}else{
		...
		$sql = "INSERT INTO tblboard_promo DEFAULT VALUES RETURNING num";
		...
		if(strlen($recipe_no)>0){
			$sql.= "recipe_no			= '".$recipe_no."', ";
		}
		...
		$sql .= "vfilename			= '".$up_vfilename."', ";
		$sql .= "filename2			= '".$up_filename2."', ";
		$sql .= "vfilename2			= '".$up_vfilename2."', ";
		$sql .= "filename3			= '".$up_filename3."', ";
		$sql .= "vfilename3			= '".$up_vfilename3."', ";
		$sql .= "filename4			= '".$up_filename4."', ";
		$sql .= "vfilename4			= '".$up_vfilename4."', ";
		...
		$sql .= "total_comment		= '0', ";
		$sql .= "content			= '".$up_memo."', ";
		$sql .= "storetel			= '".$up_storetel."', ";
		$sql .= "notice				= '0', ";
		if($_ShopInfo->memid)$sql .= "mem_id				= '".$_ShopInfo->memid."', ";
		$sql .= "promo_idx          = {$promo_idx}, ";
...
//		exit;
		...
		if($row[0]>0) {
			...
			if ($next_no) {
				$qry9 = "SELECT thread FROM tblboard_promo WHERE board='$board' AND promo_idx = {$promo_idx} AND num='$next_no' ";
				$res9 = pmysql_query($qry9,get_db_conn());
				$next_thread = pmysql_fetch_row($res9);
				@pmysql_free_result($res9);
				pmysql_query("UPDATE tblboard_promo SET prev_no='{$thisNum}' WHERE board='{$board}' AND thread = '{$next_thread[0]}' AND promo_idx = {$promo_idx}",get_db_conn());

				pmysql_query("UPDATE tblboard_promo SET prev_no='{$thisNum}' WHERE board='{$board}' AND num = '{$next_no}' AND promo_idx = {$promo_idx}",get_db_conn());
			}
			...
			if ($_POST['send_route']) {
					list($sr_productcode)=pmysql_fetch("SELECT productcode FROM tblproduct WHERE pridx = '".$pridx."'");
					echo("<meta http-equiv='Refresh' content='0; URL=/front/productdetail.php?productcode=$sr_productcode'>");
			} else {
				if($mypageid){
					echo("<meta http-equiv='Refresh' content='0; URL={$list_url}&mypageid=$mypageid'>");
				}else{
					echo("<meta http-equiv='Refresh' content='0; URL={$list_url}'>");
				}
	...
	$sql2 = "SELECT * FROM tblboard_promo WHERE board='$board' AND num = $num AND promo_idx = {$promo_idx} ";
	...
	if(($_POST['mode']=="up_result") && ($_POST['ins4e'][mode]=="up_result") && ($_POST['up_subject']!="") && ($_POST['ins4e'][up_subject]!="")) {
		if($member['admin']!="SU" && $id_data->mem_id!=$_ShopInfo->memid && !$mypageid) {
			if (strlen($_POST["up_passwd"])==0) {
				...
				if($mypageid){
				echo "<html><head><title></title></head><body onload=\"alert('".$errmsg."');location.replace('{$list_url}');\"></body></html>";
				}else{
				echo "<html><head><title></title></head><body onload=\"alert('".$errmsg."');location.replace('{$list_url}');\"></body></html>";
				}
				exit;
		...
		if($_data->icon_type == 'tem_001'){
            ...
            $select_qry="select * from tblboard_promo where board='".$board."' AND num = $num ";
            ...
            for ( $i = 1; $i <= 4; $i++ ) {
                ...
                if ( $_REQUEST["file_exist{$i}"] == "Y" ) {
                    if ( $i == 1 ) {
                        $up_filename    = $banner_img["up_filename"][$i-1]["v_file"];

                        if($banner_img["up_filename"][$i-1]["v_file"]) {
                           $banner_file->removeFile($select_data->filename);
                           $sql .= "filename	= '".$banner_img["up_filename"][$i-1]["v_file"]."', ";
                           $sql .= "vfilename	= '".$banner_img["up_filename"][$i-1]["v_file"]."', ";
                        }
                    } else {
                        if($banner_img["up_filename"][$i-1]["v_file"]) {
                            if ( $i == 2 ) { $banner_file->removeFile($select_data->filename2); }
                            else if ( $i == 3 ) { $banner_file->removeFile($select_data->filename3); }
                            else if ( $i == 4 ) { $banner_file->removeFile($select_data->filename4); }

                            $sql .= "filename{$i}	= '".$banner_img["up_filename"][$i-1]["v_file"]."', ";
                            $sql .= "vfilename{$i}  = '".$banner_img["up_filename"][$i-1]["v_file"]."', ";
                        }
                    }
                } else {
                    if ( $i == 1 ) {
		...
		$sql .= "ip				= '".$_SERVER['REMOTE_ADDR']."', ";
		$sql .= "content		= '".$up_memo."' ";
		$sql .= "WHERE board='".$board."' AND num = $num AND promo_idx = $promo_idx";
		$insert = pmysql_query($sql,get_db_conn());

		if($insert) {

			if($mypageid){
			echo("<meta http-equiv='Refresh' content='0; URL={$mod_url}&mypageid={$mypageid}'>");
			}else{
			echo("<meta http-equiv='Refresh' content='0; URL={$mod_url}'>");
			}
		...
		if($member['admin']!="SU" && $id_data->mem_id!=$_ShopInfo->memid && !$mypageid) {
			...
			if(strlen($row2['passwd'])==16 && $row2['passwd']==$new_passwd) {
				@pmysql_query("UPDATE tblboard_promo SET passwd='".$_POST["up_passwd"]."' WHERE board='".$row2['board']."' AND num='".$row2['num']."' AND promo_idx = {$promo_idx}",get_db_conn());
	...
	if(($_POST['mode'] == "up_result") && ($_POST['ins4e'][mode] == "up_result") && ($_POST['up_subject'] != "") && ($_POST['ins4e'][up_subject] != "")) {
		// ======== thread, pos, depth 정의 ========
		$sql = "UPDATE tblboard_promo SET pos = pos+1 WHERE board='$board' AND promo_idx = {$promo_idx} AND thread={$row2['thread']} AND pos>{$row2['pos']} ";
		...
		if($row2[recipe_no]) $sql .= "recipe_no				= '".$row2[recipe_no]."', ";
