...
if($type=="delete" && ord($code)) {
	...
	$onload="<script>window.onload=function(){ alert(\"개별페이지 삭제가 완료되었습니다.\"); }</script>";
	$subject="";
	$menu_type="Y";
	$menu_code="";
	$member_type="Y";
	...
	if(ord($code)==0) {
		...
		if($cnt==$maxpage) {
			$onload="<script>window.onload=function(){ alert(\"개별페이지는 최대 {$maxpage}페이지까지 지원됩니다.\\n\\n다른 페이지를 삭제 후 등록하시기 바랍니다.\"); }</script>";
		} else {
			...
			'{$code}')";
			pmysql_query($sql,get_db_conn());
			$onload="<script>window.onload=function(){ alert(\"개별페이지 등록이 완료되었습니다.\"); }</script>";
		}
	} else {
		...
		leftmenu		= '{$leftmenu}', 
		body			= '{$new_body}' 
		WHERE type='newpage' AND code='{$code}' ";
		pmysql_query($sql,get_db_conn());
		$onload="<script>window.onload=function(){ alert(\"개별페이지 수정이 완료되었습니다.\"); }</script>";
	}
}
...
<div class="admin_linemap"><div class="line"><p>현재위치 : 디자인관리 &gt; 개별 추가페이지 관리 &gt;<span>일반페이지 꾸미기</span></p></div></div>
	...
	<tr>
		...
		<tr>
			...
			<tr>
				<td>
                    <!-- 페이지 타이틀 -->
					<div class="title_depth3">일반페이지 꾸미기</div>
					<div class="title_depth3_sub"><span>개별 일반페이지를 등록 및 관리하실 수 있습니다.</span></div>
                </td>
            </tr>
            <tr>
            	<td>
					<!-- 소제목 -->
					<div class="title_depth3_sub">개별페이지 디자인</div>
				</td>
			</tr>
			<tr>
				<td>
					<div class="help_info01_wrap">
							<ul>
                                ...
                                <li>3) 왼쪽메뉴를 개별 디자인한 경우 [개별디자인 적용선택]을 해야 합니다.(템플릿 사용시에는 템플릿의 디자인으로 출력)
<br />&nbsp;&nbsp;&nbsp;<a href="javascript:parent.topframe.GoMenu(2,'design_option.php');">디자인관리 > 웹FTP 및 개별적용 선택 > 개별디자인 적용선택</a> [상단+왼쪽 동시 적용][왼쪽만 적용]</li>
								<li>4) 프레임 설정 메뉴<br />&nbsp;&nbsp;
    <a href="javascript:parent.topframe.GoMenu(1,'shop_displaytype.php');">상점관리 > 쇼핑몰 환경 설정 > 프레임/정렬 설정 </a>
</li>
							</ul>
					</div>
				</td>
...
?>
				...
				<TABLE cellSpacing=0 cellPadding=0 width="100%" border=0 style="table-layout:fixed">
				<TR>
					<TD colspan="2"><textarea name=new_body style="WIDTH: 100%; HEIGHT: 600px" class="textarea"><?=htmlspecialchars($new_body)?></textarea></TD>
				</TR>
				</TABLE>
				</td>
			</tr>
