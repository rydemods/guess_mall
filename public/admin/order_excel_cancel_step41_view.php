...
Header("Content-Disposition: attachment; filename=order_excel_cancel_step41_view_".date("Ymd",$CurrentTime).".xls"); 
Header("Pragma: no-cache"); 
Header("Expires: 0");
Header("Cache-Control: must-revalidate, post-check=0,pre-check=0");
Header("Content-Description: PHP4 Generated Data");

$excel_info = "49,53,0,54,35,52,1,5,57,6,21,25,23,56,50,28,27,51,58,59,60,61,62,63,64";
...
if($oc_no) $ordercodes= str_replace(",''","",$oc_no);
    ...
    if($oistep == "2") {
        // 결제완료, 배송준비중에서의 환불접수
        $opstep_qry = "AND    p.op_step in ('41') and p.redelivery_type = 'N' ";
    } elseif($oistep == "34") {
        // 배송중, 배송완료에서의 환불접수
        $opstep_qry = "AND    p.op_step in ('42') and p.redelivery_type = 'Y' ";
    } elseif($oistep == "1234") {
        // 결제완료, 배송준비, 배송중, 배송완료에서의 환불접수
         $opstep_qry = "AND    p.op_step in ('41','42') and p.redelivery_type != 'G' ";
    }
	if($vendercnt>0) {
		...
		$qry_from .= ", (Select p.idx as op_idx, p.vender, p.ordercode, p.productcode, p.productname, p.opt1_name, p.opt2_name, p.quantity, p.price, p.option_price,
					            p.deli_com, p.deli_num, p.deli_date, p.deli_price, 
					            p.coupon_price, p.use_point, p.op_step, p.opt1_change, p.opt2_change, p.oc_no, p.date, p.idx, p.option_price_text_change, p.option_quantity 
		...
		$qry_from .= ", (Select oc_no from tblorderproduct p where oc_no > 0 and ".$opstep_qry." group by oc_no) b ";
	...
	$sql = "SELECT  b.vender, b.ordercode, b.productcode, b.productname, b.opt1_name, b.opt2_name, b.quantity, b.price, b.option_price,
					...
					a.id, a.sender_name, a.paymethod, a.oi_step1, a.oi_step2, toc.reg_dt
			FROM {$qry_from} {$qry} 
...
//		$receiver_tel2num="=\"{$receiver_tel2num}\""; 
		...
		$rg_date=substr($row->reg_dt,0,4)."/".substr($row->reg_dt,4,2)."/".substr($row->reg_dt,6,2)." (".substr($row->reg_dt,8,2).":".substr($row->reg_dt,10,2).")";
