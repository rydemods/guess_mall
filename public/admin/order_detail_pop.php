...
//$ordercode=rtrim(strtoupper($_POST["ordercodes"]),',');
$ordercode=rtrim(strtoupper(str_replace("@","",$_POST["ordercodes"])),',');
//$ordercode="2014040403085603426A";
$gbn=$_POST["gbn"];
$mode=$_POST["mode"];
$opt1=$_POST["opt1_name"];
$opt2=$_POST["opt2_name"];
$productcode=$_POST["productcode"];
$barcodeno=str_replace("@","",$_POST["barcodeno"]);

if($mode=="update"){
	$sql="select * from tblorderproduct where ordercode='".$ordercode."' and productcode='".$productcode."' and opt1_name='".$opt1."' and opt2_name='".$opt2."'";
	$result=pmysql_query($sql);
	$data=pmysql_fetch_object($result);
	
	if($data->quantity<=$data->order_check){
		$msg="수량이 초과되었습니다. ";
	}else{
		$sql="update tblorderproduct set order_check=order_check+1 where ordercode='".$ordercode."' and productcode='".$productcode."' and opt1_name='".$opt1."' and opt2_name='".$opt2."'";
		pmysql_query($sql);
	...
	if($data->quantity<=$data->order_check){
		...
		$sql="update tblorderproduct set order_check=quantity where ordercode='".$ordercode."' and productcode='".$productcode."' and opt1_name='".$opt1."' and opt2_name='".$opt2."'";
		pmysql_query($sql);
	
	}
	
}else if($mode=="barcode"){
	$sql = "SELECT a.productcode,a.productname,a.opt1_name,a.opt2_name,a.quantity,a.order_check, ";
	$sql.= "b.option_quantity,b.option_ea,b.option1,b.option2,b.optcode,a.package_idx,a.assemble_idx,a.assemble_info FROM tblorderproduct a, tblproduct b ";
	...
	while($data=pmysql_fetch_object($result)){
		$tmp_opt1_name = explode(":",$data->opt1_name);
		$tmp_option1=explode(",",$data->option1);
		$tmp_optcode=explode(",",$data->optcode);
		$opt1_idx = array_search(trim($tmp_opt1_name[1]),$tmp_option1);
	
		if(!$data->opt1_name){
			if($barcodeno==$data->optcode){
				if($data->option2!=''){
					$msg="옵션 바코드가 존재하지않습니다.";
				}else if($data->quantity<=$data->order_check){
					$msg="수량이 초과되었습니다. ";
				}else{
					$u_qry="update tblorderproduct set order_check=order_check+1 where ordercode='".$ordercode."' and opt1_name='".$data->opt1_name."' and opt2_name='".$data->opt2_name."' and productcode='".$data->productcode."'";	
			...
			if($barcodeno==$tmp_optcode[$opt1_idx-1]){
				if($data->option2!=''){
					$msg="옵션 바코드가 존재하지않습니다.";
				}else if($data->quantity<=$data->order_check){
					$msg="수량이 초과되었습니다. ";
...
function getKey(keyStroke){
	document.barcode_form.barcodeno.focus();
	if (event.keyCode == 13){
		if (document.barcode_form.barcodeno.value != "")
		{
			document.barcode_form.mode.value="barcode";
...
?>
<form name=barcode_form action="<?=$_SERVER['PHP_SELF']?>" method=post>
<input type="hidden" name="ordercodes" value="<?=$ordercode?>">
<input type="hidden" name="mode">
<table border=0 cellpadding=0 cellspacing=0 width=645>
			
	<tr>
		<td><span style="font-size:16px; font-weight:bold;">Order No : <?=$ordercode?></td>
		<td align=right><input type="text" name="barcodeno" class="input"></td>
	</tr>
</table>
</form>
<style type="text/css">
...
<table border=0 cellpadding=0 cellspacing=0 width=645 class="order_sheet_table">
	...
	<tr>
		...
		<th class="bdb">결제확인일 : </th>
		<td class="bdb" colspan=3 ><?=$_ord->bank_date?>&nbsp;</td>
	</tr>
...
<tr bgcolor=#f4f4f4>
	...
	<td align=center>개별발송</td>
	<td align=center>전체발송</td>
</tr>
<?php
$colspan=11;
...
while($row=pmysql_fetch_object($result)) {
	...
	echo "	<td align=center style=\"font-size:8pt\">".$row->order_check."&nbsp;</td>\n";
	...
	if($row->order_check>=$row->quantity){
		echo "	<td align=center >완료</td>\n";	
	}else{
		echo "	<td align=center ><input type=\"button\" value=\"발송\" onclick=\"checkreturn('".$row->productcode."','".$row->opt1_name."','".$row->opt2_name."');\"></td>\n";
	}
	if($row->order_check>=$row->quantity){
		echo "	<td align=center >완료</td>\n";	
	}else{
		echo "	<td align=center ><input type=\"button\" value=\"발송\" onclick=\"checkreturn_all('".$row->productcode."','".$row->opt1_name."','".$row->opt2_name."');\"></td>\n";
	}
	echo "</tr>\n";
...
if($_ord) {
	...
	echo "<tr bgcolor=#FFFFFF>\n";
	echo "	<td colspan=".($colspan-8)." style=\"font-size:8pt;padding:5,27\"><B>총 합계</B> </td>\n";
...
?>
<script>
	function checkreturn(productcode, opt1, opt2){
		document.check_order_form.productcode.value=productcode;
		document.check_order_form.opt1_name.value=opt1;
		document.check_order_form.opt2_name.value=opt2;
		document.check_order_form.mode.value="update";
		
		document.check_order_form.submit();
		
	}
	
	function checkreturn_all(productcode, opt1, opt2){
		
		if(confirm("해당상품을 전체발송 하시겠습니까?")){
			document.check_order_form.productcode.value=productcode;
			document.check_order_form.opt1_name.value=opt1;
			document.check_order_form.opt2_name.value=opt2;
			document.check_order_form.mode.value="update_all";
...
<form name=check_order_form action="<?=$_SERVER['PHP_SELF']?>" method=post>
<input type="hidden" name="ordercodes" value="<?=$ordercode?>">
