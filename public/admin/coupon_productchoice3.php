	...
	$(".CLS_couponAccept").click(function(){
		...
		$("input[name='productcode']").each(function(){
			if($(this).attr('checked')){
				tempHtml = $("#ID_productLayer", opener.document).html();
				...
				if (inProduct == 0)
				{
					$("#ID_productLayer", opener.document).html(tempHtml+"<div style='padding:5px 0px;'><a href=\"javascript:;\" onClick=\"javascript:$(this).parent().remove();\"><img src='images/icon_del1.gif' border='0' style='vertical-align:middle;' /></a>&nbsp;&nbsp;"+arrThisValue[1]+"<input type = 'hidden' name ='set_productname[]' value = '"+arrThisValue[1]+"'><input type = 'hidden' name ='set_productcode[]' value = '"+arrThisValue[0]+"'></div>");
				}
				$(this).removeAttr('checked');
			}
		})
		$("input[name='productcode']", opener.document).val("GOODS");
	})
...
//							$qry.=" AND ( quantity <= 0 or option_quantity like '%,0,%' )";
						...
						$sql.= "buyprice,quantity,reserve,reservetype,addcode,display,vender,tinyimage,assembleuse,assembleproduct, date, modifydate, prodcode, colorcode ";
...
//echo $sql . "<br/>";
                        ...
                        if ( ord($row->tinyimage) ) {
							...
							echo "<img src={$Dir}images/common/noimage.gif style=\"width:50px\" onMouseOver=\"ProductMouseOver('primage{$cnt}')\" onMouseOut=\"ProductMouseOut('primage{$cnt}');\">";
						}
						...
						if ( ord($row->tinyimage) )  {
							...
							echo "		<td align=\"center\" width=\"100%\" height=\"150\" style=\"border:#000000 solid 1px;\"><img src=\"{$Dir}images/common/noimage.gif\" border=\"0\"></td>\n";
						}
						echo "		</tr>\n";
						echo "		</table>\n";
						echo "		</div>\n";
						echo "	</td>\n";
						echo "<td height=\"50\"><p class=\"ta_l\"><!--img src=\"images/producttype".($row->assembleuse=="Y"?"y":"n").".gif\" border=\"0\" align=\"absmiddle\" hspace=\"2\"--> &nbsp; ".$row->productname.($row->selfcode?"-".$row->selfcode:"").($row->addcode?"-".$row->addcode:"")."<br> &nbsp;&nbsp;".$row->productcode." / ".$row->prodcode."-".$row->colorcode."</p></td>";
