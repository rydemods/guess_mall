...
if($_GET['autoUri']){
	foreach($_GET as $serialKey => $serialVal){
		if($serialKey == "Scrolltype"){
			$_REQUEST[$serialKey] = $serialVal;
		}else{
			$_POST[$serialKey] = $serialVal;
		}
	}
}
...
if ($mode=="sequence" && $change=="Y" && ord($selcodes) && ord($special)) {
	$sql = "UPDATE tblnesignmobile SET special_list = '{$selcodes}' WHERE special='{$special}' ";
	pmysql_query($sql,get_db_conn());
	$onload="<script>alert('모바일 상품 순서 조정이 완료되었습니다.');</script>\n";
} else if ($mode=="modify" && ord($selcodes) && ord($special)) {
	$sql = "UPDATE tblnesignmobile SET special_list = '{$selcodes}' WHERE special='{$special}' ";
	pmysql_query($sql,get_db_conn());
	$onload="<script>alert('해당 상품을 모바일 진열상품 카테고리에 추가하였습니다.');</script>\n";
} else if ($mode=="insert" && ord($selcodes) && ord($special)) {
	$sql = "INSERT INTO tblnesignmobile(special,special_list) VALUES (
	'{$special}',
	'{$selcodes}')";
	pmysql_query($sql,get_db_conn());
	$onload="<script>alert('해당 상품을 모바일 진열상품 카테고리에 추가하였습니다.');</script>\n";
	...
	if(ord($selcodes)==0) {
		$sql = "DELETE FROM tblnesignmobile WHERE special='{$special}' ";
	} else {
		$sql = "UPDATE tblnesignmobile SET special_list = '{$selcodes}' WHERE special='{$special}' ";
	}
	pmysql_query($sql,get_db_conn());
	$onload="<script>alert('해당 상품을 모바일상품에서 삭제하였습니다.');</script>\n";
}
...
function InsertSpecial(chk) { //20150604 함수 내용 수정. 원재
	//alert(chk); 체크박스를 통해 호출됬는지 여부 확인
...
<input type=hidden name=Scrolltype value="<?=$Scrolltype?>">
	<div class="main_view_setup_wrap" style="margin-top:15px;">
		<div class="group">
			<input type=radio id="idx_special1" name=special value="1" <?=$chk_special1?> onClick="ChangeSpecial(this.value);">
			<label style='cursor:hand; TEXT-DECORATION: none' onmouseover="style.textDecoration='underline'" onmouseout="style.textDecoration='none'" for=idx_special1 class="font_d12">오늘의 추천</label>&nbsp;&nbsp;&nbsp;

			<input type=radio id="idx_special2" name=special value="2" <?=$chk_special2?> onClick="ChangeSpecial(this.value);">
			<label style='cursor:hand; TEXT-DECORATION: none' onmouseover="style.textDecoration='underline'" onmouseout="style.textDecoration='none'" for=idx_special2 class="font_d12">신상품</label>&nbsp;&nbsp;&nbsp;
			...
			<div class="table_main_setup"  id="divscroll" style="height:400px;overflow-x:hidden;overflow-y:auto;">
				<table border=0 cellpadding=0 cellspacing=0 width=100%>
					...
					if(ord($special)) {
						$mode="insert";
						$sp_prcode="";
						$sql = "SELECT special_list,special_icon FROM tblnesignmobile WHERE special='{$special}' ";
						$result = pmysql_query($sql,get_db_conn());
						if($row = pmysql_fetch_object($result)){
							$cnt_prcode=$row->special_list;
							$cnt_icon = $row->special_icon;
							$sp_prcode=str_replace(',','\',\'',$cnt_prcode);
							$sp_icon = str_replace(',','\',\'',$cnt_icon);
							$mode="modify";
						}
						pmysql_free_result($result);
						...
						if(ord($sp_prcode)) {
							...
							for($i=0;$i<$cnt;$i++){
								if(ord($arraycode[$viewproduct[$i]])){
									...
									}else if(ord($arraytinyimage[$viewproduct[$i]]) && file_exists($Dir.$arraytinyimage[$viewproduct[$i]])){
										$strlist.= "objlist.imgidx=\"<TD style=\\\"position:relative;\\\"><img src=\\\"".$Dir.$arraytinyimage[$viewproduct[$i]]."\\\" height=\\\"40\\\" width=\\\"40\\\" border=\\\"1\\\" onMouseOver=\\\"ProductMouseOver('primage{$image_i}')\\\" onMouseOut=\\\"ProductMouseOut('primage{$image_i}');\\\"><div id=\\\"primage{$image_i}\\\" style=\\\"position:absolute; z-index:100; display:none;\\\"><table border=\\\"0\\\" cellspacing=\\\"0\\\" cellpadding=\\\"0\\\" width=\\\"170\\\"><tr bgcolor=\\\"#FFFFFF\\\"><td align=\\\"center\\\" width=\\\"100%\\\" height=\\\"150\\\" style=\\\"border:#000000 solid 1px;\\\"><img src=\\\"".$Dir.$arraytinyimage[$viewproduct[$i]]."\\\" border=\\\"0\\\"></td></tr></table></div></td>\";\n";
									} else {
									...
									if (ord($arraytinyimage[$viewproduct[$i]]) && file_exists($imagepath.$arraytinyimage[$viewproduct[$i]])){
										...
										echo "<img style=\"width:40px;height:40px\" src=\"".$Dir.$arraytinyimage[$viewproduct[$i]]."\" border=\"1\" onMouseOver=\"ProductMouseOver('primage{$image_i}')\" onMouseOut=\"ProductMouseOut('primage{$image_i}');\">";
									} else {
									...
									if (ord($arraytinyimage[$viewproduct[$i]]) && file_exists($imagepath.$arraytinyimage[$viewproduct[$i]])){
										...
										echo "		<td align=\"center\" width=\"100%\" height=\"150\" style=\"border:#000000 solid 1px;\"><img src=\"".$Dir.$arraytinyimage[$viewproduct[$i]]."\" border=\"0\"></td>\n";
									} else {
