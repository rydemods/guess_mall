...
$sql = "select * from tblpromo where event_type in ('5') ".$where." order by idx::int desc";
...
function event_pop(mode,idx){
	...
	document.location.href="market_promotion_reg_roulette.php?mode="+mode+"&pidx="+idx;
}

function event_ins(mode,idx,seq){
	if(mode=='del'){
		if(confirm('삭제하시겠습니까?')){
			document.location.href="market_promotion_reg_roulette.php?mode="+mode+"&pidx="+idx+"&seq="+seq;
		}
	}else{
		document.location.href="market_promotion_reg_roulette.php?mode="+mode+"&pidx="+idx;
	}
...
<div class="admin_linemap"><div class="line"><p>현재위치 : 마케팅지원 &gt; 이벤트/사은품 기능 설정 &gt;<span>룰렛이벤트 관리</span></p></div></div>
	...
	<tr>
		...
		<tr>
			...
			<tr>
				<td>
					<!-- 페이지 타이틀 -->
					<div class="title_depth3">룰렛이벤트 관리</div>
					<!-- 소제목 -->
					<div class="title_depth3_sub"><span>룰렛이벤트 검색</span></div>
					<div class="table_style01 pt_20">
						<table cellpadding="0" cellspacing="0" border="0" width="100%">
							<tr>
								...
								-->
								<textarea rows="2" cols="8" class="w200" name="keyword" id="keyword" style="resize:none;vertical-align:middle;"><?=$keyword?></textarea>
								</td>
								<th><span>사용</span></th>
								<td>
									<select name="search_hidden" style="width:80px;height:32px;vertical-align:middle;" onchange="javascript:changeSelectHidden(this);">
								...
								<td>
									<select name="display_type" id="display_type" style="width:80px;height:32px;vertical-align:middle;"  onchange="javascript:changeSelectHidden(this);">
										<option value="ALL" <?php if ($display_type == "ALL") echo "selected" ?> >전체</option>
...
?>
					...
					<TD style="text-align:center;"><?=$detailViewIcon?></TD>
					...
					<TD>
						<a target='_balnk' href='/front/promotion_roulette.php?pidx=<?=$row->idx?>' style="padding-right: 20px;">PC</a>
						<a target='_balnk' href='/m/promotion_roulette.php?pidx=<?=$row->idx?>'>Mobile</a>
					</TD>
<!--
                    <td>
                        <?php
					...
					<TD><img src="images/btn_edit.gif" border="0" onclick="popup_window('market_promotion_roulette_view.php?pidx=<?=$row->idx?>&mode=mod', 1100, 1000)"></TD>
					<TD><a href="javascript:event_pop('copy','<?=$row->idx?>');"><img src="images/btn_copy_b.gif" border="0"></a></TD>
