...
Header("Content-Disposition: attachment; filename=order_excel_cancel_step34_change_view_".$opstep."_".date("Ymd",$CurrentTime).".xls"); 
Header("Pragma: no-cache"); 
Header("Expires: 0");
Header("Cache-Control: must-revalidate, post-check=0,pre-check=0");
Header("Content-Description: PHP4 Generated Data");

$excel_info = "49,0,53,54,35,52,1,5,21,38,57,25,23,56,58,50,28,27,51,59,60";
...
$excelval=array(
	...
	array("옵션가"						,&$option_price),		#56
	array("교환옵션"						,&$change_option),		#57
	array("교환옵션가"						,&$change_option_price),		#58
	array("교환상품결제가"						,&$change_totprice),		#59
	array("메모"						,&$rdesc)		#60
);
...
//		$receiver_tel2num="=\"{$receiver_tel2num}\""; 
		...
		$price=$row->price;
		$reserve=$row->reserve;
		$option=$option2=$addcode=$change_option="";
		if (ord($row->addcode)) $addcode=$row->addcode;

	
		if( strlen( trim( $row->opt1_name ) ) > 0 ) {
			
			$op_opt1_name_arr	= explode("@#", $row->opt1_name);
			$op_opt2_name_arr	= explode(chr(30), $row->opt2_name);
			$s_cnt	= 0;
			for($s=0;$s < sizeof($op_opt1_name_arr);$s++) {
				if ($op_opt2_name_arr[$s]) {
					if ($s_cnt > 0) $option	.= " / ";
					$option	.= $op_opt1_name_arr[$s].' : '.$op_opt2_name_arr[$s];
					$s_cnt++;
				}
			}
		}
		...
		if( strlen( trim( $row->text_opt_subject ) ) > 0 ) {
			$op_text_opt_subject_arr	= explode("@#", $row->text_opt_subject);
			$op_text_opt_content_arr	= explode("@#", $row->text_opt_content);

			for($s=0;$s < sizeof($op_text_opt_subject_arr);$s++) {
				if ($op_text_opt_content_arr[$s]) {
					if ($option != '') $option	.= " / ";
					$option	.= $op_text_opt_subject_arr[$s].' : '.$op_text_opt_content_arr[$s];
				}
			}
		}

		if( strlen( $row->opt1_change ) > 0 ) {
			$op_optc1_name_arr	= explode("@#", $row->opt1_change);
			$op_optc2_name_arr	= explode(chr(30), $row->opt2_change);
			for($ss=0;$ss < sizeof($op_optc1_name_arr);$ss++) {
				if ($ss > 0) $change_option	.= " / ";
				$change_option	.= $op_optc1_name_arr[$ss].' : '.$op_optc2_name_arr[$ss];
			}
		}	

		if( strlen( trim( $row->text_opt_subject_change ) ) > 0 ) {
			$op_text_opt_subject_arr	= explode("@#", $row->text_opt_subject_change);
			$op_text_opt_content_arr	= explode("@#", $row->text_opt_content_change);

			for($s=0;$s < sizeof($op_text_opt_subject_arr);$s++) {
				if ($op_text_opt_content_arr[$s]) {
					if ($change_option != '') $change_option	.= " / ";
					$change_option	.= $op_text_opt_subject_arr[$s].' : '.$op_text_opt_content_arr[$s];
				}
			}
		}
		...
		if ($row->option_price_text_change) {
			...
			$change_option_price	= $opt_price_chn * $row->option_quantity;
			$change_totprice=(($row->price + $opt_price_chn)*$row->option_quantity)-$row->coupon_price-$row->use_point+$row->deli_price;
		}
