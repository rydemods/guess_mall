...
if($type=="update") {
	...
	if($row->cnt==0) {
		...
		body		= '{$view_body}' 
		WHERE type='noticeview' ";
		pmysql_query($sql,get_db_conn());
	}
	pmysql_free_result($result);

	$onload="<script>window.onload=function(){ alert(\"공지사항 팝업창 디자인 수정이 완료되었습니다.\"); }</script>";
} elseif($type=="delete") {
	$sql = "DELETE FROM tbldesignnewpage WHERE type='noticelist' ";
	pmysql_query($sql,get_db_conn());
	$sql = "DELETE FROM tbldesignnewpage WHERE type='noticeview' ";
	pmysql_query($sql,get_db_conn());
	$onload="<script>window.onload=function(){ alert(\"공지사항 팝업창 디자인 삭제가 완료되었습니다.\"); }</script>";
} elseif($type=="clear") {
	$list_body="";
	$list_xsize=430;
	$list_ysize=470;
...
<div class="admin_linemap"><div class="line"><p>현재위치 : 디자인관리 &gt; 개별디자인-페이지 본문 &gt;<span>공지사항 팝업창 꾸미기</span></p></div></div>
	...
	<tr>
		...
		<tr>
			...
			<tr>
				<td>
                    <!-- 페이지 타이틀 -->
					<div class="title_depth3">공지사항 팝업창 꾸미기</div>
					<div class="title_depth3_sub"><span>공지사항 팝업창 디자인을 자유롭게 관리하실 수 있습니다.</span></div>
                </td>
            </tr>
            <tr>
            	<td>
					<!-- 소제목 -->
					<div class="title_depth3_sub">공지사항 팝업 목록 디자인 관리</div>
				</td>
			</tr>
			<tr>
				<td>
			...
			<tr>
				<td style="padding-bottom:3pt;">
					<!-- 소제목 -->
					<div class="title_depth3_sub">공지사항 팝업 상세 페이지 디자인 관리</div>
                </td>
            </tr>
            <tr>
            	<td>
			...
			<tr>
				...
				<!-- 메뉴얼 -->
					<div class="sub_manual_wrap">
						...
						<dl>
							<dt><span class="point_c1">공지사항 팝업창 매크로명령어</span><span>(해당 매크로명령어는 다른 페이지 디자인 작업시 사용이 불가능함)</span></dt>
							<dd>
