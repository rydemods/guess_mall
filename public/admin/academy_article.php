...
if($_REQUEST["mode"]=="comment_result") {
	...
	header("Location:{$_SERVER['PHP_SELF']}?exec=view&board=$board&num=$num&block=$block&gotopage=$gotopage&category=$category&search=$search&s_check=$s_check");
	exit;
	...
	header("Location:{$_SERVER['PHP_SELF']}?exec=view&board=$board&num=$num&s_check=$s_check&search=$search&block=$block&gotopage=$gotopage&category=$category");
	exit;
}
...
<div class="admin_linemap"><div class="line"><p>현재위치 : 커뮤니티 &gt; DIY아카데미 &gt;<span><?=$category?> 관리</span></p></div></div>
	...
	<tr>
		...
		<tr>
			...
			<tr>
				<td>
					<!-- 페이지 타이틀 -->
					<div class="title_depth3"><?=$category?> 관리</div>
					<!-- 소제목 -->
					<div class="title_depth3_sub"><span>등록된 게시판의 모든 게시물을 관리할 수 있습니다.</span></div>
				</td>
			</tr>
			<tr>
				<td><?php include("academy_article.{$exec}.inc.php")?></td>
			</tr>
