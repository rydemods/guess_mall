						...
						<td width="100%" height="100%" valign="top" bgcolor="#FFFFFF" style="padding-left:5px;padding-right:5px;">
							...
							<?php
									...
									if(strlen($code)==12) {
										...
										$sql.= "ORDER BY start_no asc, pridx desc  ";
										...
										if($cnt>0)
										{
											...
											while($row=pmysql_fetch_object($result)) {
												...
												$strlist.= "objlist.nameidx=\"<TD style=\\\"word-break:break-all; text-align:left; padding-left:10px\\\">".addslashes($row->productname.($row->selfcode?"-".$row->selfcode:"").($row->addcode?"-".$row->addcode:""))."&nbsp;</td>\";\n";
												...
												if($vendercnt>0) {
													echo "	<TD><B>".(ord($venderlist[$row->vender]->vender)?"{$venderlist[$row->vender]->brandname}":"-")."</B></td>\n";
												}
												echo "		<TD>";
												if (ord($row->tinyimage) && file_exists($imagepath.$row->tinyimage)){
													echo "<img src=\"".$imagepath.$row->tinyimage."\" style=\"width:50px\" border=\"1\">";
												}else if(ord($row->tinyimage) && file_exists($Dir.$row->tinyimage)){
													echo "<img src=\"".$Dir.$row->tinyimage."\" style=\"width:50px\" border=\"1\">";
												}else if($tinyimage){
													echo "<img src=\"".$tinyimage."\" style=\"width:50px\" border=\"1\">";
												} else {
													echo "<img src=images/space01.gif>";
												}
