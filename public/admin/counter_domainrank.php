...
$len=30;
if($type=="d") {
	$sql ="SELECT * FROM tblcounterdomain WHERE date='{$searchdate}' AND domain !='https:' AND domain !='shinwonmall.com' ORDER BY cnt DESC LIMIT ".$len;
} else if($type=="w") {
	$prevdate=date("Ymd",strtotime('-7 day'));
	$sql ="SELECT SUM(cnt) as cnt,domain FROM tblcounterdomain 
	WHERE (date<='{$searchdate}' AND date>='{$prevdate}') AND domain !='https:' AND domain !='shinwonmall.com' 
	GROUP BY domain ORDER BY cnt DESC LIMIT ".$len;
} else if($type=="m") {
	$date=substr($searchdate,0,6);  
	if ($date==date("Ym")) {
		$sql ="SELECT SUM(cnt) as cnt,domain FROM tblcounterdomain 
		WHERE date LIKE '{$date}%' AND domain !='https:' AND domain !='shinwonmall.com' GROUP BY domain ORDER BY cnt DESC LIMIT ".$len;
	} else {
		$sql ="SELECT cnt,domain FROM tblcounterdomainmonth 
		WHERE date='{$date}' AND domain !='https:' AND domain !='shinwonmall.com' ORDER BY cnt DESC LIMIT ".$len;
	}
}
...
<?php if($print!="Y"){?>
<div class="admin_linemap"><div class="line"><p>현재위치 : 통계분석 &gt; 외부 접근 경로 분석 &gt;<span>도메인별 접근순위</span></p></div></div>
...
<?php }?>
			<tr>
				<td>
					<!-- 페이지 타이틀 -->
					<div class="title_depth3">도메인별 접근순위</div>
				</td>
			</tr>
...
<?php
					...
					for($i=0;$i<$len;$i++){
						...
						echo "<tr>\n";
						echo "	<TD>".($i+1)."</td>\n";
						echo "	<TD><A HREF=\"http://{$domain[$i]}\" target=\"_blank\">http://{$domain[$i]}</A></td>\n";
...
<?php if($print!="Y"){?>
			<tr>
				...
				<div class="sub_manual_wrap">
					...
					<dl>
					  <dt><span>당사 쇼핑몰을 외부에서 찾아 오는 경로(도메인)를 순위별로 알 수 있습니다.</span></dt>
                    </dl>
                    <dl>
                    	<dt><span>외부에서 당사 쇼핑몰을 찾아오는 경로를 분석하여, 보다 효과적으로 방문자 숫자를 증가시킬 수 있습니다. </span></dt>
                    </dl>
                    <dl>
                    	<dt><span>외부에서 당사 쇼핑몰을 찾아오는 경로(도메인)를 파악하여, 그 사이트와 전략적인 제휴 등 적극적인 마케팅을 전개할 수 있습니다. </span></dt>
                    </dl>
                </div>				
                </td>
			</tr>
