...
//디자인 스킨 = design_main_backup.php    - 메뉴에 추가 필요함
foreach($fp as $v){
	if(trim($v))$v = trim($v);
					...
					<tr>
						<td>
							...
							?>
								...
								<a href="<?=$menu['value'][$i][$j]?>" name="navi" <?if(isset($menu['target'][$i][$j])) {?>target="<?=$menu['target'][$i][$j]?>"<?}?> <?if(preg_match('/'.str_replace('/','\/',str_replace('?','',$menu['value'][$i][$j])).'/',str_replace('?','',$_SERVER['REQUEST_URI']))){
								?>class="this"<?}?>>
								<?}?>
