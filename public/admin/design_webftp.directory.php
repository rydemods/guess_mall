...
for($i=$count-1;$i>=0;$i--) {
	...
	} elseif($filefolder[$i]==2) { //3,4,5,6
		if(strcmp(substr($tempdirectory[$i+1],0,$len),substr($tempdirectory[$i],0,$len))==0) {
			if($i==$count-1) $filenum[$i]=4;
			elseif((strcmp(substr($tempdirectory[$i-1],0,$len2),substr($tempdirectory[$i],0,$len2))==0 && $filefolder[$i]==$filefolder[$i-1]) || (strcmp(substr($tempdirectory[$count-1],0,$len2),substr($tempdirectory[$i],0,$len2))==0)) { 
				$filenum[$i]=4;
			} else $filenum[$i]=4;
		} elseif($filefolder[$i]!=$filefolder[$i-1]) $filenum[$i]=4;
	}
...
<body leftmargin=0 topmargin=0 marginwidth=0 marginheight=0 bgcolor="#FFFFFF" oncontextmenu="return false" ondragstart="return false" onselectstart="return false" oncontextmenu="return false">
<table border=0 cellpadding=0 cellspacing=0 width=100%>
<form name=form1 method=post action="design_webftp<?php if($popup=="ok")echo".popup";?>.php" target="_parent">
<tr>
	<td class="font_size1" style="padding:3">
	<img src="images/directory_root.gif"> <a href="javascript:document.form1.submit()"><?php if($originalpath==$path)echo"<font color=#FF4C00>"; echo "/".RootPath.substr(substr($originalpath,$rootlength),1)."</font>";?></a>
	</td>
</tr>
<?php
for($i=0;$i<$count;$i++) {
