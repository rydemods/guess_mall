...
if($mode=="modify"){
	...
	if (strlen($prcode)==18) {
		...
		if ($row=pmysql_fetch_object($result)) {
			...
			'{$row->sewon_option_code2}',
			'{$row->sabangnet_prop_val}',
			'{$row->staff_product}'	) RETURNING pridx";
...
function ProductInfo(prcode,popuptype,category_data) {
	...
	if (popup=="YES") {
		...
		window.open("about:blank","register","width=1000,height=700,scrollbars=yes,status=no");
			...
			document.form_register.action="product_register.set2.php";
		...
		<tr>
			...
			<tr>
				...
				?>
						...
						<tr>
							<th><span>사내 구매 전용 상품 보기</span></th>
							<td><input type="radio" name="staff" value="all" <?=$checked["staff"]['all']?>/>전체
							<input type="radio" name="staff" value="1" <?=$checked["staff"]['1']?>/> STAFF 전용&nbsp;&nbsp;
							<input type="radio" name="staff" value="2" <?=$checked["staff"]['2']?>/> STAFF 제외</td>
						</tr>
					</table>
...
//							$qry.=" AND option_quantity like '%,0,%' ";
						...
						if($staff==1)	$qry.=" AND a.staff_product= '1' ";
						elseif($staff==2)	$qry.=" AND a.staff_product != '1' ";
						...
						$sql = "SELECT option_price,productcode,productname,production,sellprice,consumerprice, vip_product, option_quantity, option1, staff_product, ";
						$sql.= "buyprice,quantity,reserve,reservetype,addcode,display,vender,tinyimage,assembleuse,assembleproduct, date, modifydate,sabangnet_flag, sewon_option_no, sabangnet_prop_val ";
