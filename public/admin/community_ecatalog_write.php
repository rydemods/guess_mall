...
// 이미지 경로
$imagepath = $Dir.DataDir."shopimages/ecatalog/";
// 이미지 파일
...
if($mode=="delete") {
    ...
    $qry = "DELETE FROM tblecatalog WHERE no ='".$no."'";
...
if( $mode == 'modfiy_select' ){
	$no = $_POST['no'];
	if(!$no) $no = $_GET['no'];
	$mSelectSql = "SELECT * FROM tblecatalog WHERE no =".$no."";
...
function goBackList(){
	location.href="community_ecatalog_list.php";
}


</script>

<div class="admin_linemap"><div class="line"><p>현재위치 : 커뮤니티  &gt; ECATALOG 관리 &gt; <span>ECATALOG <?=$qType_text?></span></p></div></div>
...
<tr>
	...
	<tr>
		...
		<tr>
			...
			<input type=hidden name=tb value="ecatalog">
			...
			<tr>
				<td>
					<!-- 페이지 타이틀 -->
					<div class="title_depth3">ECATALOG <?=$qType_text?></div>

					<!-- 소제목 -->
					<div class="title_depth3_sub"><span>ECATALOG <?=$qType_text?>할 수 있습니다.</span></div>
				</td>
            </tr>
			<tr>
				<td>
					<!-- 소제목 -->
					<div class="title_depth3_sub">ECATALOG 기본정보</div>
				</td>
			</tr>
...
-->				
				<tr>
					<th><span>LOOKBOKK 이미지(PC)<br></span><div style="text-align: center;">(이미지 사이즈: 1160 x 738)</div></th>
					<TD><textarea wrap=off  id="ir1" style="WIDTH: 100%; HEIGHT: 300px" name="img"><?=$mSelect['img']?></textarea></TD>
				</tr>
				<tr>
					<th><span>LOOKBOKK 이미지(mobile)<br></span></th>
