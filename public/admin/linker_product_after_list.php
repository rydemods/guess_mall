...
function registeraddSoldout(){
	var checkNumber = 0;
	$("input[name='mode']").val("linkerProductSoldout");
	$("input[name='linker_val[]']").each(function(){
		if($(this).is(":checked")){
			checkNumber++;
		}
	})
...
function registeradd(){
	var checkNumber = 0;
	$("input[name='mode']").val("linkerProductModify");
	$("input[name='linker_val[]']").each(function(){
		if($(this).is(":checked")){
			checkNumber++;
		}
...
function registeraddAll(){
	...
	if(checkNumber > 0){
		...
		/*
		document.form1.action="linker_indb.php";
		document.form1.mode="linkerProductRegister";
		document.form1.target="";
		document.form1.submit();
		*/
	}else{
...
function popupStock(pdcode){
	window.open("../admin/linker_send_stock_popup.php?chk=1&productcode="+pdcode, "registerStock","width=400,height=300,scrollbars=yes,status=no");
	//linker_send_stock_popup.php
}
</script>
<!-- 라인맵 -->
<div class="admin_linemap"><div class="line"><p>현재위치 : 상품관리 &gt;샵링커 상품관리 &gt; <span>샵링커 연동 상품리스트</span></p></div></div>
	...
	<tr>
		...
		<tr>
			...
			<form name=form1 action="<?=$_SERVER['PHP_SELF']?>" method=post>
				<table cellpadding="0" cellspacing="0" width="100%" style="table-layout:fixed">
				<input type=hidden name=mode value = 'linkerProductModify'>
				<input type=hidden name=prcode>
				<input type=hidden name=block value="<?=$block?>">
				<input type=hidden name=gotopage value="<?=$gotopage?>">			
				<tr>
					<td>
					<div class="title_depth3">샵링커 연동 상품리스트</div>
			...
			<?php
							...
							$qry.=" AND shoplinker_flag IN ('Y', 'R')";
							...
							$sql = "SELECT option_price,productcode,productname,production,sellprice,consumerprice, shoplinker_flag, ";
							...
							$arrayLinkerStatus = array('Y'=>"<font color = 'blue'>수정성공</font>", 'R'=>"<font color = 'red'>수정실패</font>");
							...
							echo "<td>".$arrayLinkerStatus[$row->shoplinker_flag]."</td>";
							echo "</tr>";
							$cnt++;	
							}
							if ($cnt==0) {
			...
			<?php					
							//페이징
								...
								echo "</div>";
								echo "</div>";
								echo "<div class=\"btn_right\"><a href=\"javascript:registeraddAll();\">[상품수정]</a>&nbsp;&nbsp;&nbsp;<a href=\"javascript:registeradd();\">[제휴몰 수정<b> (확인불가) </b>]</a>&nbsp;&nbsp;&nbsp;<a href=\"javascript:registeraddSoldout();\">[제휴몰 품절처리<b> (확인불가) </b>]</a></div>";
			...
			?>
					...
					<div class="sub_manual_wrap">
						...
						<dl>
							...
							<dd>
								 [상품수정] 항목 : 처음 등록 할 때 등록 되었던 모든 내용이 수정 가능함. ( 샵링커 내용만 수정 )<br>
								 [제휴몰 수정] 항목 : 연동 된 상품의 데이터를 수정합니다. ( 쇼핑몰에 연동된 상품이 아니면 적용되지 않습니다. )<br>
								 [제휴몰 품절처리] 항목 : 연동 된 상품의 데이터를 수정합니다. 재고와 관계없이 품절 처리 합니다( 쇼핑몰에 연동된 상품이 아니면 적용되지 않습니다. )
							</dd>
						</dl>
					</div>
					</td>
