...
// 선택된 상품 삭제
if($_GET['type'] == "delete") {
    $sql = "Delete From tblbatchapplylog Where no = ".$_GET['no']."";
...
if($_GET['type'] == "all_delete") {
    ...
    $sql = "Delete From tblbatchapplylog Where no in (".$selectChk.") ";
    pmysql_query($sql,get_db_conn());
...
$sql = "SELECT COUNT(*) as t_count FROM (SELECT a.no FROM {$qry_from} {$qry} ) a ";
//exdebug($sql);
//echo "sql = ".$sql."<br>";
			...
			<tr>
				<td>
					...
					<tr>
                        ...
                        <TR>
							<th><span>설정구분</span></th>
							<TD class="td_con1">
                                <input type="radio" name="flag" value="A" <?=$selected[flag]["A"]?>>전체</input>
                                <input type="radio" name="flag" value="1" <?=$selected[flag]["1"]?>>% 설정</input>
                                <input type="radio" name="flag" value="2" <?=$selected[flag]["2"]?>>금액 설정</input>
                            </TD>
						</TR>
...
<?php
		$sql = "SELECT  a.*, b.productname, v.brandname 
                FROM {$qry_from} {$qry} 
		        ORDER BY a.pidx desc, a.no desc 
                ";
...
<?php
		...
		while($row=pmysql_fetch_object($result)) {
            ...
            $flag_price = ($row->flag==1)?"% 적용":"금액적용";
?>
			    <tr bgcolor=<?=$thiscolor?> onmouseover="this.style.background='#FEFBD1'" onmouseout="this.style.background='<?=$thiscolor?>'">
			        <td align="center">
                        <input type=checkbox name=chkordercode value="<?=$row->no?>"><br>
                    </td>
                    <td align="center"><?=$number?></td>
                    <td align="center"><?=$row->brandname?></td>
                    <td align="center"><?=$row->productcode?></td>
			        <td style='text-align:left'><?=$productname?></td>
                    <td align="center"><?=$row->discount_rate?></td>
                    <td align=right style="font-size:8pt;padding:3"><?=number_format($row->old_sellprice)?>&nbsp;&nbsp;&nbsp;</td>
                    <td align=right style="font-size:8pt;padding:3"><?=number_format($row->new_sellprice)?>&nbsp;&nbsp;&nbsp;</td>
                    <td align="center"><?=$date?></td>
                    <td align="center"><?=$row->start_date." - ".$row->end_date?></td>
                    <td align="center"><?=$flag_price?></td>
                    <td align="center">
<?
            if($row->start_date > date("Ymd")) {
?>
                        <a href="javascript:ProductDel('<?=$row->no?>')"><img src="img/btn/btn_cate_del01.gif" alt="삭제" /></a>
<?
            } else {
?>
                        - 
