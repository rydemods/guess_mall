<?php
/********************************************************************* 
// 파 일 명		: design_collection3.php
// 설     명		: 데코앤이 COLLECTION 페이지 STAGE3 디자인
// 상세설명	: 데코앤이 COLLECTION 페이지의 STAGE3 배너 및 링크 이미지 관리
...
#---------------------------------------------------------------
	...
	if($type=="insert" || $type=="update") {				// DB를 수정한다.
		...
		$cno	= $_POST["cno"];
		$stage3_bg	= $_POST["stage3_bg"];
		$stage3_text	= pg_escape_string($_POST["stage3_text"]);
		$stage3_subtext	= pg_escape_string($_POST["stage3_subtext"]);
		...
		if($type=="insert") {
			...
			$qry_where="";
			$qry_where[] = "stage3_text='".$stage3_text."'";
			$qry_where[] = "stage3_subtext='".$stage3_subtext."'";
			...
			for($u=0;$u<2;$u++) {
				if( strlen( $up_imagefile["up_imagefile"][$u]["v_file"] ) > 0 ){
					...
					if ($u == 0) $qry_where[] = "stage3_bg='".$up_imagefile["up_imagefile"][0]["v_file"]."'";
					if ($u == 1) $qry_where[] = "stage3_bg_m='".$up_imagefile["up_imagefile"][1]["v_file"]."'";
				}
			}	
...
function goList() {
	location.href='design_collection3.php';
}
...
<div class="admin_linemap"><div class="line"><p>현재위치 : 배너관리 &gt; 메인 배너관리 &gt;<span>STAGE3 영역관리</span></p></div></div>
	...
	<tr>
		...
		<tr>
			...
			<tr>
				<td>
					<!-- 페이지 타이틀 -->
					<div class="title_depth3">STAGE3 영역관리</div>

					<!-- 소제목 -->
					<div class="title_depth3_sub"><span>STAGE3 영역 정보를 변경 할 수 있습니다.</span></div>
				</td>
            </tr>
			<tr>
				<td>
					<!-- 소제목 -->
					<div class="title_depth3_sub">STAGE3 영역정보</div>
				</td>
			</tr>
			...
			<tr>
				...
				<tr>
					...
					<TD><INPUT maxLength=80 size=80 id='stage3_text' name='stage3_text' value="<?=$_cdata->stage3_text?>"></TD>
				</tr>
				<tr>
					<th><span>하단 텍스트</span></th>
					<TD><INPUT maxLength=80 size=80 id='stage3_subtext' name='stage3_subtext' value="<?=$_cdata->stage3_subtext?>" ></TD>
				</tr>
				<tr>
					...
					<td class="td_con1" colspan="3" style="position:relative">
						...
						<input type=hidden name="v_up_imagefile[0]" value="<?=$_cdata->stage3_bg?>" >
<?	if( is_file($imagepath.$_cdata->stage3_bg) ){ ?>
						<div style='margin-top:5px' >
									<img src='<?=$imagepath.$_cdata->stage3_bg?>' style='max-height: 200px;' />
						</div>
<?	} ?>
					</td>
				</tr>
				<tr>
					...
					<td class="td_con1" colspan="3" style="position:relative">
						...
						<input type=hidden name="v_up_imagefile[1]" value="<?=$_cdata->stage3_bg_m?>" >
<?	if( is_file($imagepath.$_cdata->stage3_bg_m) ){ ?>
						<div style='margin-top:5px' >
									<img src='<?=$imagepath.$_cdata->stage3_bg_m?>' style='max-height: 200px;' />
						</div>
<?	} ?>
					</td>
				</tr>
			...
			<tr>
				<td>
					<!-- 소제목 -->
					<div class="title_depth3_sub">STAGE3 사진정보</div>
				</td>
			</tr>
			<tr><td height=3></td></tr>
			<tr>
			...
			<tr>
				<td>
                    ...
                    <tr>
                        <td>					
                            <!-- 소제목 -->
        					<div class="title_depth3_sub">STAGE3 사진목록</div>
                        </td>
                        <td align="right">
			...
			<tr>
				...
				<!-- 매뉴얼 -->
					<div class="sub_manual_wrap">
						...
						<dl>
							<dt><span>STAGE3 영역관리</span></dt>
							<dd>- STAGE3 영역 정보를 변경 할 수 있습니다.
							</dd>	
						</dl>
