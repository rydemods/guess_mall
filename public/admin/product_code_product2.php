...
if($_POST['chk_detail'] && $_POST['serial_value']){
	...
	foreach($arrSerialValue as $serialVal){
		...
		$_REQUEST[$serialValue[0]] = $serialValue[1];
	}
}
...
<tr>
	...
	<tr>
		...
		<tr>
			...
			<tr>
				<td>
				<div class="title_depth3">상품관리(제휴몰)</div>
				...
				?>
						...
						<tr>
							...
							<td><input type="radio" name="s_check" value="" <?=$checked["s_check"]['all']?>/>전체 <input type="radio" name="s_check" value="1" <?=$checked["s_check"]['1']?>/>판매중 <input type="radio" name="s_check" value="2" <?=$checked["s_check"]['2']?>/>품절</td>
						</tr>
						<tr>
							<th><span>진열 유무</span></th>
							<td><input type="radio" name="display_yn" value="" <?=$checked["display_yn"]['all']?>/>전체 
							<input type="radio" name="display_yn" value="1" <?=$checked["display_yn"]['1']?>/> 진열&nbsp;&nbsp; 
							<input type="radio" name="display_yn" value="2" <?=$checked["display_yn"]['2']?>/> 미진열</td>
						</tr>
						<!--<tr>
							<th><span>VIP 상품 보기</span></th>
							<td><input type="radio" name="vip" value="" <?=$checked["vip"]['all']?>/>전체
...
//							$qry.=" AND option_quantity like '%,0,%' ";
						...
						if($display_yn==1)	$qry.="AND a.display='Y' ";
						elseif($display_yn==2)	$qry.="AND a.display='N'";
						...
						$sql = "SELECT option_price,productcode,productname,production,sellprice,consumerprice, vip_product, option1, ";
						$sql.= "buyprice,quantity,reserve,reservetype,addcode,display,vender,tinyimage,assembleuse,assembleproduct, date, modifydate,sabangnet_flag, sewon_option_no ";
						...
						<td >
						<?if($row->sabangnet_flag == 'Y'){
								echo "<font color=\"#0000FF\">연동</font>";
						}else{
								echo "-";
						}
						?>
						...
						<span class="font_orange"><?=number_format($row->consumerprice)?>
						</span></td>
						<!--시중가-->
						<td style="text-align:right; padding-right:10px">
						<img src="images/won_icon.gif" border="0" style="margin-right:2px;">
						<span class="font_orange"><?=number_format(exchageRate($row->consumerprice))?></span></td>
						<!--판매가-->
						<td style="text-align:right; padding-right:10px"><img src="images/won_icon.gif" border="0" style="margin-right:2px;">
						<input type="text" name="sellprice" style="width:50" value="<?=number_format(exchageRate($row->sellprice))?>"></td>
						<!--적립율-->
...
<input type=hidden name=action_page value="product_code_product2.php">
</form>
