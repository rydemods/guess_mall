<?php
/********************************************************************* 
// 파 일 명		: design_studio_list.php
// 설     명	: STUDIO 관련 리스트 관리
// 상세설명	    : STUDIO 관련 리스트 관리
// 작 성 자		: 2016.02.02 - 최문성
// 수 정 자		: 
// 
// 
*********************************************************************/ 
...
#---------------------------------------------------------------
    ...
    $arrCheckList       = explode(",", $s_checklist);
    $search_hidden      = $_POST['search_hidden'];

    $mod_sort           = $_POST['mod_sort'];
    if ( $mod_sort == "Y" ) {
        // 노출 순서 변경
        $ids            = $_POST['ids'];
        $sort           = $_POST['sort'];
        ...
        if ( count($sort) > 0 ) {
            for ( $i = 0; $i < count($sort); $i++ ) {
                $sql = "UPDATE tbl{$tb} SET sort = " . $sort[$i] . " WHERE no = " . $ids[$i] . " ";
                pmysql_query($sql);
            }
        }
    }
...
function lbModify(lbno) {
	location.href="design_studio_write.php?tb=<?=$tb?>&mode=modfiy_select&lbno="+lbno;
}
...
function lbAdd() {
	location.href="design_studio_write.php?tb=<?=$tb?>";
}
		...
		<tr>
			...
			<tr>
				...
				<tr>
					...
					<!-- <input type=text name=search value="<?=$search?>" class="w200"> -->
		...
		if($t_count>0) {
			...
			while($row=pmysql_fetch_object($result)) {
				...
				if( is_file($imagepath.$row->img) ){ 
					$lb_img	= "<img src='".$imagepath.$row->img."' style='max-width: 120px; max-height: 100px;' />";
				} else {
					$lb_img	= "-";
				}
                ...
                echo "	<td align=center><input type='checkbox' name='idx[]' value='" . $row->no . "' /><input type='hidden' name='ids[]' value='" . $row->no . "'></td>\n";
				echo "	<td align=center>{$number}</td>\n";
				echo "	<td align=center>{$lb_img}</td>\n";
				echo "	<td style='text-align:left'>{$row->title}</td>\n";
				echo "	<td align=center>".$display[$row->hidden]."</td>\n";
                if ( $tb == "press" ) {
                    echo "	<td align=center><input type='text' name='sort[]' value='".$row->sort."' size='4' maxlength='4'></td>\n";
                }
...
?>
			...
			<tr>
				<td align=right>
                    <?php if ( $tb == "press" ) { ?>
					<a href="javascript:changeSort();"><img src="images/admin_button17.gif" border="0" alt="순서변경"></a>
                    <?php } ?>
			...
			<input type=hidden name='mod_sort' value=''>
			</form>
			</table>
			</td>
		</tr>
...
<script type="text/javascript">
    ...
    function changeSort() {
        if ( confirm("노출 순서를 변경하시겠습니까?") ) {
            document.pageForm.mod_sort.value = "Y"
            document.pageForm.submit();
        }
    }
</script>
