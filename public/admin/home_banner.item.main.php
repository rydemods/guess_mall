...
$code=$_POST["code"]?$_POST["code"]:"home_new_item";
...
$title_name["home_new_item"] = "NEW ITEM";
$title_name["home_hot_item"] = "HOT ITEM";
$arrSpecial = array("home_new_item"=>"4", "home_hot_item"=>"5");
$special = $arrSpecial[$code];
...
if ($mode=="sequence" && $change=="Y" && ord($selcodes) && ord($special)) {
	...
	$onload="<script>alert('홈페이지 ".$title_name[$code]." 순서 조정이 완료되었습니다.');</script>\n";
} else if ($mode=="modify" && ord($selcodes) && ord($special)) {
	$sql = "UPDATE tblspecialmain SET special_list = '{$selcodes}' WHERE special='{$special}' ";
	pmysql_query($sql,get_db_conn());
	$onload="<script>alert('해당 상품을 홈페이지 ".$title_name[$code]."에 추가하였습니다.');</script>\n";
} else if ($mode=="insert" && ord($selcodes) && ord($special)) {
	$sql = "INSERT INTO tblspecialmain(special,special_list) VALUES (
	'{$special}', 
	'{$selcodes}')";
	pmysql_query($sql,get_db_conn());
	$onload="<script>alert('해당 상품을 홈페이지 ".$title_name[$code]."에 추가하였습니다.');</script>\n";
	...
	pmysql_query($sql,get_db_conn());
	$onload="<script>alert('해당 상품을 홈페이지 ".$title_name[$code]."에서 삭제하였습니다.');</script>\n";
}
...
function InsertSpecial() {
	...
	if (document.form1.prcode.value.length==0) {
		alert('홈페이지 '+title_name+'에 추가할 상품을 선택하세요.');
		document.form1.prcode.focus();
		return;
	}
	num = all_list.length-1;
	if(num+1>=50){
		alert('홈페이지 '+title_name+'은 최대 50개까지 등록가능합니다.');
		return;
	}
	if (confirm('해당 상품을 홈페이지 '+title_name+'으로 포함하시겠습니까?')){
		temp = "";
		for (i=0;i<=num;i++) {
...
function Delete(delcode) {
	ProductInfoStop = "1";
	var title_name = $("input[name='title_name']").val();
	if(!confirm('해당 상품을 홈페이지 '+title_name+'에서 삭제하시겠습니까?')) return;
	val="";
	for(i=0;i<all_list.length;i++)
	{
...
<input type='hidden' name='title_name' value = '<?=$title_name[$code]?>'>
...
<input type='hidden' name='Scrolltype' value="<?=$Scrolltype?>">
	<div class="main_view_setup_wrap" style="margin-top:15px;">
		<div class="group">&nbsp;&nbsp;<?=$title_name[$code]?></div>
		<div class="list">
						...
						if(ord($sp_prcode)) {
							...
							for($i=0;$i<$cnt;$i++){
								if(ord($arraycode[$viewproduct[$i]])){
									...
									echo "		</div>\n";
									echo "		</td>\n";
									echo "		<TD style=\"word-break:break-all;text-align:left;\" colspan = '4'><img src=\"images/producttype".($arrayassembleuse[$viewproduct[$i]]=="Y"?"y":"n").".gif\" border=\"0\" align=\"absmiddle\" hspace=\"2\">".$arrayproductname[$viewproduct[$i]].($arrayselfcode[$viewproduct[$i]]?"-".$arrayselfcode[$viewproduct[$i]]:"").($arrayaddcode[$viewproduct[$i]]?"-".$arrayaddcode[$viewproduct[$i]]:"")."&nbsp;</td>\n";
									/*
