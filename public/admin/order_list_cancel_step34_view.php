...
# 넘어온 값들을 정리한다.
#---------------------------------------------------------------
	$opstep     = $_GET["opstep"];  // 단계정보 (40 : 배송중, 배송완료일때의 반품신청, 41 : 배송중, 배송완료일때의 반품접수)
	$mode=$_POST["mode"];
	...
	if ($mode == 'update_40') {
		...
		$c_paymethod		= explode(",", $_POST['c_paymethod']);
		...
		for ($i=0;$i < count($oc_no);$i++) {
			...
			if (strstr("Q", $c_paymethod)) {
				// 상품별 반품이 아니거나 반품접수, 환불접수 상태가 아닌게 있는지 체크한다.
				list($op_rt_cnt)=pmysql_fetch_array(pmysql_query("select count(redelivery_type) as op_rt_cnt from tblorderproduct WHERE ordercode='".trim($ordercode[$i])."' AND idx NOT IN ('".str_replace("|", "','", $idxs[$i])."') AND (redelivery_type != 'Y' or (redelivery_type = 'Y' AND op_step NOT IN ('41', '42'))) "));
				if ($op_rt_cnt == 0) { // 전체가 반품접수, 환불접수 상태일 경우
					...
					// 정산보류로 보낸다.
					$query="sitecd={$pgid_info["ID"]}&sitekey={$pgid_info["KEY"]}&ordercode=".$ordercode[$i];
			...
			if ($fail_cnt == 0) {
                ...
                //SendVenderForm( $shopname, $shopurl, $ordercode[$i], $mail_type, $info_email, 3, $oc_no[$i] );
		...
		for ($i=0;$i < count($oc_no);$i++) {
			...
			if(pmysql_query($sql,get_db_conn())) {
				...
				list($op_deli_gbn_cnt)=pmysql_fetch_array(pmysql_query("select count(deli_gbn) as op_deli_gbn_cnt from tblorderproduct WHERE ordercode='".trim($ordercode[$i])."'AND idx NOT IN ('".str_replace("|", "','", $idxs[$i])."') AND deli_gbn != 'E' AND op_step != '44'"));
				if ($op_deli_gbn_cnt == 0 ) { // 주문취소가 아닌 상품들 모두 반송상태일 경우
					$sql = "UPDATE tblorderinfo SET deli_gbn='E' WHERE ordercode='".trim($ordercode[$i])."' ";
					//echo $sql;
...
/*$ordercode		= "2016010322014826299A";
$idxs		= "1442990518";
$oi_step1	="3";
$oi_step2	="0";
$paymethod		= "C";
...
$bankuser		="";
$re_type		="B";*/
//orderCancel($exe_id, $ordercode, $idxs, $oi_step1, $oi_step2, $paymethod, $code, $memo, $bankcode, $bankaccount, $bankuser, $re_type );
...
//	$period[4] = substr($_shopdata->regdate,0,4)."-".substr($_shopdata->regdate,4,2)."-".substr($_shopdata->regdate,6,2);
	...
	if($vendercnt>0) {
		...
		$qry_from .= "join (Select p.oc_no, pb.brandname, p.vender 
					...
					where p.oc_no > 0 
					and p.redelivery_type = 'Y' 
					and p.op_step=".$opstep." 
					group by p.oc_no, p.vender, pb.brandname) b 
					on toc.oc_no=b.oc_no {$subqry} ";
		...
		$qry_from .= "join (Select oc_no from tblorderproduct p where oc_no > 0 and redelivery_type = 'Y' and op_step=".$opstep." group by oc_no) b on toc.oc_no=b.oc_no ";

		$excel_qry_from .= ", (Select oc_no from tblorderproduct p where oc_no > 0 and redelivery_type = 'Y' and op_step=".$opstep." group by oc_no) b ";
		$excel_qry.= "AND toc.oc_no=b.oc_no ";
	}
	...
	$excel_sql = "SELECT  b.vender, b.ordercode, b.productcode, b.productname, b.opt1_name, b.opt2_name, b.quantity, b.price, b.option_price,
					...
					b.coupon_price, b.use_point, b.op_step, b.opt1_change, b.opt2_change, b.oc_no, b.date, b.idx, b.option_price_text_change, b.option_quantity, toc.regdt, toc.code,
					a.id, a.sender_name, a.paymethod, a.oi_step1, a.oi_step2, b.self_goods_code, b.prodcode, b.colorcode
...
function searchForm() {
	document.form1.action="order_list_cancel_step34_view.php";
...
function OrderRegoodsSubmit() {
	...
	document.exeform.c_paymethod.value="";
	var k = 0;
	if (chk_len == 1)
	{
		if(document.form2.chk_oc_no.checked) {
			...
			document.exeform.c_paymethod.value+=document.form2.c_paymethod.value;
			k++;
		}
	} else {
		for(i=0;i<document.form2.chk_oc_no.length;i++) {
			if(document.form2.chk_oc_no[i].checked) {
				if (k == 0)
				{
					...
					document.exeform.c_paymethod.value+=document.form2.c_paymethod[i].value;
				} else {
					document.exeform.oc_no.value+=","+document.form2.chk_oc_no[i].value;
					document.exeform.ordercode.value+=","+document.form2.ordercode[i].value;
					document.exeform.idxs.value+=","+document.form2.idxs[i].value;
...
<body>
			...
			<tr>
				<td>
					<!-- 소제목 -->
					<div class="title_depth3_sub"><span>반품<?if ($opstep=='40') {?>신청<?} else if ($opstep=='41') {?>접수<?}?> 내역을 확인/처리하실 수 있습니다.</span></div>
				</td>
			</tr>
...
?>
			<tr bgcolor=<?=$tr_bgc?>>
			<td align="center" style='border:1px solid <?=$tr_bgc?>'><input type=checkbox name="chk_oc_no" value="<?=$row->oc_no?>"<?=$chk_dis?>><input type=hidden name="ordercode" value="<?=$row->ordercode?>"><input type=hidden name="c_paymethod" value="<?=$row->paymethod[0]?>"></td>
...
?>
			...
			<tr>
				...
				<col width=130></col>
				<tr>
					<td align='left' valign=middle><a href="javascript:OrderRegoodsSubmit();"><img src="images/btn_<?if ($opstep=='40') {?>req<?} else if ($opstep=='41') {?>regoods<?}?>.gif" border="0" hspace="0"></a></td>
					<td align='center'>
					...
					<td align='right' valign=middle><a href="javascript:OrderRegoodsExcel();"><img src="images/btn_excel_select.gif" border="0" hspace="1"></a></td>
				<tr>
				</table>
			</tr>
			...
			<input type=hidden name="item_type" value="order_cancel_34_<?=$opstep?>">
			...
			<input type=hidden name=c_paymethod>
			</form>
