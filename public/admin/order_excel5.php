...
//if($_SERVER[REMOTE_ADDR]!="121.126.44.129"){
	...
	Header("Content-Disposition: attachment; filename=세이브힐즈_발주".date("Ymd",$CurrentTime)."(정상재고_발주)_사은품.csv"); 
	Header("Pragma: no-cache"); 
	Header("Expires: 0");*/
//}	
	header( 'Content-type: application/vnd.ms-excel' );
	Header("Content-Disposition: attachment; filename=물류발주".date("Ymd",$CurrentTime).".xls"); 
; 
	header( 'Expires: 0' );
...
$excel_info = "49,50,11,19,15,16,51,52,27,24,35,44,53,54";
...
$excelval=array(
	...
	array("상품명-갯수-옵션"	,&$productname),		#24
	array("상품가격"								,&$price),				#25
	...
	array("박스수량"									,&$temp2),				#51
	array("운임구분"									,&$temp3),				#52
	array("VIP ZONE"									,&$vip_product),				#53
	array("STAFF ZONE"									,&$staff_product)				#54
);
$temp1="일반주문";
$temp2=" ";
$temp3="선불";
...
//		$receiver_tel2num="=\"{$receiver_tel2num}\""; 
		...
		if (ord($row->opt2_name)) $option.=(ord($option)==0?"":"-").$row->opt2_name;
		$option2=$addcode.$option;
		$productname.=" <br>{$product}-".$quantity.(ord($option2)==0?"":"-".$option2);
		$productname2.="<br>{$product}-".$quantity.(ord($option2)==0?"":"-".$option2);
		$prdt_message=str_replace($pattern,$replacement,strip_tags($row->order_prmsg));
		$prdt_deli_gbn=$row->prdt_deli_gbn;
		$prdt_deli_date=$row->prdt_deli_date;
		$prdt_selfcode=$row->selfcode;
	...
	$vssql ="select vip_product, staff_product FROM tblproduct where productcode ='{$row->productcode}' ";
	$vsres = pmysql_query($vssql);
	$vsrow = pmysql_fetch_object($vsres);
	if( $vsrow->vip_product == "1"){
		$vip_product = "VIP";
	}
	if( $vsrow->staff_product == "1"){
		$staff_product = "STAFF";
	}
