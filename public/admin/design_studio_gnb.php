...
function lbModify(lbno) {
	location.href="design_studio_gnb_write.php?&mode=modfiy_select&lbno="+lbno;
}
...
function lbAdd() {
	location.href="design_studio_gnb_write.php";
}
</script>
		...
		<tr>
			...
			<tr>
				...
				<tr>
					...
					<option value="left_title" <?php if($s_check=="title")echo"selected";?>>좌측 타이틀로 검색</option>
					<option value="right_title" <?php if($s_check=="subtitle")echo"selected";?>>우측 타이틀로 검색</option>
					</select>
...
#---------------------------------------------------------------
		...
		if($t_count>0) {
			...
			while($row=pmysql_fetch_object($result)) {
				...
				echo "	<td align=center>" . $row->left_title . "</td>\n";
				echo "	<td align=center>" . $row->right_title . "</td>\n";
			...
			echo "<tr><td colspan=7 align=center>검색된 정보가 존재하지 않습니다.</td></tr>";
		}
?>
				</TABLE>
				</div>
