...
$excel_info = "49,35,1,11,15,16,18,19,44,6,42,10,21,38,23,25,9,27,52,50,51";
...
$excelval=array(
	...
	array("번호"									,&$vnum),				#49
	array("VIP ZONE"									,&$vip_product),#50
	array("STAFF ZONE"									,&$staff_product),#51
	array("배송방법"										,&$deli_type) #52
);
...
if($isproduct=="Y" || $isproductall=="Y") {
	...
	$sql.= "a.receiver_addr,a.order_msg,a.del_gbn,a.deli_type";
...
while ($row=pmysql_fetch_object($result)) {
	...
	if ($temp!=$row->ordercode) {
		if($isproductall=="Y" && strlen($temp)!=0) {
			for($i=0;$i<$cnt;$i++) {
				if($i!=0) echo ",";
				echo '"x' . doubleQuote($excelval[$arr_excel[$i]][1]) . '"';
			}
			echo "\n";
		}
