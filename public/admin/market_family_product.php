...
<div class="admin_linemap"><div class="line"><p>현재위치 : 마케팅지원 &gt; 이벤트/사은품 기능 설정 &gt;<span>패밀리세일 진열관리</span></p></div></div>
	...
	<tr>
		...
		<tr>
			...
			<tr>
				<td>
					<!-- 페이지 타이틀 -->
					<div class="title_depth3">패밀리세일 진열관리
						<div align="right">
						<a href="/admin/market_family_reg.php?mode=mod&pidx=<?=$pidx?>" target="_self">
					...
					<!-- 소제목 -->
					<div class="title_depth3_sub"><span>쇼핑몰 패밀리세일 페이지에 진열할 상품을 등록할 수 있습니다.</span></div>
...
<?php
		...
		?>
							...
							<td valign=top style="height: 1100px;">
								<!-- 소제목 -->
								<?
								$tsql = "SELECT title FROM tblfamily_list WHERE idx='{$pidx}'";
								$tres = pmysql_fetch_array(pmysql_query($tsql));
								?>
								<div class="title_depth3_sub"><?=$tres[title]?><br> 진열 리스트</div>

								<IFRAME name="MainPrdtFrame" src="market_family_product.main.php?pidx=<?=$pidx?>&special=<?=$special?>" width=100% height=100% frameborder=0 align=TOP scrolling="no" marginheight="0" marginwidth="0"></IFRAME>
