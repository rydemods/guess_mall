...
$dc_rate_check = $_POST["dc_rate_check"];
...
//				move_uploaded_file($file[0],$imagepath2.$image[0]);
	...
	if ($mode=="insert") {
		...
		$join_productcode = "";
		$join_productname = "";
		foreach($_REQUEST[set_productcode] as $key=>$val) {
			$join_productcode = $join_productcode . $val . "|";
				
		}
		foreach($_REQUEST[set_productname] as $key=>$val) {
			$join_productname = $join_productname . $val . "|";
				
		}
		$join_productcode = substr($join_productcode,0,strlen($join_productcode)-1);
		$join_productname = substr($join_productname,0,strlen($join_productname)-1); 
		$join_productcode = $join_productcode . "^" .  $join_productname; //결합상품추가david 20170321
		...
		$sql.= "display			= '{$display}', ";
		$sql.= "join_yn			= '{$join_yn}', ";
		$sql.= "join_productcode= '{$join_productcode}', ";
        $sql.= "naver_display	= '{$naver_display}', ";
        $sql.= "erp_price_yn	= '{$erp_price_yn}', ";
		$sql.= "dc_rate_check	= '{$dc_rate_check}', ";
...
<tr>
	...
	<input type=hidden name=insertdate value="Y">
...
<?php } ?>
					...
					<tr>
						<th><span>세트상품여부</span> <font color='#FF0000' > *필수</font></th>
						<td class="td_con1" colspan="2">
<?php
	
	if($_data->join_yn=="Y") $join_y_checked = "checked";
	if($_data->join_yn=="N" or $_data->join_yn=="") $join_n_checked = "checked";

	
?>

<script>
function joinproduct_check(){
	
	if($('[name="join_yn"]:checked').val()=='Y'){
		$('#join_admin_area').show();
		//가격표시비활성화
		$('.joinproduct_disable_area').hide();

		
	}else{
		$('#join_admin_area').hide();
		//가격표시활성화
		$('.joinproduct_disable_area').show();
...
function ChoiceProduct(){
	window.open("coupon_productchoice3.php","coupon_product","width=700,height=800,scrollbars=yes");
}
...
$(document).ready(function(){
	joinproduct_check();
							...
							<input type=radio id="idx_display1" name="join_yn" value="N" <?=$join_n_checked?> onclick="joinproduct_check();">
							<label style='cursor:hand;' onMouseOver="style.textDecoration='underline'" onMouseOut="style.textDecoration='none'" for=idx_display1>일반상품</label> &nbsp;
							<input type=radio id="idx_display2" name="join_yn" value="Y" <?=$join_y_checked?> onclick="joinproduct_check();">
							<label style='cursor:hand;' onMouseOver="style.textDecoration='underline'" onMouseOut="style.textDecoration='none'" for=idx_display2>세트상품</label>
							
							<table cellpadding="0" cellspacing="0" width="100%" id="join_admin_area" style="display: none;" >
								<tbody><tr>
									<td width="100"><a href="javascript:ChoiceProduct();"><img src="images/btn_select2.gif" border="0" hspace="2"></a></td>
									<td>
										<div id="ID_productLayer">
											
<?
//세트상품 david 20170321
$join_productcode_arr = "";



$join_productcode_arr = explode("^", $_data->join_productcode);
$join_productcode_arr0 = explode("|", $join_productcode_arr[0]);
$join_productcode_arr1 = explode("|", $join_productcode_arr[1]);

?>
<?

if(count($join_productcode_arr0)<1){
	
}else{
	 for($joi=0; $joi < count($join_productcode_arr0); $joi++) { 
    
?>
	
											<div style="padding:5px 0px;">
												<a href="javascript:;" onclick="javascript:$(this).parent().remove();">
													<img src="images/icon_del1.gif" border="0" style="vertical-align:middle;">
												</a>&nbsp;&nbsp;<?=$join_productcode_arr1[$joi]?>
												<input type="hidden" name="set_productcode[]" value="<?=$join_productcode_arr0[$joi]?>">
												<input type="hidden" name="set_productname[]" value="<?=$join_productcode_arr1[$joi]?>">		
											</div>
<?	}
}
?>											
					...
					<tr>
						<th><span>상품진열여부</span> <font color='#FF0000' > *필</font></th>
						<td class="td_con1">
<?php
...
?>
					...
					<tr class="joinproduct_disable_area">
...
<?php	
	if($_data->vender>0){ 
		if($_data->sellprice=="") $_data->sellprice =0;
?>
		<input type=hidden name="assembleuse" value="N">
					<tr class="joinproduct_disable_area">
						...
						<td class="td_con1">
							<span><span class="font_orange">* 가격 일괄변경</span></span>
							<input type=radio id="dc_rate_check1" name=dc_rate_check value="Y" <?if($_data->dc_rate_check=="Y"){echo "checked";}?> onclick="sellpriceDisabled('Y');">
							<label style='cursor:hand;' onMouseOver="style.textDecoration='underline'" onMouseOut="style.textDecoration='none'" for=dc_rate_check1>연동함</label> &nbsp;
							<input type=radio id="dc_rate_check2" name=dc_rate_check value="N" <?if($_data->dc_rate_check=="N"){echo "checked";}?> onclick="sellpriceDisabled('N');">
							<label style='cursor:hand;' onMouseOver="style.textDecoration='underline'" onMouseOut="style.textDecoration='none'" for=dc_rate_check2>연동안함(가격별도수정시)</label>
							
							<input name=sellprice value="<?=$_data->sellprice?>" size=16 maxlength=10 class="input" style=width:98% >
						</td>
...
<script>
/*
function sellpriceDisabled(opt){
	if(opt=='Y'){
		$('[name="sellprice"]').prop("disabled", true);	
	}
	if(opt=='N'){
		$('[name="sellprice"]').prop("disabled", false);	
	}
	
}

sellpriceDisabled(dc_rate_check);
*/
var dc_rate_check = '<?=$_data->dc_rate_check?>';
</script>
...
?>
                    ...
                    <tr class="joinproduct_disable_area" style='display:none;'>
...
?>
								<?foreach($arrDbColorCode as $ck => $cv){?>
                                        ...
                                        <!-- <img src="../../static/img/test/<?=$cv->color_img?>" alt="<?=$cv->color_name?>" title="<?=$cv->color_name?>"> --><?=$cv->color_name=='transparent'?"없음":$cv->color_name?>
								<?}?>
						</td>
					</tr>
					<tr>
...
<?php
	echo "<option value='001' selected >INFO</option>";
	/* deco는 info만 볼수엤게 한다
...
<input type=hidden name=dc_rate_check value="<?=$dc_rate_check?>">
