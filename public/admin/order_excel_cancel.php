...
Header("Content-Disposition: attachment; filename=order_excel_cancel_".date("Ymd",$CurrentTime).".xls"); 
Header("Pragma: no-cache"); 
Header("Expires: 0");
Header("Cache-Control: must-revalidate, post-check=0,pre-check=0");
Header("Content-Description: PHP4 Generated Data");

$excel_info = "49,0,35,1,52,21,6,25,23,50,28,27,51,10";
...
if($ordercodes) $ordercodes= str_replace(",''","",$ordercodes);
	...
	$sql = "SELECT  a.vender, v.com_name, a.ordercode, a.productcode, a.productname, a.opt1_name, a.opt2_name, a.quantity, a.price, 
                        ...
                        b.id, b.sender_name, b.paymethod, b.oi_step1, b.oi_step2 
                FROM {$qry_from} {$qry} 
		        ORDER BY a.ordercode {$orderby} 
			";
...
while ($row=pmysql_fetch_object($result)) {
	...
	//if ($temp!=$row->ordercode) {
		...
		//$totprice=$row->price-$row->coupon_price-$row->use_point+$row->deli_price;
        $totprice=($row->price*$row->quantity)-$row->coupon_price-$row->use_point+$row->deli_price;
