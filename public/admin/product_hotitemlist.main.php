...
if( $mall_type == '' || is_null($mall_type) ){
	$mall_type = '0';
}
//exdebug($inCateCode);
################### 상품 insert
...
if($mode == "insert"){
	...
	$num++;
	$sql = "INSERT INTO tblproduct_hotitem_list
				(
					pridx
					,category_idx
					,sort
...
if($mode == "delete"){
	for($i=0;$i<count($list_idx);$i++){
		$sortNum = $i+1;
		$sql = "UPDATE tblproduct_hotitem_list SET
			sort = {$sortNum}
	...
	$sql = "DELETE FROM tblproduct_hotitem_list WHERE list_idx = {$delIdx} ";
	pmysql_query($sql,get_db_conn());
}
...
$nowCateList_sql = "
	...
	FROM tblproduct_hotitem_list a
	JOIN tblproductcode b ON a.category_idx=b.idx
	JOIN tblproduct c ON a.pridx=c.pridx ";

if ($nowCode) $nowCateList_sql .= "WHERE a.category_idx = {$nowCode} ";
$nowCateList_sql .= "AND a.mall_type = {$mall_type} ORDER BY sort ASC";
