<?php
/********************************************************************* 
// 파 일 명		: order_list_regoods_req.php 
// 설     명		: 주문상품 반품접수 요청
// 상세설명	: 반품접수 요청내역을 확인/처리하실 수 있습니다.
...
#---------------------------------------------------------------
	...
	if ($mode == 'update') {
		...
		for ($i=0;$i < count($oc_no);$i++) {	
			...
			if (strstr("Q", $c_paymethod[$i])) {
				// 상품별 반품이 아니거나 반품접수, 환불접수 상태가 아닌게 있는지 체크한다.
				list($op_rt_cnt)=pmysql_fetch_array(pmysql_query("select count(redelivery_type) as op_rt_cnt from tblorderproduct WHERE ordercode='".trim($ordercode[$i])."' AND idx NOT IN ('".str_replace("|", "','", $idxs[$i])."') AND (redelivery_type != '{$c_redelivery_type}' or (redelivery_type = '{$c_redelivery_type}' AND op_step NOT IN ('41', '42'))) "));
...
//orderCancel($exe_id, $ordercode, $idxs, $oi_step1, $oi_step2, $paymethod, $code, $memo, $bankcode, $bankaccount, $bankuser, $re_type );
	...
	if($vendercnt>0) {
		...
		$qry_from .= "join (Select p.oc_no, pb.brandname, p.vender from tblorderproduct p left join tblvenderinfo v on p.vender = v.vender left join tblproductbrand pb on p.vender=pb.vender where p.oc_no > 0 and p.redelivery_type = 'Y' and p.op_step=40 group by p.oc_no, p.vender, pb.brandname) b on toc.oc_no=b.oc_no {$subqry} ";
	} else {
		$qry_from .= "join (Select oc_no from tblorderproduct p where oc_no > 0 and redelivery_type = 'Y' and op_step=40 group by oc_no) b on toc.oc_no=b.oc_no ";
	}
...
function searchForm() {
	document.form1.action="order_list_regoods_req.php";
...
function OrderExcel() {
    //alert("excel");
	document.form1.action="order_excel_regoods_req.php";
...
<div class="admin_linemap"><div class="line"><p>현재위치 : 주문/매출  &gt; 주문조회 및 배송관리 &gt;<span>반품접수 내역 관리</span></p></div></div>
	...
	<tr>
		...
		<tr>
			...
			<tr>
				<td>
					<!-- 페이지 타이틀 -->
					<div class="title_depth3">반품접수 내역 관리</div>
					<!-- 소제목 -->
					<div class="title_depth3_sub"><span>반품접수 내역을 확인/처리하실 수 있습니다.</span></div>
				</td>
			</tr>
...
?>
			...
			<tr>
				...
				<col width=130></col>
				<tr>
					<td align='left'><a href="javascript:OrderRegoodsSubmit();"><img src="images/btn_regoods.gif" border="0" hspace="0"></a></td>
					<td align='center'>
					<table cellpadding="0" cellspacing="0" width="100%">
<?php				
		echo "<tr>\n";
...
?>
					...
					<td align='right'><a href="javascript:OrderRegoodsExcel();"><img src="images/btn_excel_select.gif" border="0" hspace="1"></a></td>
				<tr>
				</table>
			</tr>
			...
			<form name=checkexcelform action="order_excel_regoods_req.php" method=post>
...
<iframe name="processFrame" src="about:blank" width="1000" height="2000" scrolling=no frameborder=no></iframe>
<?=$onload?>
<?php 
