...
$redelivery_type = $_POST["redelivery_type"];
...
if(substr($search_s,0,8)==substr($search_e,0,8)) {
	$qry.= "WHERE sabangnet_idx ='' AND a.ordercode LIKE '".substr($search_s,0,8)."%' ";
} else {
	$qry.= "WHERE sabangnet_idx ='' AND a.ordercode>='{$search_s}' AND a.ordercode <='{$search_e}' ";
}
...
WHERE sabangnet_idx ='' AND a.deli_gbn='N'
...
WHERE sabangnet_idx ='' AND a.deli_gbn='S'
{$check_qry}"));

//발송중
$y_count=pmysql_num_rows(pmysql_query("SELECT a.* FROM tblorderinfo a
WHERE sabangnet_idx ='' AND a.deli_gbn='Y'
{$check_qry}"));
...
WHERE sabangnet_idx ='' AND a.deli_gbn='C'
{$check_qry}"));
...
//반송신청
$redelivery_count=pmysql_num_rows(pmysql_query("SELECT a.* FROM tblorderinfo a
WHERE sabangnet_idx ='' AND a.deli_gbn='Y' AND redelivery_type='Y' 
{$check_qry}"));

//반송
$r_count=pmysql_num_rows(pmysql_query("SELECT a.* FROM tblorderinfo a
WHERE sabangnet_idx ='' AND a.deli_gbn='R'
{$check_qry}"));

//취소요청
$d_count=pmysql_num_rows(pmysql_query("SELECT a.* FROM tblorderinfo a
WHERE sabangnet_idx ='' AND a.deli_gbn='D'
{$check_qry}"));

//환불대기
$e_count=pmysql_num_rows(pmysql_query("SELECT a.* FROM tblorderinfo a
WHERE sabangnet_idx ='' AND a.deli_gbn='E'
{$check_qry}"));
...
function serial_pop(a,b,c,d){
	new_win= window.open("about:blank","serial_pop","scrollbars=yes,width=500,height=400,resizable=yes,location='no',menubar='no',toolbar='no'");
	document.serial_form.target="serial_pop";
	document.serial_form.ordercode.value=a;
	document.serial_form.productcode.value=b;
	document.serial_form.quantity.value=c;
	document.serial_form.arr_value.value=d;
	document.serial_form.action="order_serial_pop.php";
	document.serial_form.submit();
	new_win.focus();
	}
								...
								<div class="order_step_btn02">
									...
									<div class="cancle_btn">
										<ul>
											...
											<li><a>반송신청 <?=$redelivery_count?></a></li>
...
?>
						...
						<tr>
							<th><span>반송신청</span></th>
							<TD>
								<input type="checkbox" name="redelivery_type" value="Y" <? if($redelivery_type) echo "checked"; ?>> 반송신청
							</TD>
						</tr>
						</TABLE>
						</div>
...
<?
$ordersteparr = array("1"=>"주문접수","2"=>"입금확인","3"=>"발송준비","4"=>"발송완료(배송중)","5"=>"주문취소","6"=>"카드실패","7"=>"반송","8"=>"취소요청","9"=>"환불대기","10"=>"환불","11"=>"배송완료");
//exdebug($ordersteparr);
			...
			<tr>
				...
				foreach($list as $row){
					...
					while($row2=pmysql_fetch_object($result2)) {
						...
						$prval.="<tr>\n";
						if($vendercnt>0) {
							$prval.="	<td>".(ord($venderlist[$row2->vender]->vender)?"<B><a href=\"javascript:viewVenderInfo({$row2->vender})\">{$venderlist[$row2->vender]->com_name}</a></B>":"-")."</td>\n";
							$prval.="	<td>";
				...
				//	echo "	<col width=75 align=center></col>\n";
					...
					if($row->redelivery_type == "Y") echo "<br><font color=red>[반송신청]</font>";
					...
					echo "			</tr>\n";
						...
						foreach($row->items as $itemVal){
							...
							echo " <a style='margin: 0 0 0 0; width=70%;' href = \"javascript:ProductInfo('".substr($itemVal->productcode,0,12)."','".$itemVal->productcode."','YES')\">".$itemVal->productname."</a>";

							echo "	<td style = 'width:40px;".$tdBorderColor."'>".number_format($itemVal->quantity)."</td>\n";
							echo "	<td style = 'width:100px;".$tdBorderColor."'>".number_format( ( $itemVal->price + $itemVal->op_price ) * $itemVal->quantity)."</td>\n";
...
?>
			...
			<tr>
				...
				<!--<a href="javascript:OrderDeliPrint();"><img src="images/btn_print.gif"  border="0" hspace="1"></a>-->
				&nbsp;<a href="javascript:OrderCheckPrint();"><img src="images/btn_juprint.gif"  border="0" hspace="0"></a>&nbsp;<a href="javascript:OrderCheckExcel();"><img src="images/btn_excel1.gif"  border="0" hspace="1"></a>&nbsp;<a href="javascript:OrderCheckDelete();"><img src="images/btn_judel.gif"  border="0"></a>&nbsp;<a href="javascript:OrderCheckExcel4();"><img src="img/btn/btn_company_order_data.gif"  border="0" hspace="1"></a>&nbsp;<a href="javascript:OrderCheckExcel3();"><img src="images/btn_order_data.gif"  border="0" hspace="1"></a>&nbsp;<a href="javascript:OrderCheckExcel5();"><img src="images/btn_distribution_data.gif"  border="0" hspace="1"></a>&nbsp;<a href="javascript:OrderCheckExcel2();"><img src="images/btn_erp_sale.gif"  border="0" hspace="1"></a></td>
			</tr>
			<!--tr>
				<td align="center">
				<table cellpadding="0" cellspacing="0" width="100%">
...
<form name=serial_form  method=post>
<input type=hidden name=ordercode></input>
<input type=hidden name=productcode></input>
<input type=hidden name=quantity></input>
<input type=hidden name=arr_value></input>
</form>
<?=$onload?>
<?php
