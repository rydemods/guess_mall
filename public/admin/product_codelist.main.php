...
if ($mode=="sequence" && $change=="Y" && ord($selcodes) && ord($code) && ord($special)) {
...
} elseif ($mode=="modify" && ord($selcodes) && ord($code) && ord($special)) {
	$sql = "UPDATE tblspecialcode SET special_list = '{$selcodes}' ";
	$sql.= "WHERE code='{$code}' AND special='{$special}' ";
	pmysql_query($sql,get_db_conn());
	$onload="<script>alert('해당 상품을 카테고리 진열상품에 추가하였습니다.');</script>\n";
} elseif ($mode=="insert" && ord($selcodes) && ord($code) && ord($special)) {
	$sql = "INSERT INTO tblspecialcode(code,special,special_list) VALUES (
	'{$code}', 
	'{$special}', 
	'{$selcodes}')";
	pmysql_query($sql,get_db_conn());
	$onload="<script>alert('해당 상품을 카테고리 진열상품에 추가하였습니다.');</script>\n";
} elseif ($mode=="delete" && ord($code) && ord($special)) {
	if(ord($selcodes)==0) {
...
<input type=hidden name=Scrolltype value="<?=$Scrolltype?>">
	<div class="main_view_setup_wrap" style="margin-top:15px;">
		<div class="group">
			...
			<label style='cursor:hand; TEXT-DECORATION: none' onmouseover="style.textDecoration='underline'" onmouseout="style.textDecoration='none'" for=idx_special1 class="font_d12">Best Item</label>

			<input type=radio id="idx_special2" name=special value="2" <?=$chk_special2?> onClick="ChangeSpecial(this.value);"> 
			<label style='cursor:hand; TEXT-DECORATION: none' onmouseover="style.textDecoration='underline'" onmouseout="style.textDecoration='none'" for=idx_special2 class="font_d12">MD's Pick</label>

			<input type=radio id="idx_special3" name=special value="3" <?=$chk_special3?> onClick="ChangeSpecial(this.value);"> 
			<label style='cursor:hand; TEXT-DECORATION: none' onmouseover="style.textDecoration='underline'" onmouseout="style.textDecoration='none'" for=idx_special3 class="font_d12">상품 상세 추천상품</label>

			<input type=radio id="idx_special4" name=special value="4" <?=$chk_special4?> onClick="ChangeSpecial(this.value);"> 
			<label style='cursor:hand; TEXT-DECORATION: none' onmouseover="style.textDecoration='underline'" onmouseout="style.textDecoration='none'" for=idx_special4 class="font_d12">상품리스트 FOCUS ON</label>
<br />
			<input type=radio id="idx_special5" name=special value="5" <?=$chk_special5?> onClick="ChangeSpecial(this.value);"> 
			<label style='cursor:hand; TEXT-DECORATION: none' onmouseover="style.textDecoration='underline'" onmouseout="style.textDecoration='none'" for=idx_special5 class="font_d12">NEW ARRIVALS</label>
			
			<input type=radio id="idx_special6" name=special value="6" <?=$chk_special6?> onClick="ChangeSpecial(this.value);"> 
			<label style='cursor:hand; TEXT-DECORATION: none' onmouseover="style.textDecoration='underline'" onmouseout="style.textDecoration='none'" for=idx_special6 class="font_d12">Hot Item</label>
