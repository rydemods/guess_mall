...
} else if($type=="issuedelete" && ord($coupon_code) && ord($uid)) {	//회원에게 발급한 쿠폰 삭제
...
<?php
					...
					while($row=pmysql_fetch_object($result)) {
						...
						if($row->used=="Y") {
							...
							echo "	<TD class=\"td_con1\"><a href=\"javascript:IssueCouponDelete('{$row->coupon_code}','{$row->id}');\"><img src=\"images/btn_del7.gif\" border=\"0\"></a></TD>\n";
						}
						echo "</TR>\n";
					}
					pmysql_free_result($result);
