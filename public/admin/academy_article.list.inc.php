...
-->
    ...
    <form name=changeForm method=post>
        ...
        FROM tblboard a LEFT OUTER JOIN tblproduct b
        ON a.pridx=b.pridx
        {$qry}
        ...
        while ($row = pmysql_fetch_object($result)) {
            ...
            $subject .= "<a href=\"{$_SERVER['PHP_SELF']}?exec=view&num={$row->num}&board={$board}&block={$nowblock}&gotopage={$gotopage}&category={$category}&search={$search}&s_check={$s_check}\">";
            $title = $row->title;
            if ($len_title) {
                        ...
                        <TD>
                            <table border="0" cellpadding="0" cellspacing="0" width="100%">
                                <tr>
                                    ...
                                    <TD align="right">
                                        <A HREF="<?=$_SERVER['PHP_SELF']?>?board=<?=$board?>&exec=write&category=<?=$_REQUEST[category]?>">
...
<SCRIPT LANGUAGE="JavaScript">
    ...
    function changeListView(kind) {
        ...
        if (!isTrue) {
            ...
            if (kind == "change") {
                changeForm.action = "community_article_changepop.php?board=<?=$board?>&block=<?=$nowblock?>&gotopage=<?=$gotopage?>&category=<?=$category?>&search=<?=$search?>&s_check=<?=$s_check?>";
                ...
                if (con) {
                    changeForm.action = "community_article_deletepop.php?board=<?=$board?>&block=<?=$nowblock?>&gotopage=<?=$gotopage?>&category=<?=$category?>&search=<?=$search?>&s_check=<?=$s_check?>";
