...
if ($mode=="sequence" && $change=="Y" && ord($selcodes) && ord($special)) {
	$sql = "UPDATE tblissuemain SET issue_list = '{$selcodes}' WHERE issue_number='{$special}' ";
	pmysql_query($sql,get_db_conn());
	$onload="<script>alert('이슈 상품 순서 조정이 완료되었습니다.');</script>\n";
} else if ($mode=="modify" && ord($selcodes) && ord($special)) {
	$sql = "UPDATE tblissuemain SET issue_list = '{$selcodes}' WHERE issue_number='{$special}' ";
	pmysql_query($sql,get_db_conn());
	$onload="<script>alert('해당 상품을 이슈상품 카테고리에 추가하였습니다.');</script>\n";
} else if ($mode=="insert" && ord($selcodes) && ord($special)) {
	$sql = "INSERT INTO tblissuemain(issue_number,issue_list) VALUES (
	'{$special}', 
	'{$selcodes}')";
	pmysql_query($sql,get_db_conn());
	$onload="<script>alert('해당 상품을 이슈상품 카테고리에 추가하였습니다.');</script>\n";
	...
	if(ord($selcodes)==0) {
		$sql = "DELETE FROM tblissuemain WHERE issue_number='{$special}' ";
	} else {
		$sql = "UPDATE tblissuemain SET issue_list = '{$selcodes}' WHERE issue_number='{$special}' ";
	}
	pmysql_query($sql,get_db_conn());
	$onload="<script>alert('해당 상품을 이슈상품에서 삭제하였습니다.');</script>\n";
}
...
function InsertSpecial() {
	if (document.form1.prcode.value.length==0) {
		alert("이슈상품에 추가할 상품을 선택하세요.");
		document.form1.prcode.focus();
		return;
	}
	num = all_list.length-1;
	if(num+1>=50){
		alert('이슈 진열상품은 최대 50개까지 등록가능합니다.');
		return;
	}
	if (confirm("해당 상품을 이슈 진열상품으로 포함하시겠습니까?")){
		temp = "";
		for (i=0;i<=num;i++) {
...
function Delete(delcode) {
	ProductInfoStop = "1";
	if(!confirm("해당 상품을 이슈상품에서 삭제하시겠습니까?")) return;
	val="";
	for(i=0;i<all_list.length;i++)
	{
...
<input type=hidden name=Scrolltype value="<?=$Scrolltype?>">
	<div class="main_view_setup_wrap" style="margin-top:15px;">
		<div class="group">
		<?
		$query="select * from tblmainbannerimg where banner_name='issue'";
		$result=pmysql_query($query);
		$i=1;
		while($data=pmysql_fetch_object($result)){
			
		?>
			<input type=radio id="idx_special<?=$data->no?>" name=special value="<?=$data->no?>" <?=$checked[$data->no]?> onClick="ChangeSpecial(this.value);"> 
			<label style='cursor:hand; TEXT-DECORATION: none' onmouseover="style.textDecoration='underline'" onmouseout="style.textDecoration='none'" for="idx_special<?=$data->no?>" class="font_d12"><?=$data->banner_title?></label>&nbsp;&nbsp;&nbsp;
		
		<?$i++;}?>
			<div style="display:none">
			<input type=radio id="idx_special2" name=special value="2" <?=$chk_special2?> onClick="ChangeSpecial(this.value);"> 
			<label style='cursor:hand; TEXT-DECORATION: none' onmouseover="style.textDecoration='underline'" onmouseout="style.textDecoration='none'" for=idx_special2 class="font_d12">인기상품</label>&nbsp;&nbsp;&nbsp;
			<input type=radio id="idx_special3" name=special value="3" <?=$chk_special3?> onClick="ChangeSpecial(this.value);"> 
			<label style='cursor:hand; TEXT-DECORATION: none' onmouseover="style.textDecoration='underline'" onmouseout="style.textDecoration='none'" for=idx_special3 class="font_d12">추천상품</label>&nbsp;&nbsp;&nbsp;
			<input type=radio id="idx_special4" name=special value="4" <?=$chk_special4?> onClick="ChangeSpecial(this.value);"> 
			<label style='cursor:hand; TEXT-DECORATION: none' onmouseover="style.textDecoration='underline'" onmouseout="style.textDecoration='none'" for=idx_special4 class="font_d12">특별상품</label>
			</div>
			...
			<div class="table_main_setup"  id="divscroll" style="height:400px;overflow-x:hidden;overflow-y:auto;">
				<table border=0 cellpadding=0 cellspacing=0 width=100%>
					...
					if(ord($special)) {
						$mode="insert";
						$sp_prcode="";
						$sql = "SELECT issue_list FROM tblissuemain WHERE issue_number='{$special}' ";
						$result = pmysql_query($sql,get_db_conn());
						if($row = pmysql_fetch_object($result)){
							$cnt_prcode=$row->issue_list;
