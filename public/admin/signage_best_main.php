...
$nowCode = $_POST["nowCode"];   // 1:Best Sellers, 2:MD's Pick, 3:New Arrivals
$prcode = $_POST["prcode"];
$mode = $_POST["mode"];
$num = $_POST["num"];
$delIdx = $_POST["delIdx"];
...
if($mode == "insert"){
	...
	$num++;
	$sql = "INSERT INTO tblsignage_best
				(
					pridx
					,category_type
					,sort
...
if($mode == "delete"){
	for($i=0;$i<count($list_idx);$i++){
		$sortNum = $i+1;
		$sql = "UPDATE tblsignage_best SET
			sort = {$sortNum}
	...
	$sql = "DELETE FROM tblsignage_best WHERE list_idx = {$delIdx} ";
	pmysql_query($sql,get_db_conn());
}
...
$market_sql="select * from tblsignage_store order by no asc";
$market_result=pmysql_query($market_sql);


$select["market_no"][$market_no]="selected";
...
<input type="hidden" name="delIdx" >
	...
	<div class="main_view_setup_wrap" style="margin-top:15px;">
		...
		<div>
			<select name=market_no onchange="javascript:market_go()">
				<option value="">=== 매장 선택 ===</option>
				<?while($market_data=pmysql_fetch_array($market_result)){?>
					<option value='<?=$market_data["no"]?>' <?=$select["market_no"][$market_data["no"]]?>><?=$market_data["name"]?></option>
				<?}?>
			</select>
		</div>
		<div class="group">
			<input type=radio id="idx_special1" name=special value="1" <?if($nowCode == '1') echo "checked";?> onClick="ChangeSpecial(this.value);">
			<label style='cursor:hand; TEXT-DECORATION: none' onmouseover="style.textDecoration='underline'" onmouseout="style.textDecoration='none'" for=idx_special1 class="font_d12">베스트상품</label>&nbsp;&nbsp;&nbsp;
			<input type=radio id="idx_special2" name=special value="2" <?if($nowCode == '2') echo "checked";?> onClick="ChangeSpecial(this.value);">
			<label style='cursor:hand; TEXT-DECORATION: none' onmouseover="style.textDecoration='underline'" onmouseout="style.textDecoration='none'" for=idx_special2 class="font_d12">오늘의 신상템</label>&nbsp;&nbsp;&nbsp;
			<input type=radio id="idx_special3" name=special value="3" <?if($nowCode == '3') echo "checked";?> onClick="ChangeSpecial(this.value);">
			<label style='cursor:hand; TEXT-DECORATION: none' onmouseover="style.textDecoration='underline'" onmouseout="style.textDecoration='none'" for=idx_special3 class="font_d12">오늘 이런신발 어때요?</label>&nbsp;&nbsp;&nbsp;
...
<?php
					foreach($nowCateList as $k=>$v){
						$min_img = getProductImage($imagepath, $v[tinyimage] );
?>
					<tr align="center" name="prList" alt="<?=$v[list_idx]?>">
						...
						<td style="position:relative;">
							...
							<?}else{?>
							<img style="width: 40px; height:40px;" src="<?=$min_img?>" border="1"/>
							<?}?>
