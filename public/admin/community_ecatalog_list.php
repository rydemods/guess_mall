...
#---------------------------------------------------------------
	...
	if ( $mode == "delete" ) {
		...
		$sql  = "SELECT * FROM tblecatalog WHERE no = '".$_POST["no"]."' ";
		...
		$qry = "DELETE FROM tblecatalog WHERE no ='".$_POST["no"]."'";
		pmysql_query( $qry, get_db_conn() );
		//exdebug($sql);
		callNaver('ecatalog', $_POST["no"], 'del');
...
// 		}
		...
		if ($s_check == 'title') {
			$qry.= "AND ( UPPER(title) LIKE any ( array[".implode(",", $search_arr)."] ) ) ";
		} else if ($s_check == 'content') {
			$qry.= "AND ( UPPER(content) LIKE any ( array[".implode(",", $search_arr)."] ) ) ";
		}
	}
...
#---------------------------------------------------------------
	...
	$sql = "SELECT COUNT(*) as t_count FROM  tblecatalog {$qry} ";
...
function Modify(no) {
	location.href="community_ecatalog_write.php?&mode=modfiy_select&no="+no;
}
...
function Add() {
	location.href="community_ecatalog_write.php";
}

</script>
<div class="admin_linemap"><div class="line"><p>현재위치 : 커뮤니티  &gt; ECATALOG 관리 &gt; <span>ECATALOG 정보관리</span></p></div></div>
	...
	<tr>
		...
		<tr>
			...
			<tr>
				<td>
					<!-- 페이지 타이틀 -->
					<div class="title_depth3">ECATALOG 정보관리</div>
					<!-- 소제목 -->
			...
			<tr>
				<td>
					<!-- 소제목 -->
					<div class="title_depth3_sub">ECATALOG 검색 선택</div>
				</td>
			</tr>
			<tr>
				<td>
...
#---------------------------------------------------------------
		...
		if($t_count>0) {
			$sql = "SELECT * FROM tblecatalog {$qry} ";
			...
			<tr>
				...
				<!-- 매뉴얼 -->
					<div class="sub_manual_wrap">
						...
						<dl>
							<dt><span>ECATALOG 정보관리</span></dt>
							<dd>- 등록된 ECATALOG 리스트와 기본적인 정보사항을 확인할 수 있습니다.
							</dd>	
						</dl>
