...
function MainInsert(prcode,chk) {//chk_in에서 호출 확인 조건 추가
	max=50;
...
<div class="admin_linemap"><div class="line"><p>현재위치 : 상품관리 &gt; 카테고리/상품관리 &gt;<span>메뉴별 상품 진열관리</span></p></div></div>
	...
	<tr>
		...
		<tr>
			...
			<tr>
				<td>
					<!-- 페이지 타이틀 -->
					<div class="title_depth3">메뉴별 상품 진열관리</div>
					<!-- 소제목 -->
            	...
            	<td>
					<div class="table_style01">
						...
						<table width=100% cellpadding=0 cellspacing=0 border=0>
							...
							<td>
								<select name=code_a style="width:170px;">
									<option value="">〓〓 1차 카테고리 〓〓</option>
				<?php
								$sql = "SELECT * FROM tblproductcode WHERE group_code!='NO' ";
								$sql.= "AND (type!='T' AND type!='TX' AND type!='TM' AND type!='TMX') AND (type='L' OR type='LX') ORDER BY cate_sort ASC ";
								$result = pmysql_query($sql,get_db_conn());
								while($row=pmysql_fetch_object($result)) {
				?>
									<option <?if($row->code_a==$code_a) echo "SELECTED"?> value="<?=$row->code_a?>"><?=$row->code_name?></option>
				<?php			}
...
<?php
		...
		?>
							...
							<td valign=top>
								<!-- 소제목 -->
								<div class="title_depth3_sub">메뉴별 진열 리스트</div>
							<?

								/*if($serial_value){
										$iframeSrc = "product_recommendlist.main.php?".$serial_value."&autoUri=1";
								}else{*/
									$iframeSrc = "product_menulist.main.php?inCateCode=".$inCateCode;
								/*}*/
							?>
