...
if($it['it_id']) {
    ...
    $sql = " SELECT * FROM tblproduct_option WHERE option_type = '0' AND productcode = '".$it['it_id']."' ORDER BY option_num ASC ";
...
<div class="sit_option_frm_wrapper">
    ...
    if($it['it_id']) {
        for($i=0; $row=pmysql_fetch_array($result); $i++) {
            ...
            $opt_stock_qty = $row['option_quantity'];
           // $opt_noti_qty = $row['io_noti_qty'];
            $opt_use = $row['option_use'];
    ?>
    <tr>
        <td class="td_chk">
        ...
        for($i=0; $i<$opt1_count; $i++) {
            ...
            do {
                ...
                do {
                    ...
                    // 기존에 설정된 값이 있는지 체크
                    if($_POST['w'] == 'u') {
                        $sql = " SELECT option_price, option_quantity, option_quantity_noti, option_use 
                                    FROM tblproduct_option 
                                    WHERE productcode = '{$_POST['it_id']}'
                                      AND option_code = '$opt_id'
                                      AND option_type = '0' ";
                        $row = pmysql_fetch($sql);

                        if($row) {
                            $opt_price = (int)$row['option_price'];
                            $opt_stock_qty = (int)$row['option_quantity'];
                            $opt_noti_qty = (int)$row['option_quantity_noti'];
                            $opt_use = (int)$row['option_use'];
                        }
                    }
    ?>
    <tr>
