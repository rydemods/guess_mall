...
//	$strName = "<A style=\"cursor:point;text-decoration:underline\">{$row['name']}</A>";
	$strName = "<A href='javascript:;' onClick=\"javascript:CrmView('" . $row['mem_id'] . "');\">{$row['name']}</A>";
}

// 입점문의 글일 경우 핸폰 및 sms 수신여부 추가..2016-08-23 jhjeong
if(ord($row['storetel'])) {
    if($row['sms_send']) $sms_yn = "Y";
    else $sms_yn = "N";

    $strName .= " [HP : ".$row['storetel']." / SMS 수신여부 : ".$sms_yn."]";
}
...
$vstorefilename = stripslashes($row['vstorefilename']);
...
<tr>
	...
	<TR>
		...
		if($vstorefilename!=''){
		?>
		<a href='<?=$link_url?>' target='_blank'><img src='/data/shopimages/board/event/<?=$vstorefilename?>'></a>
		<?
		}
		?>
	...
	<TR>
		<Th>링크 URL</Th>
		<TD><p align="center"><?=$link_url?></p></TD>
	</TR>
	<?
	}
	?>
...
<?php
	if ($setup['use_comment'] == "Y") {
		...
		echo "<form method='post' name='modi_comment_form' action='community_article.php'>\n";		
...
//		if(isdev()) echo "	alert(document.comment_form.mode.value);\n";
		...
		if ($board == "qna") {
			...
			echo "	<TD class=board_cell1 width=581 bgColor=#fafafa colSpan=2>\n";
			 echo "	&nbsp;▣ <b>문의 답변</b><INPUT type=hidden name=up_name value='관리자'>";
			echo "	</TD>\n";
			echo "</TR>\n";
		} else {
			echo "	<TR>\n";
		...
		*/
		...
		for ($jjj=0;$jjj<count($com_list);$jjj++) {
			...
			echo "	</TR>\n";
			echo "	<TR>\n";
			echo "		<TD class=tk1 width=\"760\" height=22><B><span class=\"font_blue\">{$c_name}</span></B> / <span class=\"board_con1s\">{$c_writetime} ({$c_ip})</span> <A style=\"CURSOR:hand;\" onclick=\"modiOpen('{$c_num}')\"><IMG SRC=\"{$imgdir}/btn_modify1.gif\" border=0 align=\"absmiddle\" vspace=\"4\" alt=\"수정\"></A> <A style=\"CURSOR:hand;\" onclick=\"check_del('{$_SERVER['PHP_SELF']}?mode=comment_del&board={$board}&num={$num}&c_num={$c_num}&s_check={$s_check}&search={$search}&block={$block}&gotopage={$gotopage}')\"><IMG SRC=\"{$imgdir}/btn_delete1.gif\" border=0 align=\"absmiddle\" vspace=\"4\" alt=\"삭제\"></A></TD>\n";
			echo "	</TR>\n";
			...
			$re_qry="select * from tblboardcomment_re where parent={$c_num}";
			$re_result=pmysql_query($re_qry);
			while($re_data=pmysql_fetch_object($re_result)){
				$c_num_re = $re_data->num;
				$c_name_re = $re_data->name;

				$c_uip_re=$re_data->ip;

				$c_writetime_re = date("Y-m-d H:i:s",$re_data->writetime);
				$c_comment_re = nl2br(stripslashes($re_data->comment));
				$c_ip_re = $re_data->ip;
				...
				echo "	</TR>\n";
				echo "	<TR>\n";
				echo "		<TD class=tk1 width=\"760\" height=22><img src=\"/board/images/skin/L_TEM01/re_mark.gif\" style=\"display:inline;\"><B><span class=\"font_blue\">{$c_name_re}</span></B> / <span class=\"board_con1s\">{$c_writetime_re} ({$c_ip})</span><A style=\"CURSOR:hand;\" onclick=\"check_del('{$_SERVER['PHP_SELF']}?mode=comment_del_re&board={$board}&num={$num}&c_num={$c_num_re}&s_check={$s_check}&search={$search}&block={$block}&gotopage={$gotopage}')\"><IMG SRC=\"{$imgdir}/del_x.gif\" width=27 border=0 align=\"absmiddle\" vspace=\"4\" alt=\"삭제\"></A></TD>\n";
				echo "	</TR>\n";
				echo "	<TR>\n";
				echo "		<TD style='word-break:break-all;' class=tk1 width=\"760\" height=22>{$c_comment_re}</TD>\n";
				echo "	</TR>\n";
				echo "	<TR>\n";
				echo "		<TD width=\"760\" height=5></TD>\n";
				echo "	</TR>\n";
