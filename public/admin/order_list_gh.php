...
$CurrentTime = time();
$period[0] = date("Y-m-d",$CurrentTime);					// 년-월-일 (현재시간)
$period[1] = date("Y-m-d",$CurrentTime-(60*60*24*7));		// -1주
$period[2] = date("Y-m-d",$CurrentTime-(60*60*24*14));		// -2주
$period[3] = date("Y-m-d",strtotime('-3 month'));			// -3달
$period[4] = date("Y-m-d",$CurrentTime-(60*60*24*10));		// -10일
$period[4] = date("Y-m-d",strtotime('-3 day'));				// -3일
...
$search_start=$search_start?$search_start:$period[4];				// post변수값 없으면 현재시간-3일
$search_end=$search_end?$search_end:date("Y-m-d",$CurrentTime);		// post변수 값 없으면 현재시간
				...
				<td>
					...
					<table border=0 cellpadding=0 cellspacing=0 width=100%>
						...
						<tr>
							...
							<td><div class="order_step_btn01"><a class="this"><span>미입금<br /><?=$n_count?> 건
							</span></a></div></td>
...
?>
		<div class="table_style02">
			...
			<tr>
				...
				foreach($list as $row){ /////////// 모든 조건에 맞는 쿼리를 order class에서 수행한 결과 - return
					list($q_count)=pmysql_fetch_array(pmysql_query("SELECT count(id) FROM tblmember_question WHERE id = '".$row->id."'"));	// list($q_count) 사용하는곳 없음
