...
// 설     명		: APP Intro 이미지 관리
// 상세설명	: APP Intro 이미지 관리
...
#---------------------------------------------------------------
	...
	$banner_type	= 'F'; // F : Intro 이미지, E : 종료 팝업
	...
	# 이미지 경로
	$imagepath = $Dir.DataDir."shopimages/app/intro/";
	...
	if($mode=="delete") {
		...
		list($v_banner_img1, $v_banner_img2, $v_banner_img3, $v_banner_img4, $v_banner_img5) = pmysql_fetch("SELECT img1, img2, img3, img4, img5 FROM tblappbannerimg WHERE no ='".trim($img_no)."' ");
		...
		if( strlen( $v_banner_img3 ) > 0 && is_file( $imagepath.$v_banner_img3 ) ){ $banner_file->removeFile( $v_banner_img3 ); }
		if( strlen( $v_banner_img4 ) > 0 && is_file( $imagepath.$v_banner_img4 ) ){ $banner_file->removeFile( $v_banner_img4 ); }
		if( strlen( $v_banner_img5 ) > 0 && is_file( $imagepath.$v_banner_img5 ) ){ $banner_file->removeFile( $v_banner_img5 ); }
		...
		for($u=0;$u<5;$u++) {
			if( strlen( $banner_img["banner_img"][$u]["v_file"] ) > 0 ){
				...
				if ($u == 2) $where[] = "img3='".$banner_img["banner_img"][2]["v_file"]."'";
				if ($u == 3) $where[] = "img4='".$banner_img["banner_img"][3]["v_file"]."'";
				if ($u == 4) $where[] = "img5='".$banner_img["banner_img"][4]["v_file"]."'";
			}
		}	
		...
		$qry="insert into tblappbannerimg (
			...
			'".$banner_img["banner_img"][2]["v_file"]."',
			'".$banner_img["banner_img"][3]["v_file"]."',
			'".$banner_img["banner_img"][4]["v_file"]."',
			'".$banner_hidden."',
			'now()'
		) ";
...
function changeAction( mode , num ){
	...
	if( mode == '0' ){
		...
		if ($("input[name='banner_img[1]']").val() != '') img_cnt++;
		if ($("input[name='banner_img[2]']").val() != '') img_cnt++;
		if ($("input[name='banner_img[3]']").val() != '') img_cnt++;
		if ($("input[name='banner_img[4]']").val() != '') img_cnt++;
		//alert(img_cnt);
...
<div class="admin_linemap"><div class="line"><p>현재위치 : 배너관리 &gt; App 이미지 관리 &gt;<span>Intro 이미지 관리</span></p></div></div>
	...
	<tr>
		...
		<tr>
			...
			<tr>
				<td>
					<!-- 페이지 타이틀 -->
					<div class="title_depth3">Intro 이미지 관리</div>

					<!-- 소제목 -->
					<div class="title_depth3_sub"><span>Intro 이미지 등록/수정/삭제 처리를 할 수 있습니다.</span></div>
				</td>
            </tr>
					...
					<TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
						...
						<tr>
							...
							<td class="td_con1" colspan="3" style="position:relative">
								...
								<span class="font_orange">(권장이미지 : 640px X 1136px)</span>
...
<?	} ?>
						...
						<tr>
							...
							<td class="td_con1" colspan="3" style="position:relative">
								...
								<span class="font_orange">(권장이미지 : 750px X 1334px)</span>
...
<?	} ?>
						...
						<tr>
							<th><span>이미지3</span></th>
							<td class="td_con1" colspan="3" style="position:relative">
								<input type=file name="banner_img[2]" style="WIDTH: 400px"><br>
								<span class="font_orange">(권장이미지 : 1242px X 2208px)</span>
								<input type=hidden name="v_banner_img[2]" value="<?=$mSelect['img3']?>" >
								<div style='margin-top:5px' >
<?	if( is_file($imagepath.$mSelect['img3']) ){ ?>
									<img src='<?=$imagepath.$mSelect['img3']?>' style='max-width: 125px;' />
<?	} ?>
								</div>
							</td>
						</tr>
						<tr>
							<th><span>이미지4</span></th>
							<td class="td_con1" colspan="3" style="position:relative">
								<input type=file name="banner_img[3]" style="WIDTH: 400px"><br>
								<span class="font_orange">(권장이미지 : 720px X 1280px)</span>
								<input type=hidden name="v_banner_img[3]" value="<?=$mSelect['img4']?>" >
								<div style='margin-top:5px' >
<?	if( is_file($imagepath.$mSelect['img4']) ){ ?>
									<img src='<?=$imagepath.$mSelect['img4']?>' style='max-width: 125px;' />
<?	} ?>
								</div>
							</td>
						</tr>
						<tr>
							<th><span>이미지5</span></th>
							<td class="td_con1" colspan="3" style="position:relative">
								<input type=file name="banner_img[4]" style="WIDTH: 400px"><br>
								<span class="font_orange">(권장이미지 : 1440px X 2560px)</span>
								<input type=hidden name="v_banner_img[4]" value="<?=$mSelect['img5']?>" >
								<div style='margin-top:5px' >
<?	if( is_file($imagepath.$mSelect['img5']) ){ ?>
									<img src='<?=$imagepath.$mSelect['img5']?>' style='max-width: 125px;' />
<?	} ?>
								</div>
							</td>
						</tr>
...
?>	
					...
					<td>
						<div id='img_display' >
<?php
			if( is_file($imagepath.$bVal['img3']) ){
?>
							<img src='<?=$imagepath.$bVal['img3']?>' style='max-width : 70px;' >
<?php
			} else {
				echo '-';
			}
?>	
					...
					<td>
						<div id='img_display' >
<?php
			if( is_file($imagepath.$bVal['img4']) ){
?>
							<img src='<?=$imagepath.$bVal['img4']?>' style='max-width : 70px;' >
<?php
			} else {
				echo '-';
			}
?>	
					...
					<td>
						<div id='img_display' >
<?php
			if( is_file($imagepath.$bVal['img5']) ){
?>
							<img src='<?=$imagepath.$bVal['img5']?>' style='max-width : 70px;' >
<?php
			} else {
				echo '-';
			}
