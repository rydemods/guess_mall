					...
					<tr>
						<td>
							...
							?>
								<!-------------------- 측면 작은메뉴 시작 ------------------------------->
								<li>
								<?if(trim($menu['value'][$i][$j])=="none.php"){?>
									 <a href="javascript:alert('준비중입니다.')">
									
								<?}else if(strstr($_SERVER['SCRIPT_FILENAME'],"community_article.php")){ ?>

								<a href="<?=$menu['value'][$i][$j]?>" name="navi" <?if(isset($menu['target'][$i][$j])) {?>target="<?=$menu['target'][$i][$j]?>"<?}?> <?if(strstr($menu['value'][$i][$j],"&board=".$_GET[board]) && $_GET[board]){?>class="this"<?}?>>

								<?}else if(strstr($_SERVER['SCRIPT_FILENAME'],"cscenter_freeboard.php")){ ?>
								...
								<?}else if(trim($menu['value'][$i][$j])){?>
								<a href="<?=$menu['value'][$i][$j]?>" name="navi" <?if(isset($menu['target'][$i][$j])) {?>target="<?=$menu['target'][$i][$j]?>"<?}?> <?if(preg_match('/'.str_replace('/','\/',$menu['value'][$i][$j]).'/',$_SERVER['SCRIPT_FILENAME'])){?>class="this"<?}?>>
								<?}?>
								<?=trim($menu['subject'][$i][$j])?>
								<?if(trim($menu['value'][$i][$j])){?></a><?}?>
								<?if(strstr($_SERVER['SCRIPT_FILENAME'],"community_article.php")){?>
									<?$menuopen="1";?>
								<?}else{?>
									<?if(preg_match('/'.str_replace('/','\/',$menu['value'][$i][$j]).'/',$_SERVER['SCRIPT_FILENAME'])){$menuopen=$i;}?>
								<?}?>
								<?if(strstr($_SERVER['SCRIPT_FILENAME'],"cscenter_freeboard.php")){?>
									<?$menuopen="1";?>
								<?}else{?>
									<?if(preg_match('/'.str_replace('/','\/',$menu['value'][$i][$j]).'/',$_SERVER['SCRIPT_FILENAME'])){$menuopen=$i;}?>
								<?}?>
