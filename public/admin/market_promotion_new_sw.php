...
function event_pop(mode,idx){
	...
	document.location.href="market_promotion_reg_new.php?page_type=event&mode="+mode+"&pidx="+idx;
}
...
<div class="admin_linemap"><div class="line"><p>현재위치 : 마케팅지원 &gt; 이벤트/사은품 기능 설정 &gt;<span>이벤트 관리</span></p></div></div>
		...
		<tr>
			...
			<tr>
				<td>
					...
					<!-- 소제목 -->
					<div class="title_depth3_sub"><span>이벤트 검색</span></div>
...
?>
					...
					<?
						...
						if($row->display_type=="A"){
							?><a target="_balnk" href="/front/promotion_detail.php?idx=<?=$row->idx?>&event_type=<?=$row->event_type?>"> <?echo "/front/promotion_detail.php?idx=".$row->idx."&event_type=".$row->event_type;?></a></a>
