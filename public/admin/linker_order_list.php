...
if(substr($search_s,0,8)==substr($search_e,0,8)) {
	$qry.= "WHERE a.shoplinker_order_id !='' AND a.ordercode LIKE '".substr($search_s,0,8)."%' ";
} else {
	$qry.= "WHERE a.shoplinker_order_id !='' AND a.ordercode>='{$search_s}' AND a.ordercode <='{$search_e}' ";
}
...
WHERE a.shoplinker_order_id !='' AND a.deli_gbn='N'
...
WHERE a.shoplinker_order_id !='' AND a.deli_gbn='S'
{$check_qry} {$qry2}"));

//발송중
$y_count=pmysql_num_rows(pmysql_query("SELECT a.* FROM tblorderinfo a
WHERE a.shoplinker_order_id !='' AND a.deli_gbn='Y'
{$check_qry} {$qry2}"));
...
WHERE a.shoplinker_order_id !='' AND a.deli_gbn='C'
{$check_qry} {$qry2}"));
...
WHERE a.shoplinker_order_id !='' AND a.deli_gbn='R'
{$check_qry} {$qry2}"));

//취소요청
$d_count=pmysql_num_rows(pmysql_query("SELECT a.* FROM tblorderinfo a
WHERE a.shoplinker_order_id !='' AND a.deli_gbn='D'
{$check_qry} {$qry2}"));

//환불대기
$e_count=pmysql_num_rows(pmysql_query("SELECT a.* FROM tblorderinfo a
WHERE a.shoplinker_order_id !='' AND a.deli_gbn='E'
{$check_qry} {$qry2}"));


//환불
$cns_count=pmysql_num_rows(pmysql_query("SELECT a.* FROM tblorderinfo a
WHERE a.shoplinker_order_id !='' AND ((SUBSTR(a.paymethod,1,1) IN ('B','O','Q') AND LENGTH(a.bank_date)=9) OR (SUBSTR(a.paymethod,1,1) IN ('C','P','M','V') AND a.pay_flag='0000' AND a.pay_admin_proc='C'))
{$check_qry} {$qry2}"));
?>
<style>
	.crmView{
...
function searchForm() {
	document.form1.action="linker_order_list.php";
...
?>
						...
						<tr>
							...
							<input type=text name=search value="<?=$search?>" style="width:197" class="input" onkeydown="if(event.keyCode==13){javascript:searchForm();}"></TD>
						</tr>
						<tr>
							...
							<input type=text name=search2 value="<?=$search2?>" style="width:197" class="input" onkeydown="if(event.keyCode==13){javascript:searchForm();}"></TD>
						</tr>
						<tr>
							<th><span>바코드검색</span></th>
							<TD class="">
								<input type="text" name="hlcBarcode" class="input" style="ime-mode:disabled;"><!-- onkeydown="if(event.keyCode==13){javascript:barcodesubmit();}"-->
							</TD>
						</tr>
						</TABLE>
						</div>
		...
		<div class="table_style02">
			...
			<tr>
				...
				foreach($list as $row){ 
					...
					echo "	<td align=\"center\"><A HREF=\"javascript:OrderDetailView('{$row->ordercode}')\">{$date}<br>{$row->ordercode}<br>{$row->shoplinker_order_id}</A></td>\n";
					echo "	<td style=\"font-size:8pt;padding:3;line-height:11pt\">\n";
					echo "	주문자: <FONT COLOR=\"blue\">{$name}</font>".$blackImage;
					/*
					if(ord($stridX)) {
						echo "<br> 주문번호.: ".$stridX;
					} else if(ord($stridM)) {
					...
					list($clameProcCount) = pmysql_fetch_array(pmysql_query("SELECT count(shoplinker_order_id) FROM tblorderlinkerclame WHERE shoplinker_order_id = '".$row->shoplinker_order_id."' AND shoplinker_proc_flag = 'N' "));
					list($clameDoneCount) = pmysql_fetch_array(pmysql_query("SELECT count(shoplinker_order_id) FROM tblorderlinkerclame WHERE shoplinker_order_id = '".$row->shoplinker_order_id."' AND shoplinker_proc_flag = 'Y' "));
					$clameStr = "";
					if($clameProcCount){
				...
				//	echo "	<col width=75 align=center></col>\n";
					...
					echo "사방넷 주문<br>";
					echo " <font color=#FF5D00>[".$row->shoplinker_mall_name."]</font>";
