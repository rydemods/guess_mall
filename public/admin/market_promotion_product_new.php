...
function MainInsert(prcode,chk) {
	...
	*/
	...
	if( $("#MainPrdtFrame").contents().find('#special :selected').val().length > 0 ){
		is_special = true;
	}
            	...
            	<td>
					<div class="table_style01">
						...
						<table width=100% cellpadding=0 cellspacing=0 border=0>
							...
							<td><!-- <input class="w200" type="text" name="keyword" value="<?=$keyword?>"> --><textarea rows="2" cols="10" class="w200" name="keyword" id="keyword" style="resize:none;vertical-align:middle;"><?=$keyword?></textarea></td>
							</tr>
							<tr>
...
//                                         $qry.= "AND (UPPER(productname) || UPPER(productcode)) LIKE '%{$search_word}%' ";
										...
										$qry.= "AND (UPPER(productname) || UPPER(productcode)) LIKE any ( array[".implode(",", $search_arr)."] ) ";
                                    }
									...
									$sql0 = "SELECT COUNT(*) as t_count FROM tblproduct a ";
                                    $sql0.= "WHERE 1=1 AND sabangnet_flag='N'";
									$sql0.= $qry;
									...
									echo "<div id=\"primage{$cnt}\" style=\"position:absolute; z-index:100; left:50px;display:none;\"><table border=\"0\" cellspacing=\"0\" cellpadding=\"0\" width=\"170\">\n";
...
<?php
		...
		?>
							<div style="text-align:center;">
								<a href="/admin/market_promotion_reg_new.php?mode=mod&pidx=<?=$pidx?>" target="_self">
									<img src="/admin/images/btn_promo_mod.gif" alt="기획전수정"/></a>&nbsp;
								<a href="/front/promotion_detail.php?pidx=<?=$pidx?>" target="_blank">
									<img src="/admin/images/btn_preview.gif" alt="미리보기"/></a>
							</div>
							</td>
							<td align=center><input type="button" value="선택상품 일괄등록" class="chk_in_all" onclick="chk_in()" style="padding:5px;margin:0px 10px"></td>
							<td valign=top style="height: 1100px;">
								...
								<IFRAME name="MainPrdtFrame" id='MainPrdtFrame' src="market_promotion_product.main_new.php?pidx=<?=$pidx?>&special=<?=$special?>" width=100% height='100%' frameborder=0 align=TOP scrolling="no" marginheight="0" marginwidth="0"></IFRAME>
