...
if($type=="update" && ord($body)) {
	...
	if($row->cnt==0) {
		...
		body		= '{$body}' 
		WHERE type='reviewall' ";
		pmysql_query($sql,get_db_conn());
	}
	pmysql_free_result($result);
	$onload="<script>window.onload=function(){ alert(\"리뷰모음 화면 디자인 수정이 완료되었습니다.\"); }</script>";
} elseif($type=="delete") {
	$sql = "DELETE FROM tbldesignnewpage WHERE type='reviewall' ";
	pmysql_query($sql,get_db_conn());
	$onload="<script>window.onload=function(){ alert(\"리뷰모음 화면 디자인 삭제가 완료되었습니다.\"); }</script>";
} elseif($type=="clear") {
	$body="";
...
<div class="admin_linemap"><div class="line"><p>현재위치 : 디자인관리 &gt; 개별디자인-페이지 본문 &gt;<span>리뷰모음 화면 꾸미기</span></p></div></div>
	...
	<tr>
		...
		<tr>
			...
			<tr>
				<td>
                    <!-- 페이지 타이틀 -->
					<div class="title_depth3">리뷰모음 화면 꾸미기</div>
					<div class="title_depth3_sub"><span>리뷰모음 화면 디자인을 자유롭게 디자인 하실 수 있습니다.</span></div>
                </td>
            </tr>
            <tr>
            	<td>
					<!-- 소제목 -->
					<div class="title_depth3_sub">리뷰모음 화면 개별디자인</div>
				</td>
			</tr>
			<tr>
				<td>
			...
			<tr>
				<td>
					...
					<div class="sub_manual_wrap">
						...
						<dl>
							<dt><span class="point_c1">리뷰모음 화면 매크로명령어</span><span>(해당 매크로명령어는 다른 페이지 디자인 작업시 사용이 불가능함)</span></dt>
							<dd>
