...
if ($mode=="sequence" && $change=="Y" && ord($selcodes) && ord($special)) {
	$sql = "UPDATE tblspecialmainmobile SET special_list = '{$selcodes}' WHERE special='{$special}' ";
	...
	$sql = "INSERT INTO tblspecialmainmobile(special,special_list) VALUES (
	'{$special}', 
	'{$selcodes}')";
	pmysql_query($sql,get_db_conn());
	$onload="<script>alert('해당 상품을 메인화면 진열상품 카테고리에 추가하였습니다.');</script>\n";
	...
	if(ord($selcodes)==0) {
		$sql = "DELETE FROM tblspecialmainmobile WHERE special='{$special}' ";
	} else {
		$sql = "UPDATE tblspecialmainmobile SET special_list = '{$selcodes}' WHERE special='{$special}' ";
	}
	pmysql_query($sql,get_db_conn());
...
function ProductInfo(prcode) {
	...
	if (popup=="YES") {
		document.form_reg.action="product_register.set.php?p_code=popup";
...
<input type=hidden name=Scrolltype value="<?=$Scrolltype?>">
	<div class="main_view_setup_wrap" style="margin-top:15px;">
		<div class="group">
			...
			<label style='cursor:hand; TEXT-DECORATION: none' onmouseover="style.textDecoration='underline'" onmouseout="style.textDecoration='none'" for=idx_special1 class="font_d12">신상품</label>&nbsp;&nbsp;&nbsp;
			...
			<div class="table_main_setup"  id="divscroll" style="height:400px;overflow-x:hidden;overflow-y:auto;">
				<table border=0 cellpadding=0 cellspacing=0 width=100%>
					...
					if(ord($special)) {
						$mode="insert";
						$sp_prcode="";
						$sql = "SELECT special_list FROM tblspecialmainmobile WHERE special='{$special}' ";
						$result = pmysql_query($sql,get_db_conn());
