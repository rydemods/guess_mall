...
//orderCancel($exe_id, $ordercode, $idxs, $oi_step1, $oi_step2, $paymethod, $code, $memo, $bankcode, $bankaccount, $bankuser, $re_type );
	...
	if($vendercnt>0) {
		...
		$qry_from .= "join (Select p.oc_no, pb.brandname, p.vender from tblorderproduct p left join tblvenderinfo v on p.vender = v.vender left join tblproductbrand pb on p.vender=pb.vender where p.oc_no > 0 and p.redelivery_type = 'Y' and p.op_step=41 group by p.oc_no, p.vender, pb.brandname) b on toc.oc_no=b.oc_no {$subqry} ";
	} else {
		$qry_from .= "join (Select oc_no from tblorderproduct p where oc_no > 0 and redelivery_type = 'Y' and op_step=41 group by oc_no) b on toc.oc_no=b.oc_no ";
	}
...
function searchForm() {
	document.form1.action="order_list_regoods.php";
...
function OrderExcel() {
    //alert("excel");
	document.form1.action="order_excel_regoods.php";
...
?>
			...
			<form name=checkexcelform action="order_list_regoods_excel.php" method=post>
