...
if($mode=="insert") {
	...
	if($group_code==1111){
		$sql = "UPDATE tblmember SET vip_type = 1 WHERE id IN ({$inid}) ";
	}else{
...
?>
									<option value="1111" <?if($group_code==1111){echo " selected";$group_description='VIP';}?> >VIP</option>
									</select>
								</TD>
...
?>
	...
	?>
								...
								<tr>
									<th><span>VIP</span></th>
									<TD class="td_con1" colspan="3"><input type=checkbox name=vipmember value="Y" style="BORDER-RIGHT: medium none; BORDER-TOP: medium none; BORDER-LEFT: medium none; BORDER-BOTTOM: medium none;" <?php if($vipmember=="Y") echo "checked"?> > <label style='cursor:hand;' onmouseover="style.textDecoration='underline'" onmouseout="style.textDecoration='none'" for=idx_groupmember1>VIP</label> &nbsp;&nbsp; 
									</TD>
								</tr>
								</table>
								</div>
...
//					if ($searchtype=="M" || $searchtype=="U") {
						$qry= "WHERE a.member_out = 'N' ";
						if ($groupmember=="YES")
							$qry.= "AND a.group_code = '' ";
						else if ($groupmember=="ONE")
							$qry.= "AND (a.group_code = '' OR a.group_code != '{$group_code}') ";
						if ($vipmember) $qry.= "AND a.vip_type = 1 ";
						if ($search!='') {
							$qry.= "AND (a.name LIKE '%{$search}%' OR a.id LIKE '%{$search}%' ) ";
						}
						if($sex!="ALL" && $sex=="M") $qry.= "AND  a.gender='1' ";
						else if($sex!="ALL" && $sex=="F") $qry.= "AND  a.gender='2' ";
						if ($age!="ALL"){
							...
							if ($start_year < 2000 && $end_year < 2000) {
								$qry.= "AND (SUBSTR(a.birth,3,2) BETWEEN '{$s_year}' AND '{$e_year}') ";
								//$qry.= "AND SUBSTR(resno,7,1) < '3' ";
							} else if ($start_year < 2000 && $end_year > 1999) {
								$qry.= "AND (SUBSTR(a.birth,3,2) BETWEEN '{$s_year}' AND '99') ";
						...
						if($birth!="ALL"){
							if($birthday!="ALL") $qry.= "AND SUBSTR(a.birth,6,5) = '".$birthmonth."-".$birthday."' ";
							else $qry.= "AND SUBSTR(a.birth,6,2) = '{$birthmonth}' ";
						}
						if($memregdate!="ALL"){
							...
							$qry.= "AND a.date >= '{$memregdate1}' AND a.date <= '{$memregdate2}' ";
						}
						if($reserve!="ALL"){
							...
							$qry.= "AND a.reserve >= '{$reserveminvalue}' AND a.reserve <= '{$reservemaxvalue}' ";
						}
						if($addr!="NO"){
							$qry.= "AND a.home_addr LIKE '{$seladdr}%' ";
						}
						...
						if($buydate!="ALL" || $price!="ALL" || $ordercnt!="ALL"){
							...
							*/
							...
							$qry.= "and a.id IN ({$mulid}) ";
						}
						
						
						$sql = "SELECT COUNT(a.*) as t_count FROM tblmember a ";
						$sql.= $qry;
						$paging = new Paging($sql,10,20);
						$t_count = $paging->t_count;
						$gotopage = $paging->gotopage;

						$sql = "SELECT a.id, a.reserve, a.name, a.age, a.gender as sex, SUBSTR(birth,1,4) as birth,
									a.group_code, SUBSTR(a.home_addr,1,2) as addr, a.memo ,a.vip_type FROM tblmember a
								LEFT JOIN tblmembergroup b on a.group_code = b.group_code ";
						$sql.= $qry." ";
						if($sort=="reserve_desc") $sql.= "ORDER BY a.reserve DESC ";
						elseif($sort=="reserve_asc") $sql.= "ORDER BY a.reserve ";
						elseif($sort=="name_asc") $sql.= "ORDER BY a.name ";
						else $sql.= "ORDER BY b.group_level asc, a.name DESC ";
...
*/
					...
					for($i=0;$i<$count;$i++) {
						...
						echo "	</td>\n";
						echo "	<TD>".($vip[$arcount[$i]])."&nbsp;</td>\n";
						echo "	<TD>".($address[$arcount[$i]])."&nbsp;</td>\n";
						echo "	<TD>".($arsex[$arcount[$i]]%2==0?"여자":"남자")."</td>\n";
						echo "	<TD>".(ord($arage[$arcount[$i]])==0?"&nbsp;":$arage[$arcount[$i]])."</td>\n";
