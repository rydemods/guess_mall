		...
		if($insert = pmysql_query($sql,get_db_conn())) {
			...
			if( count( $spl_option_code ) > 0 ){
				foreach( $spl_option_code as $splKey=>$splVal ) {
					...
					$spl_optInsertSql.= "( option_code, productcode, option_price, option_quantity, option_use ) ";
					$spl_optInsertSql.= "VALUES ( '".$splVal."', '".$code.$productcode."', '".$spl_option_price[$splKey]."', '".$spl_option_quantity[$splKey]."', '".$spl_option_use[$splKey]."' ) ";
...
<?php
		$sql = "SELECT vender,id,com_name FROM tblvenderinfo";
		$result=pmysql_query($sql,get_db_conn());
		while($row=pmysql_fetch_object($result)) {
...
?>
					...
					<tr>
						<th><span class="font_orange"><b><?=($gongtype=="Y"?"교육 할인가":"교육 할인가")?></b></span></th>
						<td class="td_con1"><input name=sellprice value="<?=$_data->sellprice?>" size=16 maxlength=10 class="input" style=width:98%></td>
						<th><span class="font_orange"><b><?=($gongtype=="Y"?"인터넷최저가":"인터넷최저가")?></b></span></th>
...
?>
								<span class="font_orange"><b><?=($gongtype=="Y"?"교육 할인가":"교육 할인가")?></b></span>
<?php 
			} 
?>
							</td>
...
<!-- 옵션 수정 START -->
					<tr>
						<th scope="row"><span>상품선택옵션</span></th>
						<td colspan="2">
...
?>
			...
			<!-- 정보고시 등록/수정 안보이게 임시로 막음 2015 10 27 유동혁 -->
			...
			<tr style='display: none;'>
				...
				<tr>
					...
					<tr>
						<!--<th><span>종류</span></th>-->
<?php
	if(!$sabangnet_prop_option[1]){
		$sabangnet_prop_option[1]="종류";
	}
?>
						<th><input type="text" id="prop_opt0011" value="<?=$sabangnet_prop_option[1];?>" style=""/></th>
						<td><input type=text id=prop0011 value="<?=$sabangnet_prop_val[1];?>" style="width:450px;"></td>
						<!--<th><span>제조국</span></th>-->
<?php
	if(!$sabangnet_prop_option[2]){
		$sabangnet_prop_option[2]="제조국";
	}
?>
						<th><input type="text" id="prop_opt0012" value="<?=$sabangnet_prop_option[2];?>" style=""/></th>
						<td><input type=text id=prop0012 value="<?=$sabangnet_prop_val[2];?>" style="width:450px;"></td>
					</tr>
					<tr>
						<!--<th><span>소재</span></th>-->
<?php
	if(!$sabangnet_prop_option[3]){
		$sabangnet_prop_option[3]="소재";
}?>
						<th><input type="text" id="prop_opt0013" value="<?=$sabangnet_prop_option[3];?>" style=""/></th>
						<td><input type=text id=prop0013 value="<?=$sabangnet_prop_val[3];?>"  style="width:450px;"></td>
						<!--<th><span>취급시주의사항</span></th>-->
<?php
	if(!$sabangnet_prop_option[4]){
		$sabangnet_prop_option[4]="취급시 주의사항";
	}
?>
						<th><input type="text" id="prop_opt0014" value="<?=$sabangnet_prop_option[4];?>" style=""/></th>
						<td><input type=text id=prop0014 value="<?=$sabangnet_prop_val[4];?>"  style="width:450px;"></td>
					</tr>
					<tr>
						<!--<th><span>색상</span></th>-->
<?php
	if(!$sabangnet_prop_option[5]){
		$sabangnet_prop_option[5]="색상";
	}
?>
						<th><input type="text" id="prop_opt0015" value="<?=$sabangnet_prop_option[5];?>" style=""/></th>
						<td><input type=text id=prop0015 value="<?=$sabangnet_prop_val[5];?>" style="width:450px;"></td>
						<!--<th><span>품질 보증기준</span></th>-->
<?php
	if(!$sabangnet_prop_option[6]){
		$sabangnet_prop_option[6]="품질 보증기준";
	}
?>
						<th><input type="text" id="prop_opt0016" value="<?=$sabangnet_prop_option[6];?>" style=""/></th>
						<td><input type=text id=prop0016 value="<?=$sabangnet_prop_val[6];?>" style="width:450px;"></td>
					</tr>
					<tr>
						<!--<th><span>크기</span></th>-->
<?php
	if(!$sabangnet_prop_option[7]){
		$sabangnet_prop_option[7]="크기";
	}
?>
						<th><input type="text" id="prop_opt0017" value="<?=$sabangnet_prop_option[7];?>" style=""/></th>
						<td><input type=text id=prop0017 value="<?=$sabangnet_prop_val[7];?>"  style="width:450px;"></td>
						<!--<th><span>고객센터 전화번호</span></th>-->
<?php
	if(!$sabangnet_prop_option[8]){
		$sabangnet_prop_option[8]="고객센터 전화번호";
	}
?>
						<th><input type="text" id="prop_opt0018" value="<?=$sabangnet_prop_option[8];?>" style=""/></th>
						<td><input type=text id=prop0018 value="<?=$sabangnet_prop_val[8];?>" style="width:450px;"></td>
					</tr>
						<!--<th><span>제조자</span></th>-->
<?php
	if(!$sabangnet_prop_option[9]){
		$sabangnet_prop_option[9]="제조자";
	}
?>
						<th><input type="text" id="prop_opt0019" value="<?=$sabangnet_prop_option[9];?>" style=""/></th>
						<td><input type=text id=prop0019 value="<?=$sabangnet_prop_val[9];?>" style="width:450px;"></td>
						<!--<th><span></span></th>-->
						<th><input type="text" id="prop_opt00110" value="<?=$sabangnet_prop_option[10];?>" style=""/></th>
						<td><input type=text id=prop00110 value="<?=$sabangnet_prop_val[10];?>" style="width:450px;"></td>
					<tr>
						<!--<th><span>수입자</span></th>-->
<?php
	if(!$sabangnet_prop_option[11]){
		$sabangnet_prop_option[11]="수입자";
	}
?>
						<th><input type="text" id="prop_opt00111" value="<?=$sabangnet_prop_option[11];?>" style=""/></th>
						<td><input type=text id=prop00111 value="<?=$sabangnet_prop_val[11];?>" style="width:450px;"></td>
						<th><input type="text" id="prop_opt00112" value="<?=$sabangnet_prop_option[12];?>" style=""/></th>
						<td><input type=text id=prop00112 value="<?=$sabangnet_prop_val[12];?>" style="width:450px;"></td>
					</tr>
					</table>
					</div>
					<tr><td height=10></td></tr>
