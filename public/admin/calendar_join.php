...
var inputobj="";
/*
	HTML구조가 아래 형식이면 텍스트 박스를 클릭하지 않아도 달력 출력 가능
	<div><input type="text" name="date1"/></div><a href="javascript:;" class="btn_calen CLS_cal_btn"></a> 
*/
function Calendar(e) {
	...
	var strtop = event.clientY+$(window).scrollTop();
	if( dy_calOpen == 'n' ){
		if(tagNm == 'A' || tagNm == 'BUTTON'){
			var now = $(eventElement).prev().find("input[type='text']").val().split("-");
		}else{
...
function doClick(date) {															// 날자를 선택하였을 경우
	...
	if( tagNm == "INPUT" ){
		eventElement.value = cal_Day;
	}else{
		if(tagNm == 'A' || tagNm == 'BUTTON'){
			$(eventElement).prev().find("input[type='text']").val(cal_Day);
		}else{
			eventElement.innerHTML = cal_Day;
		}
	}
	dy_calOpen = 'n';
...
function Show_cal(sYear, sMonth, sDay) {
	...
	Cal_HTML += "			<a href=\"#1\" onclick=\"doOut();\"><img src=\"/admin/images/btn_close.gif\"></a>";
	Cal_HTML += "			</div>";
	Cal_HTML += "		</div>";
	Cal_HTML += "	</div>";
