...
if ((!$del_ok) || ($del_ok == -1)) {
	$errmsg="삭제할 글이 없습니다.\\n\\n다시 확인 하십시오.";
	alert_go($errmsg,"{$_SERVER['PHP_SELF']}?board=$board&s_check=$s_check&search=$search&block=$block&gotopage=$gotopage");
} else {
	...
	if ($mode == "delete") {
		...
		if($delete) {
			...
			alert($errmsg,"{$_SERVER['PHP_SELF']}?board=$board&s_check=$s_check&search=$search&block=$block&gotopage=$gotopage");
			exit;
		}
