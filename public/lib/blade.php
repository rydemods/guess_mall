...
class Blade {
	 ...
	 * @return void
	 */
	public static function sharpen($path)
	{
			...
			if ( ! str_contains($path, BLADE_EXT))
			{
				return;
			}
			$path = realpath($path);
			...
			if ( ! file_exists($compiled) or Blade::expired($path))
			{
				file_put_contents($compiled, Blade::compile($path));
			}
