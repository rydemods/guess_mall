		...
		<form name=form1 id = 'ID_goodsviewfrm' method=post action="<?=$Dir.FrontDir?>basket.php">
			...
			<table class="goods_info">
							...
							<option value="">옵션을 선택해주세요.</option>
								<?for($i=1;$i<$count;$i++) {?>
									<?if(strlen($tok[$i]) > 0) {?>
										...
										<?}else{?>
											<span><?=$tok[$i]?></span>&nbsp;(<?=number_format($optprice[$i-1])?>원)
										<?}?>
									<?}?>
									</option>
								<?}?>
				...
				<tr>
					...
					<td>
						<div class="select_type" style="width:180px;z-index:10;">
							<select name="option2" id="option2" style="display:none;" alt='<?=$tok[0]?>'>
							<option value="">옵션을 선택해주세요..</option>
								<?for($i=1;$i<$count;$i++) {?>
									<?if(strlen($tok[$i]) > 0) {?>
										<option value="<?=$i?>">
			...
			<input type = 'hidden' value = '<?=$priceDcPercent?>' id = 'ID_priceDcPercent'>
				...
				<script>
					...
					$(function(){
						...
						$("#option1").change(function(){
							var val = $("#option1 option:selected").val();
							if (val != "" ){
								$("#option2").show();
							} else {
								$("#option2").hide();
						...
						$("#option2").change(function(){
							...
							appendHtml += "	-<?=$_pdata->productname?>,"+option1Arr_[val1]+","+option2Arr_[val2]+" <p id='itemPrice-"+controlIdx_+"'>"+jsSetComa(itemTotalPrice)+"원</p>";
							appendHtml += "</div>";
							...
							appendHtml += "			<span class='item_ea_up' onclick='javascript:item_ea_up(\""+controlIdx_+"\",\""+val1+"\");'>더하기</span>";
							appendHtml += "			<span class='item_ea_dn' onclick='javascript:item_ea_dn(\""+controlIdx_+"\",\""+val1+"\");'>빼기</span>";
							appendHtml += "		</div>";
							appendHtml += "		<div style='float:right;'><span><img src='../img/btn/btn_close03.gif' alt='삭제'  onclick='javascript:items_del(\""+controlIdx_+"\",\""+val1+"\");' class='item_del' id='item_del' /></span></div>";
							appendHtml += "	</div>";
							appendHtml += "</li>";
							controlIdx_++;
				...
				<tr>
					...
					?>
						...
						<input type = 'text' value = '<?=$SellpriceValue?>' id = 'ID_goodsprice' name="ID_goodsprice">
					<?
						}
					?>
		...
		<div class="right_info_wrap">
			<div class="info">
				<table class="info" width="166">
					...
					<tr>
						<th>무이자할부</th>
						<td><a href="javascript:setcardInfo();">해당카드사 확인</a></td>
					</tr>
					<tr>
						<th>원산지</th>
						<td><?=$_pdata->madein?></td>
