...
foreach ( $arrSecondCategory as $key => $arrData ) {
	...
	$item_list_html .= '
                <li>
                        <a href="../front/productlist.php?code=' . $secondCateCode . '&category=second">' . $code_b_name . '</a>
                            <ul class="category-sub" >';

	while ( $row = pmysql_fetch_object($result) ) {
		$item_list_html .= '<li id='.$row->cate_code.'><a href="../front/productlist.php?code=' . $row->cate_code . '&category=third">' . $row->code_name . '</a></li>';
	}
	pmysql_free_result($result);
...
p.content, p.over_minimage, p.rate, p.vcnt, p.prodcode, p.colorcode, re.marks_total_cnt,
