...
<!--  사은품선택 -->
		...
		<tr>
			...
			<tr>
				<td colspan=3><IMG SRC="<?=$Dir?>images/common/order/<?=$_data->design_order?>/order_skin_stitle7.gif" vspace="3"></td>
			</tr>
			<tr>
...
<!--총결제 금액 종료-->
	...
	<tr>
		<td>
			...
			<table cellpadding="0" cellspacing="0" width="100%" height="100%">
			<tr>
				<td><IMG SRC="<?=$Dir?>images/common/order/<?=$_data->design_order?>/order_skin_stitle6.gif" vspace="3"></td>
			</tr>
			<tr>
				<td height="2"></td>
			</tr>
...
<!-- 결제수단 선택 -->
		...
		<!--div class="card_type" id="card_type" style="display:none">
			<table summary="임금계좌를 선택" style="text-align:left">
				<caption>임금계좌</caption>
				<colgroup>
					<?if($etcmessage[2]=="Y") {?><col width="13%" /><?}?>
					<col />
				</colgroup>
				<?if($etcmessage[2]=="Y") {?>
				<tr>
		...
		?>
			...
			<div class="order_gift">
				<h5><em>사은품</em><br />선택하기</h5>
				<ul class="order_gift_list">
				<?
					while($gift_row=pmysql_fetch_array($gift_res)){
				...
				?>
					<li><input id="dev_gift<?=$i?>" type="radio" name="gift_sel" value="<?=$gift_row['gift_regdate']?>"  <?if($i==0){ echo "checked"; }?>/>
					<img src="<?=$gift_image_src?>" style="width:68px;height:68px;border:0px" alt="" /><label for="dev_gift<?=$i?>"><span><?=$gift_row['gift_name']?></span></label>
					</li>
				<?
					$i++;	
					}
