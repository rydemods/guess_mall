	...
	<TR height="50">
		...
		<tr>
			...
			<?}else{?>
			<A HREF="board.php?pagetype=write&exec=reply&board=<?=$board?>&num=<?=$num?>&s_check=<?=$s_check?>&search=<?=$search?>&block=<?=$nowblock?>&gotopage=<?=$gotopage?>">
			<?}?>
			
			<IMG SRC="<?=$imgdir?>/butt-reply.gif" border=0></A><?=$reply_end?>
			...
			<?=$hide_write_start?>
			<?if($mypageid){?>
			<A HREF="board.php?pagetype=write&exec=write&board=<?=$board?>&mypageid=<?=$mypageid?>">
			<?}else{?>
			<A HREF="board.php?pagetype=write&exec=write&board=<?=$board?>">
			<?}?>
			
			
			<IMG SRC="<?=$imgdir?>/butt-write.gif" border=0></A><?=$hide_write_end?>

			</td>
			<TD align=right>
			<?if($mypageid){?>
			<A HREF="board.php?pagetype=list&board=<?=$board?>&s_check=<?=$s_check?>&search=<?=$search?>&block=<?=$nowblock?>&gotopage=<?=$gotopage?>&mypageid=<?=$mypageid?>">
			<?}else{?>
			<A HREF="board.php?pagetype=list&board=<?=$board?>&s_check=<?=$s_check?>&search=<?=$search?>&block=<?=$nowblock?>&gotopage=<?=$gotopage?>">
			<?}?>
			
			<IMG SRC="<?=$imgdir?>/butt-list.gif" border=0></A>
