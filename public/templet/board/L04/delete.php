...
<tr>
	...
	<tr>
		<td align="center"><INPUT type=image src="<?=$imgdir?>/board_btn01.gif" border="0" style="border:none;width:52;height:24">
		<?if($mypageid){?>
		<A HREF="board.php?pagetype=list&board=<?=$board?>&s_check=<?=$s_check?>&search=<?=$search?>&block=<?=$block?>&gotopage=<?=$gotopage?>&mypageid=<?=$mypageid?>">
		<?}else{?>
		<A HREF="board.php?pagetype=list&board=<?=$board?>&s_check=<?=$s_check?>&search=<?=$search?>&block=<?=$block?>&gotopage=<?=$gotopage?>">
		<?}?>
		
		
		<IMG SRC="<?=$imgdir?>/board_btn02.gif" border=0 hspace="5"></A></td>
	</tr>
