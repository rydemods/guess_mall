			...
			<ul>
				...
				<li>
					<ul class="board_list_down">
						<li><a href="#"><img src="/image/recipe/bt_list_up.gif" alt="이전글" /></a></li>
				...
				<li>
					<ul class="board_list_up">
						<li><a href="#"><img src="/image/recipe/bt_list_down.gif" alt="다음글" /></a></li>
...
<?}else{?>
		...
		<TR height="50">
			...
			<tr>
				...
				<?= $hide_delete_start ?>
				<A HREF="board.php?pagetype=passwd_confirm&exec=modify&board=<?=$board?>&num=<?=$num?>&s_check=<?=$s_check?>&search=<?=$search?>&block=<?=$block?>&gotopage=<?=$gotopage?>" class="btn_ty02">수정하기</A>
				<A HREF="board.php?pagetype=passwd_confirm&exec=delete&board=<?=$board?>&num=<?=$num?>&s_check=<?=$s_check?>&search=<?=$search?>&block=<?=$block?>&gotopage=<?=$gotopage?>" class="btn_ty02">삭제하기</A>
				<?= $hide_delete_end ?>

				<?=$hide_write_start?><A HREF="board.php?pagetype=write&exec=write&board=<?=$board?>" class="btn_ty03">글쓰기</A><?=$hide_write_end?>

				</td>
				<TD align=right>
				
				<A HREF="board.php?pagetype=list&board=<?=$board?>&s_check=<?=$s_check?>&search=<?=$search?>&block=<?=$nowblock?>&gotopage=<?=$gotopage?>" class="btn_ty01">목록보기</A>
