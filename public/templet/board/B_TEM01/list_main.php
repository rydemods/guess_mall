			...
			<TR>
				...
				<tr>
					...
					<tr>
						...
						<A HREF="javascript:comment_view('<?=$row['num']?>')" class="btn_ty01">덧글쓰기</A> 
						<? } ?>

						</td>
						<td align="right">
						<?= $hide_delete_start ?>
						<A HREF="board.php?pagetype=passwd_confirm&exec=modify&board=<?=$board?>&num=<?=$row['num']?>&s_check=<?=$s_check?>&search=<?=$search?>&block=<?=$block?>&gotopage=<?=$gotopage?>"  class="btn_ty01">수정하기</A><A HREF="board.php?pagetype=passwd_confirm&exec=delete&board=<?=$board?>&num=<?=$row['num']?>&s_check=<?=$s_check?>&search=<?=$search?>&block=<?=$block?>&gotopage=<?=$gotopage?>"  class="btn_ty02">삭제하기</A>
						<?= $hide_delete_end ?>
						</td>
					</tr>
					</table>
