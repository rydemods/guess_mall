<?PHP
    ...
    $orderTitle             = $_REQUEST['orderTitle'];
    // 중복 결제를 막기위하여 추가 2016-06-30 유동혁
    $paycode                = $_REQUEST['paycode'];
    $basketidxs             = $_REQUEST['basketidxs'];

    // 현재 결제 프로세스가 모바일인지 아닌지 체크
    $bMobile = false;
...
//	$TotalProductPaymentAmt			= $TotalProductPaymentAmt + $productPaymentAmt;		// 주문정보를 구성하기 위한 상품들 누적 결제 금액(상품 결제 금액)
    ...
    $productAmt                     = $_REQUEST['totalProductPaymentAmt'];
    $productPaymentAmt              = $_REQUEST['totalProductPaymentAmt'];
    $TotalProductPaymentAmt         = $_REQUEST['totalProductPaymentAmt'];
	
//	$iOption						= "신발사이즈";										// 옵션 ( 최대 100 자리 )
	$iOption						= "";										        // 옵션 ( 최대 100 자리 )
	$sortOrdering					= $OrderNumber;										// (필수) 상품노출순서, 10자 이내
//	$productName					= $_REQUEST['orderTitle'];		                    // (필수) 상품명, 4000자 이내
	$productName					= $orderTitle;		                    // (필수) 상품명, 4000자 이내
...
//	$sellerOrderProductReferenceKey = "ITEM_100001"	;									// 외부가맹점에서 관리하는 주문상품 연동 키(sellerOrderProductReferenceKey)는 주문 별로 고유한 key이어야 한다.(주문당 1건에 대한 상품을 보낼시엔 해당사항없음) ===> payco_config.php에 정의되어 있음.
	...
	$virtualAccountExpiryYmd					= date("YmdHis", strtotime("+3 days"));

//	$cancelMobileUrl 							= "http://www.payco.com";
	$cancelMobileUrl 							= "";
    if ( $bMobile ) {
        $cancelMobileUrl                        = "http://" . $_SERVER['HTTP_HOST'] . "/m/basket.php";
    }
...
//			$extraDataArray["appUrl"] 					= "Intent://shinwon_install#Intent;scheme=ko_shinwon;package=com.shinwon.android;end";
			$appUrl = "Intent://shinwon_install#Intent;scheme=ko_shinwon;package=com.shinwon.android;end";														 // IOS 인앱 결제시 ISP 모바일 등의 앱에서 결제를 처리한 뒤 복귀할 앱 URL
		} else if (preg_match('/(iPod|iPhone)/i', $_SERVER['HTTP_USER_AGENT'])) {
//			$extraDataArray["appUrl"] 					= "shinwon://open";
			$appUrl = "shinwon://open";														 // IOS 인앱 결제시 ISP 모바일 등의 앱에서 결제를 처리한 뒤 복귀할 앱 URL
		}
	}
...
//	$orderTitle						= $_REQUEST['orderTitle'];											// 주문 타이틀	

    // 주문완료 시 PAYCO에서 호출할 가맹점의 Service API의 URL
	$serviceUrl						= $AppWebPath."/payco_callback.php";
	...
	$cartNoArray["orderCode"] = $sellerOrderReferenceKey;     // 주문번호
    $cartNoArray['paycode'] = $paycode;                    // 결제 확인용 번호
    $cartNoArray['basketidxs'] = $basketidxs;              // 결제 확인용 basketidx
...
//		$payco1 = urldecode(stripslashes(json_encode($strJson)));
//		Write_Log("payco_reserve.php is Called >>>> res : $payco1");
		echo $res;
	} catch ( Exception $e ) {
		$Error_Return				= array();
