 ...
 */
	 ...
	 * 3. 지불 정보 설정 *
	 *********************/
	$inipay->m_inipayHome = $_SERVER['DOCUMENT_ROOT']."/".RootPath."paygate/D/escrow"; 	// 이니페이 홈디렉터리
	$inipay->m_type = "securepay"; 					// 고정 (절대 수정 불가)
	$inipay->m_pgId = "INIpay".$pgid; 				// 고정 (절대 수정 불가)
	$inipay->m_subPgIp = "203.238.3.10"; 			// 고정 (절대 수정 불가)
	$inipay->m_keyPw = $pgid_info["EKEY"]; 			// 키패스워드(상점아이디에 따라 변경)
     ...
     * 가상계좌입금내역수신모듈 설정 *                                *
     *-----------------------------------------------------------------*/
    $inipay->m_returntype = "U"; // URL 통보 방식 : U , java 데몬 수신: J , window 데몬 수신 :W
    $inipay->m_returnurl = "http://".$shopurl."/paygate/".$pg_type."/inicis_process.php";  // URL 로 전달 받기 위한 상점 수신 URL
    $inipay->m_returnip = $_SERVER['SERVER_ADDR'];   // tcp/ip 통신을 통한 전달을 받기 위한 상점 IP
    $inipay->m_returnport = $_SERVER['SERVER_PORT'];  // tcp/ip 통신을 통한 전달을 받기 위한 상점 port
	...
	if($row=pmysql_fetch_object($result)) {
		...
		if($paymethod_self=="P") $tblname="tblpcardlog";
		else if($paymethod_self=="Q") $tblname="tblpvirtuallog";
	}
	pmysql_free_result($result);
	...
	if(strlen($tblname)>0) {
		...
		if($row=pmysql_fetch_object($result)) {
			...
			if($paymethod_self=="P") $PAY_AUTH_NO = "00000000";
		}
		pmysql_free_result($result);
	}
	...
	if($isreload!=true) {
		...
		if ($inipay->m_resultCode == "00") {	//정상승인
			...
			$sql = "INSERT INTO {$tblname}(ordercode) VALUES ('{$ordr_idxx}') ";
			pmysql_query($sql,get_db_conn());
			...
			$sql.= "msg				= '".$MSG1."' WHERE ordercode='{$ordr_idxx}' ";
			pmysql_query($sql,get_db_conn());
			//backup_save_sql($sql);
			...
			if ($gopaymethod == "onlycard") {	//신용카드
				$tblname="tblpcardlog";
				$paymethod_self="P";
			} else if ($gopaymethod == "onlydbank") { //가상계좌
				$tblname="tblpvirtuallog";
				$paymethod_self="Q";
				$card_name="";
				$noinf="";
