...
foreach($tag as $i => $v){
	...
	$posting_sql.= "SELECT ROW_NUMBER() OVER(order by access desc) AS ROWNUM,* FROM( ";
	...
	WHERE (".$qry4.") ";
	$posting_sql .= " ) a";
	$posting_sql .= " ) INFO WHERE rownum > ".$rownum." ORDER BY ROWNUM  LIMIT 1";
	...
	$check_sql .= "SELECT 'forum_list' as type, f.index::varchar as idx, f.title ,f.content, f.img as img_file, f.writetime::varchar, '' as link_url,'' as link_m_url, f.view as access, li.section,
		COALESCE((select COUNT( tl.hott_code )AS hott_cnt from tblhott_like tl WHERE tl.section = 'forum_list' AND f.index::varchar = tl.hott_code),0) AS hott_cnt
	FROM tblforumlist f
	LEFT JOIN ( SELECT hott_code, section FROM tblhott_like WHERE section = 'forum_list' AND like_id = '".$_ShopInfo->getMemid()."' GROUP BY hott_code, section ) li on f.index::varchar = li.hott_code
	WHERE (".$qry4.") ";
	...
	if(count($arrPosting) > 0){?>
		...
		<input type="hidden" id="chk_rownum" value="<?=$chk_rownum ?>" />
	<?}else{?>
