...
if($mode == "insert"){
	...
	if(!pmysql_error()){
		echo  "<script>alert(' 정상적으로 등록되었습니다.'); location.href=\"/front/mypage_as.php\"</script>";
	}else{
	...
	if(!pmysql_error()){
		echo  "<script>alert('수정이 완료되었습니다.'); location.href=\"/front/mypage_as.php\"</script>";
	}else{
		echo  "<script>alert('오류가 발생하였습니다.'); location.href=\"/front/front/mypage_as.php\"</script>";
	}
} else if ($mode == "delete"){
	#파일삭제
	$filepath = $Dir.DataDir."shopimages/personal/";
	...
	if(!pmysql_error()){
		echo  "<script>alert('삭제가 완료되었습니다..'); location.href=\"/front/mypage_as.php\"</script>";
	}else{
		//alert_go('오류가 발생하였습니다.', $_SERVER['REQUEST_URI']);
		echo  "<script>alert('오류가 발생하였습니다.'); location.href=\"/front/mypage_as.php\"</script>";
	}
}
