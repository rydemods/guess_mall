...
?>
			<script>
				parent.certi_return('0','<?=$check_ipin_data->name?>','<?=$check_ipin_data->id?>','<?=str_replace("-","",$_SESSION[ipin][mobileno])?>');
			</script>
<?php
		}else{
?>
			<script>
				parent.certi_return('1','','','<?=str_replace("-","",$_SESSION[ipin][mobileno])?>');
			</script>
<?php
