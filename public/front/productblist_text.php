...
if(strpos(" ".$body,"[BRANDEVENT]")) {
	if($_bdata->title_type=="image") {
		...
		if(ord($_bdata->title_body)) {
			if (strpos(strtolower($_bdata->title_body),"<table")!=false)
				$codeevent=$_bdata->title_body;
			else
				$codeevent=nl2br($_bdata->title_body);
		}
	}
}

$codegroup="";
if($_data->ETCTYPE["CODEYES"]=="Y" && ord($brand_qryA)) {
	if($num=strpos($body,"[BRANDGROUP]")) {
		...
		if(ord($likecode)==0) {
			...
			$sql.= "AND (type='L' OR type='T' OR type='LX' OR type='TX') ";
			$sql.= $brand_qryA;
			...
			if($cnt>0) {
				...
				while($row=pmysql_fetch_object($result)) {
					...
					$codegroup.="	<td id=group1_td><a href=\"".$Dir.FrontDir."productblist.php?{$brand_link}code=".$row->code_a.$row->code_b.$row->code_c.$row->code_d."\"><FONT class=upcodename>{$row->code_name}</FONT></a></td>\n";
					...
					if(!strstr($row->type,"X")) {
						...
						while($row2=pmysql_fetch_object($result2)) {
							$_[]="<a href=\"".$Dir.FrontDir."productblist.php?{$brand_link}code=".$row2->code_a.$row2->code_b.$row2->code_c.$row2->code_d."\"><FONT class=subcodename>{$row2->code_name}</FONT></a>";
						}
						$codegroup .= implode(" | ",$_);
				...
				while($row=pmysql_fetch_object($result)) {
					if($i>0) $codegroup.=" | ";
					$codegroup.="<a href=\"".$Dir.FrontDir."productblist.php?{$brand_link}code=".$row->code_a.$row->code_b.$row->code_c.$row->code_d."\"><FONT class=subcodename>{$row->code_name}</FONT></a>";
					$i++;
				}
				$codegroup.="	</td>\n";
				$codegroup.="</tr>\n";
			...
			if($_cdata->type!="LX" && $_cdata->type!="TX") {	//하위분류가 있을 경우에만
				...
				while($row=pmysql_fetch_object($result)) {
					if($i>0) $codegroup.=" | ";
					$codegroup.="<a href=\"".$Dir.FrontDir."productblist.php?{$brand_link}code=".$row->code_a.$row->code_b.$row->code_c.$row->code_d."\"><FONT class=subcodename>";
			...
			while($row=pmysql_fetch_object($result)) {
				...
				if ($i!=0 && $i%$prlist1_cols!=0) {
					...
					if($prlist1_rowline=="N") $prlist1.="<img width=3 height=0>";
						...
						if(preg_match("/#prlist_rowline/i",$body)) {
				...
				$prlist1.= "<tr>";
				$prlist1.= "	<td align=center valign=top style=\"word-break:break-all;\"><A HREF=\"".$Dir.FrontDir."productdetail.php?{$brand_link}productcode=".$row->productcode.$add_query."&sort={$sort}\" onmouseover=\"window.status='상품상세조회';return true;\" onmouseout=\"window.status='';return true;\"><FONT class=\"prname\">".viewproductname($row->productname,$row->etctype,$row->selfcode)."</FONT></A></td>\n";
				$prlist1.= "</tr>\n";
				//모델명/브랜드/제조사/원산지
			...
			while($row=pmysql_fetch_object($result)) {
				...
				if ($i>0 && $i%$prlist2_cols==0) {
					if($prlist2_colline=="Y") {
						...
						if(preg_match("/#prlist_colline/i",$body)) {
				...
				$prlist2.= "	<table border=0 cellpadding=0 cellspacing=0 width=100%>\n";
				$prlist2.= "<tr>";
				$prlist2.= "	<td align=left valign=top style=\"word-break:break-all;\"><A HREF=\"".$Dir.FrontDir."productdetail.php?{$brand_link}productcode=".$row->productcode.$add_query."&sort={$sort}\" onmouseover=\"window.status='상품상세조회';return true;\" onmouseout=\"window.status='';return true;\"><FONT class=\"prname\">".viewproductname($row->productname,$row->etctype,$row->selfcode)."</FONT></A></td>\n";
				$prlist2.= "</tr>\n";
				//모델명/브랜드/제조사/원산지
