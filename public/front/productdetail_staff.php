...
$vqry = "SELECT staff_type,staff_limit FROM tblmember WHERE id= '{$_ShopInfo->memid}' ";
$vres = pmysql_query($vqry);
$vrow = pmysql_fetch_object($vres);
if ($vrow->staff_type!="1") {
...
$staff_limit = $vrow->staff_limit - $orow->pricesum;
...
if(strlen($productcode)==18) {
	...
	$sql.= "WHERE a.productcode='{$productcode}' AND a.display='N' ";
...
function CheckForm(gbn,temp2) {
	...
	var totalPriceTemp = document.form1.ID_goodsprice.value * document.form1.quantity.value;
	if(document.form1.staff_limit.value < totalPriceTemp){
		alert("이번달 구매가능 금액을 초과했습니다.");		
		return;
	}
