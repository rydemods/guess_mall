...
<?php 
	if($_minidata->main_toptype=="image") {
		...
		if(ord($_minidata->main_topdesign)) {
			echo "<table width=100% border=0 cellpadding=0 cellspacing=0>\n";
			echo "<tr>\n";
...
<?php 
	if($_minidata->hot_used=="1") {
		...
		if(ord($hot_disptype) && $hot_dispcnt>0) {
			...
			$sql.= "a.option_price, a.tag, a.minimage, a.tinyimage, a.etctype, a.option_price, a.selfcode FROM tblproduct AS a ";
			...
			if($_minidata->hot_linktype=="2") {
				...
				if($row2=pmysql_fetch_object($result2)) {
					...
					$codes = explode(',',$row2->special_list);
				}
...
<?php 
	if($_minidata->new_used=="1") {
		...
		pmysql_free_result($result);
		if(ord($new_disptype) && $new_dispcnt>0) {
			$sql = "SELECT a.productcode,a.productname,a.sellprice,a.quantity,a.consumerprice,a.reserve,a.reservetype,a.production, ";
...
?>
	<?php if($_minidata->code_distype!="NY"){?>
...
<?php 
		...
		for($i=0;$i<count($prdataA);$i++) {
			$tmpcode=$prdataA[$i]->code_a.$prdataA[$i]->code_b;
			echo " | ";
