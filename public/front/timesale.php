...
/*
if(ord($sno)==0) {
	alert_go('타임세일 정보가 없습니다.',$Dir.MainDir."main.php");
}
*/
##### 타임세일 정보
...
if(ord($productcode)==0) {
	if($sno){
		alert_go('해당 상품의 타임세일 기간이 아닙니다.',-1);
	}else{
		alert_go('타임세일 상품이 없습니다.',$Dir.MainDir."main.php");
	}
}
...
//할인 비율
//$_odata['ratio'] = floor(($_pdata->consumerprice-$_odata['s_price'])/$_odata['price']*100);
$_odata['ratio'] = floor((($_pdata->consumerprice-$_odata['s_price'])/$_pdata->consumerprice)*100);
//(감소값 / 최대값) * 100 = 할인률
...
function CheckForm(gbn,temp2) {
	...
	if(typeof($("#option2").val())){
		if($("#option1").val() == ""){
			...
			$("#optionArr").val($("#option1 option:selected").val()+"_"+$("#option2 option:selected").val());
		}
	}else if(typeof($("#option1").val())){
		if($("#option1").val() == ""){
			alert('해당 상품의 옵션을 선택하세요.');
			document.form1.option1.focus();
			return;
		}else{
			$("#optionArr").val($("#option1 option:selected").val()+"_0");
		}
	}
	...
	/*if(typeof(document.form1.option1)!="undefined" && (!$("#option1").val() && $("#option1").length > 0)) {
		alert('해당 상품의 옵션을 선택하세요.');
		document.form1.option1.focus();
		return;
	}
	if(typeof(document.form1.option2)!="undefined" && (!$("#option2").val() && $("#option2").length > 0)) {
		alert('해당 상품의 옵션을 선택하세요.');
		document.form1.option2.focus();
		return;
	}*/
	...
	//$("#optionArr").val("");
	$("#quantityArr").val(document.form1.quantity.value);
	//$("#priceArr").val("");
	$("#option1").val("");
	$("#option2").val("");
...
*/	
	if (strlen($_cdata->detail_type)==6 && $_cdata->detail_type[5]=="U") {
		...
		if($_ndata) {
			$body=$_ndata->body;
			$body=str_replace("[DIR]",$Dir,$body);
			include($Dir.TempletDir."timesale/detail_U.php");
		} else {
			include($Dir.TempletDir."timesale/detail_".substr($_cdata->detail_type,0,5).".php");
		}
	}else{
		include($Dir.TempletDir."timesale/detail_{$_cdata->detail_type}.php");
	}
?>
	</td>
</tr>
